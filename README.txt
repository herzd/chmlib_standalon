...
+ Installation verified on blank Debian 10 (see below) and Centos 7.

+ Recreates the environment used in original research papers of AG Zanghellini. The Debian installer uses a newer version of flint (2.7.0 instead of 2.5.2 as in the original environment), as 2.5.2 fails to build with gcc 8.3.0 at the flags '-r -pie' at compilation time.  Furthermore the resulting binary causes memory access errors at random occurence at about 15% times of invocations. Nevertheless the results calculated are the same.  For this reason we recommend to use the Centos-7 installer, as we could not detect these errors with the binaries created by Centos-7's gcc-4.9.

# package contents

+ The unpacked tarball has this structure:
------------------------------------------------------
~ $ tree chmlib_deb_cent
chmlib_deb_cent
├── deps
│   ├── installations
│   ├── sources
...
│   │   └── zlib-1.2.8.tar.gz
│   └── unpacked
├── install_centos7.bash
├── install_debian10.bash
├── install_no_depcheck_centos7.bash
├── install_no_depcheck_debian10.bash
├── README.txt
├── simpleNetr1r2r3.ine
└── uninstaller.bash

4 directories, 15 files
~ $ du -sh chmlib_deb_cent
35M	chmlib_deb_cent
---------------------------------------------------------------
