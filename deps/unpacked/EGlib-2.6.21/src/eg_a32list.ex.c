...
typedef struct
{
	...
	EGa32List_t cn;	/**< structure to put this together in a list */
}
integer_list_t;
...
int main (void)
{
	...
	for (i = 1; i < 20; i++)
	{
		...
		EGa32ListAddBefore (base,stsz,i,head);
	}
	...
	back_up=1;
	EGa32ListInit(base,stsz,back_up);
	for(i=3;i<20;i+=2)
	{
		EGa32ListAddBefore(base,stsz,i,back_up);
	}
	...
	EGa32ListMoveAfter(base,stsz,3,head);
	EGa32ListMoveBefore(base,stsz,18,back_up);
	fprintf (stderr, "Numbers in head:\n");
	it=head;
	do
	{
	...
	EGa32ListSplice (base,stsz,head,back_up);
	/* now we display both lists */
	fprintf (stderr, "Numbers in head:\n");
	it=head;
	do
	...
	EGa32ListDel(base,stsz,10);
	EGa32ListReplace(base,stsz,11,10);
