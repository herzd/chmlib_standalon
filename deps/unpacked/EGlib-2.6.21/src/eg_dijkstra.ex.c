...
static int32_t source=0;
static int verbose=1;
/* ========================================================================= */
static inline void djk_usage(const char*const progname)
{
...
/* ========================================================================= */
/** @brief parse-args */
static inline void djk_parseargs(int argc,char**argv)
{
	int c, has_file=0;
...
int main (int argc,char**argv)
{
	...
	EGlpNum_t*weight=0,*sweight=0,*dist=0;
	EGioFile_t* input=0;
	EGtimerReset(&dt);
	EGtimerReset(&rt);
	EGtimerReset(&pp);
	...
	djk_parseargs(argc,argv);
	input = EGioOpen(fname,"r");
	EGcallD(EGguReadXgraph(input,&n,&m,&edges,&weight));
	EGioClose(input);
	EGtimerStop(&rt);
	EGtimerStart(&pp);
	...
	dist=EGlpNumAllocArray(n);
	sweight=EGlpNumAllocArray(m);
	...
	for(i=m;i--;)
		EGlpNumCopy(sweight[in_perm[i]],weight[i]);
	EGtimerStop(&pp);
	EGtimerStart(&dt);
	EGcallD(EGalgDJK(n,m,in_d,in_beg,in_e,sweight,source,n,t,father,dist));
	EGtimerStop(&dt);
	...
	CLEANUP:
	EGlpNumFreeArray(weight);
	EGlpNumFreeArray(sweight);
	EGlpNumFreeArray(dist);
	EGfree(edges);
	EGfree(in_d);
	EGfree(in_e);
	EGfree(in_beg);
