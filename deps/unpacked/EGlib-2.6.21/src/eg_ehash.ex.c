...
int main (void)
{
	...
	EGeHashTable_t htable=EGeHashTableNull;
	EGeHashInfo_t hinfo[HASH_SIZE];
	unsigned success = 0;
	...
	EGlib_version();
	EGrandInit(&rstate);
	EGeHashTableInit(&htable,HASH_MAXSIZE);
	...
	for( i = HASH_SIZE ; i-- ; )
	{
		EGeHashTableAdd(&htable,hinfo+i);
	}
	EGtimerStop(&timer);
	...
	for(factor = 2 ; factor<HASH_MAXFACTOR ; factor=(factor*3)/2) 
	{
		...
		while(j--)
		{
			...
			{
				key = EGrand(&rstate);
				info = EGeHashTableFind(&htable,key);
				if(info)
				{
					success++;
					info = EGeHashTableFindNext(&htable,info);
				}	
			}
		}
		EGtimerStop(&timer);
		...
		EGeHashTableResize(&htable,HASH_MAXSIZE*factor);
	}
	CLEANUP:
	EGeHashTableClear(&htable);
	return rval;
}
