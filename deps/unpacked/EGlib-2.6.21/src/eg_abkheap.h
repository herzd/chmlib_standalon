 ...
 * @par History:
 * - 2013-04-12
 * 						- First implementation, starting from base EGeHeap_t 
 * @note 
...
/** @{ */
/** @example eg_abkheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaBKheap */
/* ========================================================================= */
#ifndef __EG_ABKHEAP__
#define __EG_ABKHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef EG_ABKHEAP_DEBUG
#define EG_ABKHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if EG_ABKHEAP_DEBUG <= DEBUG
#define EGaBKheapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define EGaBKheapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define EGaBKheapCHECK_CN(__hp,__hcn) 
#define EGaBKheapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
/** @brief number of maximum entries in the vector values */
#ifndef EG_ABKHEAP_ENTRY
#define EG_ABKHEAP_ENTRY 3
#endif
/* ========================================================================= */
...
typedef struct 
{
	EGlpNum_t val[EG_ABKHEAP_ENTRY];		/**< Value of this node in the heap */
	uint32_t pos;					/**< Position in the heap-array for this node, if set to
												 #EG_ABKHEAP_POISON, then the connector is not in any 
												 heap.*/
}
EGaBKheapCn_t;
/* ========================================================================= */
/** @brief Poison position for heap connector not in a heap. */
#define EG_ABKHEAP_POISON UINT32_MAX
/* ========================================================================= */
...
#define EGaBKheapIsLess(__hcn1,__hcn2) ({\
	const EGlpNum_t*const __EGABKH1 = (__hcn1);\
	const EGlpNum_t*const __EGABKH2 = (__hcn2);\
	int __EGABKHj = 0, __EGABKHrval = 0;\
	for( ; __EGABKHj < EG_ABKHEAP_ENTRY ; __EGABKHj++)\
	{\
		if(EGlpNumIsLess(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj])){\
			__EGABKHrval = 1; break;}\
		else if (EGlpNumIsNeq(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj],epsLpNum)){\
 ...
 * */
#define EGaBKheapCnInit(__hcn) do{\
	EGaBKheapCn_t*const __ABKHcn=(__hcn);\
	int __ABKHi = EG_ABKHEAP_ENTRY;\
	memset(__ABKHcn,0,sizeof(EGaBKheapCn_t));\
	for( ; __ABKHi-- ; ){EGlpNumInitVar(__ABKHcn->val[__ABKHi]);}\
	__ABKHcn->pos = EG_ABKHEAP_POISON;}while(0)
/* ========================================================================= */
 ...
 * */
#define EGaBKheapCnReset(__hcn) ((__hcn)->pos = EG_ABKHEAP_POISON)
/* ========================================================================= */
 ...
 * */
#define EGaBKheapCnClear(__hcn) do{\
	EGaBKheapCn_t*const __ABKHcn = (__hcn);\
	int __ABKHi = EG_ABKHEAP_ENTRY;\
	for( ; __ABKHi-- ; ){EGlpNumClearVar(__ABKHcn->val[__ABKHi]);}}while(0)
/* ========================================================================= */
 ...
 * */
#define EGaBKheapReset(__hp) EGaBKheapResize(__hp,0)
 ...
 * */
#define EGaBKheapClear(__hp) EGaBKheapResize(__hp,0)
 ...
 * */
#define EGaBKheapGetMin(__hp) ({\
	EGaBKheap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : EG_ABKHEAP_POISON;})
 ...
 * */
#define EGaBKheapResize(__hp,__new_sz) ({\
	EGaBKheap_t*const __EGehp = (__hp);\
 ...
 * */
#define EGaBKheapFatherId(__id) ((uint32_t)((__id)?(((uint32_t)((__id)-1))>>1U):0))
 ...
 * */
#define __EGaBKheapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const uint32_t __EGstsz3=((uint32_t)(__psz3));\
	EGaBKheap_t*const __EGehp3 = (__hp3);\
	const uint32_t __EGapos3 = __EGehp3->cn[__hcn3];\
	EGaBKheapCn_t*const __EGecn3 = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	uint32_t __EGcpos3 = __EGecn3->pos;\
	uint32_t __EGfpos3 = EGaBKheapFatherId(__EGcpos3);\
	EGaBKheapCn_t*__EGfcn3 = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	EGaBKheapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				EGaBKheapIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((uint32_t)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = EGaBKheapFatherId(__EGcpos3);\
		__EGfcn3 = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define EGaBKheapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	EGaBKheapCn_t*const __EGlcn = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	EGaBKheapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	__EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
/** @brief Give the first child for a given position.
 * @param __id position that we want to get the first child.
 * */
#define EGaBKheapFirstChildId(__id) ((uint32_t)(((uint32_t)(__id))<<1U)+1U)
/* ========================================================================= */
 ...
 * */
#define __EGaBKheapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const uint32_t __EGstsz=((uint32_t)(__psz));\
	EGaBKheap_t*const __EGehp = (__hp);\
	const uint32_t __EGapos = __EGehp->cn[__hcn];\
	EGaBKheapCn_t*const __EGecn = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const uint32_t __EGhsz = __EGehp->sz;\
	uint32_t __EGcpos = __EGecn->pos;\
	uint32_t __EGfchd = EGaBKheapFirstChildId(__EGcpos);\
	uint32_t __EGlchd = ((uint32_t)(__EGfchd + 2U));\
	EGaBKheapCn_t*__EGcchd=0,*__EGnchd=0;\
	EGaBKheapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		if(__EGlchd<=__EGhsz){\
			__EGnchd = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd+1])));\
			if(EGaBKheapIsLess(__EGnchd->val,__EGcchd->val)) __EGcchd=__EGnchd;}\
		/* if the minimum child is less than the current position, move the minimum\
		 * child to the position of the current element */\
		if(EGaBKheapIsLess(__EGcchd->val,__EGecn->val))\
		{\
			...
			__EGfchd = EGaBKheapFirstChildId(__EGcpos);\
			__EGlchd = ((uint32_t)(__EGfchd + 2U));\
		}\
		/* else we exit the main loop */\
		else __EGfchd = EG_ABKHEAP_POISON;\
	}\
}while(0)
/* ========================================================================= */
 ...
 * */
#define EGaBKheapCopyVal(__dst,__src) do{\
	EGlpNum_t*const __EGABKHdst=(__dst);\
	const EGlpNum_t*const __EGABKHsrc=(__src);\
	int __EGABKHi=EG_ABKHEAP_ENTRY;\
	for(;__EGABKHi--;){EGlpNumCopy(__EGABKHdst[__EGABKHi],__EGABKHsrc[__EGABKHi]);}}while(0)
/* ========================================================================= */
 ...
 * */
#define EGaBKheapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	EGaBKheap_t*const __EGlhp=(__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	EGaBKheapCn_t*__EGlcn2=((EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	if(EGaBKheapIsLess(__new_val,__EGlcn2->val)){\
		EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		__EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		__EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define EGaBKheapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	EGaBKheapCn_t*const __EGlhpcn = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const uint32_t __EGlcn = __EGlhpcn->pos;\
	const uint32_t __EGlhsz = __EGlhp->sz - 1;\
	EGaBKheapCn_t*const __EGllcn = ((EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = EG_ABKHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		__EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = EG_ABKHEAP_POISON;}while(0)
