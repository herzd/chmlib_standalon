...
typedef struct
{
	...
	EGa16List_t cn;	/**< structure to put this together in a list */
}
integer_list_t;
...
int main (void)
{
	...
	for (i = 1; i < 20; i++)
	{
		...
		EGa16ListAddBefore (base,stsz,i,head);
	}
	...
	back_up=1;
	EGa16ListInit(base,stsz,back_up);
	for(i=3;i<20;i+=2)
	{
		EGa16ListAddBefore(base,stsz,i,back_up);
	}
	...
	EGa16ListSplice (base,stsz,head,back_up);
	/* now we display both lists */
	fprintf (stderr, "Numbers in head:\n");
	it=head;
	do
	...
	EGa16ListDel(base,stsz,10);
	EGa16ListReplace(base,stsz,11,10);
