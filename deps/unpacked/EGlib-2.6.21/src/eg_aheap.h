...
#ifndef EG_AHEAP_DEBUG
#define EG_AHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if EG_AHEAP_DEBUG <= DEBUG
#define EGaHeapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define EGaHeapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define EGaHeapCHECK_CN(__hp,__hcn) 
#define EGaHeapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
...
typedef struct 
{
	...
	unsigned pos;					/**< Position in the heap-array for this node, if set to
												 #EG_AHEAP_POISON, then the connector is not in any 
												 heap.*/
}
EGaHeapCn_t;
...
#define EG_AHEAP_POISON UINT_MAX
 ...
 * */
#define EGaHeapCnInit(__hcn) do{EGlpNumInitVar((__hcn)->val);(__hcn)->pos = EG_AHEAP_POISON;}while(0)
 ...
 * */
#define EGaHeapCnReset(__hcn) ((__hcn)->pos = EG_AHEAP_POISON)
 ...
 * */
#define EGaHeapCnClear(__hcn) EGlpNumClearVar((__hcn)->val)
...
#define EGaHeapIsFull(__hp) ({EGaHeap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define EGaHeapEmpty(__hp) ((__hp)->sz = 0)
 ...
 * */
#define EGaHeapInit(__hp,__d) (*(__hp) = (EGaHeap_t){0,__d,0,0})
 ...
 * */
#define EGaHeapReset(__hp) EGaHeapResize(__hp,0)
 ...
 * */
#define EGaHeapClear(__hp) EGaHeapResize(__hp,0)
 ...
 * */
#define EGaHeapGetMinVal(__hp,__number) ({\
	EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? (EGlpNumCopy(__number,__EGehp->cn[0]->val),0):1;})
 ...
 * */
#define EGaHeapGetMin(__hp) ({\
	EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : EG_AHEAP_POISON;})
 ...
 * */
#define EGaHeapResize(__hp,__new_sz) ({\
	EGaHeap_t*const __EGehp = (__hp);\
 ...
 * */
#define EGaHeapFatherId(__d,__id) ((unsigned)((__id)?(((__id)-1)/(__d)):0))
 ...
 * */
#define __EGaHeapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const size_t __EGstsz3=((size_t)(__psz3));\
	EGaHeap_t*const __EGehp3 = (__hp3);\
	const unsigned __EGapos3 = __EGehp3->cn[__hcn3];\
	EGaHeapCn_t*const __EGecn3 = ((EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	unsigned __EGcpos3 = __EGecn3->pos;\
	unsigned __EGfpos3 = EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
	EGaHeapCn_t*__EGfcn3 = ((EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	EGaHeapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
	...
	{\
		...
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
		__EGfcn3 = ((EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define EGaHeapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	EGaHeapCn_t*const __EGlcn = ((EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	EGaHeapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	__EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
...
#define EGaHeapFirstChildId(__d,__id) ((unsigned)((__d)*(__id)+1U))
/* ========================================================================= */
 ...
 * */
#define __EGaHeapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const size_t __EGstsz=((size_t)(__psz));\
	EGaHeap_t*const __EGehp = (__hp);\
	const unsigned __EGapos = __EGehp->cn[__hcn];\
	EGaHeapCn_t*const __EGecn = ((EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const unsigned __EGhsz = __EGehp->sz;\
	unsigned __EGcpos = __EGecn->pos;\
	unsigned __EGfchd = EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
	unsigned __EGlchd = __EGfchd + __EGehp->d;\
	EGaHeapCn_t*__EGcchd=0,*__EGnchd=0;\
	register unsigned __EGehi = 0;\
	EGaHeapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		for(__EGehi = (unsigned)((__EGlchd > __EGhsz) ? __EGhsz-1U:__EGlchd-1U); __EGehi > __EGfchd ; __EGehi--){\
			if(EGlpNumIsLeq((__EGnchd=((EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGehi]))))->val,__EGcchd->val))\
				__EGcchd = __EGnchd;}\
		...
		{\
			...
			__EGfchd = EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
			__EGlchd = __EGfchd + __EGehp->d;\
		}\
		/* else we exit the main loop */\
		else __EGfchd = EG_AHEAP_POISON;\
	}\
}while(0)
 ...
 * */
#define EGaHeapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	EGaHeap_t*const __EGlhp=(__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	EGaHeapCn_t*__EGlcn2=((EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	EGlpNum_t __EGnum;\
	EGlpNumCopy(__EGnum,(__new_val));\
	if(EGlpNumIsLess(__EGnum,__EGlcn2->val)){\
		EGlpNumCopy(__EGlcn2->val,__EGnum);\
		__EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		EGlpNumCopy(__EGlcn2->val,__EGnum);\
		__EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define EGaHeapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	EGaHeapCn_t*const __EGlhpcn = ((EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const unsigned __EGlcn = __EGlhpcn->pos;\
	const unsigned __EGlhsz = __EGlhp->sz - 1;\
	EGaHeapCn_t*const __EGllcn = ((EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = EG_AHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		__EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = EG_AHEAP_POISON;}while(0)
