					...
					char **argv)
{
	...
	EGlpNum_t v[EG_ABKHEAP_ENTRY];
	EGaBKheap_t my_heap;
	EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	EGcallD(aheap_parseargs(argc, argv, &d, &c, v, &file_name));
	EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (file_name, "r");
	TEST (!file, "unable to open file %s", file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(EGaBKheapCn_t, nval);
	EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, EGlpNumToLf (all_cn[i].val[0]));
		EGaBKheapAdd (all_cn,sizeof(EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, EGlpNumToLf (all_cn[ccn].val[0]));
		EGaBKheapDel (all_cn,sizeof(EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) EGaBKheapAdd (all_cn,sizeof(EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, EGlpNumToLf (all_cn[c].val[0]), EGlpNumToLf (v[0]));
	EGaBKheapChangeVal (all_cn,sizeof(EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, EGlpNumToLf (all_cn[ccn].val[0]));
		EGaBKheapDel (all_cn,sizeof(EGaBKheapCn_t),&my_heap, ccn);
	}
