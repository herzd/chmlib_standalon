					...
					char **argv)
{
	...
	for (i = 0; i < nval; i++)
	{
		ccn = EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, EGlpNumToLf (all_cn[ccn].val));
		EGaHeapDel (all_cn,sizeof(EGaHeapCn_t),&my_heap, ccn);
	}
