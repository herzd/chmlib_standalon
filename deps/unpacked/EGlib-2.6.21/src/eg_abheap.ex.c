					...
					char **argv)
{
	...
	EGaBheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (file_name, "r");
	TEST (!file, "unable to open file %s", file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(EGaBheapCn_t, nval);
	EGaBheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		EGaBheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, EGlpNumToLf (all_cn[i].val));
		EGaBheapAdd (all_cn,sizeof(EGaBheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = EGaBheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, EGlpNumToLf (all_cn[ccn].val));
		EGaBheapDel (all_cn,sizeof(EGaBheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) EGaBheapAdd (all_cn,sizeof(EGaBheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, EGlpNumToLf (all_cn[c].val), EGlpNumToLf (v));
	EGaBheapChangeVal (all_cn,sizeof(EGaBheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, EGlpNumToLf (all_cn[ccn].val));
		EGaBheapDel (all_cn,sizeof(EGaBheapCn_t),&my_heap, ccn);
	}
