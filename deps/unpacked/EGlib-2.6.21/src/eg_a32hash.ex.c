...
int main (void)
{
	...
	EGa32HashTable_t htable=EGa32HashTableNull;
	EGa32HashInfo_t hinfo[HASH_SIZE];
	char*const base=(char*)hinfo;
	const size_t stsz=sizeof(EGa32HashInfo_t);
	unsigned success = 0;
	...
	EGlib_version();
	EGrandInit(&rstate);
	EGa32HashTableInit(&htable,HASH_MAXSIZE);
	...
	for( i = HASH_SIZE ; i-- ; )
	{
		EGa32HashTableAdd(base,stsz,&htable,i);
	}
	EGtimerStop(&timer);
	...
	for(factor = 2 ; factor<HASH_MAXFACTOR ; factor=(factor*3)/2) 
	{
		...
		while(j--)
		{
			...
			{
				key = EGrand(&rstate);
				info = EGa32HashTableFind(base,stsz,&htable,key);
				if(info<UINT32_MAX)
				{
					success++;
					info = EGa32HashTableFindNext(base,stsz,&htable,info);
				}
			}
		}
		EGtimerStop(&timer);
		...
		EGa32HashTableResize(base,stsz,&htable,HASH_MAXSIZE*factor);
	}
	CLEANUP:
	EGa32HashTableClear(&htable);
	return rval;
}
