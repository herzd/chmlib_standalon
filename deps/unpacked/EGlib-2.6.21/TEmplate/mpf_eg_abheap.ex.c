					...
					char **argv)
{
	...
	mpf_t v;
	mpf_EGaBheap_t my_heap;
	mpf_EGaBheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	mpf_EGaBheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (mpf_file_name, "r");
	TEST (!file, "unable to open file %s", mpf_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(mpf_EGaBheapCn_t, nval);
	mpf_EGaBheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpf_EGaBheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		mpf_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, mpf_EGlpNumToLf (all_cn[i].val));
		mpf_EGaBheapAdd (all_cn,sizeof(mpf_EGaBheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpf_EGaBheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, mpf_EGlpNumToLf (all_cn[ccn].val));
		mpf_EGaBheapDel (all_cn,sizeof(mpf_EGaBheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) mpf_EGaBheapAdd (all_cn,sizeof(mpf_EGaBheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, mpf_EGlpNumToLf (all_cn[c].val), mpf_EGlpNumToLf (v));
	mpf_EGaBheapChangeVal (all_cn,sizeof(mpf_EGaBheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, mpf_EGlpNumToLf (all_cn[ccn].val));
		mpf_EGaBheapDel (all_cn,sizeof(mpf_EGaBheapCn_t),&my_heap, ccn);
	}
