...
/** @{ */
/** @example mpq_eg_abkheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaBKheap */
/* ========================================================================= */
#ifndef mpq___EG_ABKHEAP__
#define mpq___EG_ABKHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef mpq_EG_ABKHEAP_DEBUG
#define mpq_EG_ABKHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if mpq_EG_ABKHEAP_DEBUG <= DEBUG
#define mpq_EGaBKheapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define mpq_EGaBKheapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define mpq_EGaBKheapCHECK_CN(__hp,__hcn) 
#define mpq_EGaBKheapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
/** @brief number of maximum entries in the vector values */
#ifndef mpq_EG_ABKHEAP_ENTRY
#define mpq_EG_ABKHEAP_ENTRY 3
#endif
/* ========================================================================= */
...
typedef struct 
{
	mpq_t val[mpq_EG_ABKHEAP_ENTRY];		/**< Value of this node in the heap */
	uint32_t pos;					/**< Position in the heap-array for this node, if set to
												 #mpq_EG_ABKHEAP_POISON, then the connector is not in any 
												 heap.*/
}
mpq_EGaBKheapCn_t;
/* ========================================================================= */
/** @brief Poison position for heap connector not in a heap. */
#define mpq_EG_ABKHEAP_POISON UINT32_MAX
/* ========================================================================= */
...
#define mpq_EGaBKheapIsLess(__hcn1,__hcn2) ({\
	const mpq_t*const __EGABKH1 = (__hcn1);\
	const mpq_t*const __EGABKH2 = (__hcn2);\
	int __EGABKHj = 0, __EGABKHrval = 0;\
	for( ; __EGABKHj < mpq_EG_ABKHEAP_ENTRY ; __EGABKHj++)\
	{\
		if(mpq_EGlpNumIsLess(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj])){\
			__EGABKHrval = 1; break;}\
		else if (mpq_EGlpNumIsNeq(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj],mpq_epsLpNum)){\
 ...
 * */
#define mpq_EGaBKheapCnInit(__hcn) do{\
	mpq_EGaBKheapCn_t*const __ABKHcn=(__hcn);\
	int __ABKHi = mpq_EG_ABKHEAP_ENTRY;\
	memset(__ABKHcn,0,sizeof(mpq_EGaBKheapCn_t));\
	for( ; __ABKHi-- ; ){mpq_EGlpNumInitVar(__ABKHcn->val[__ABKHi]);}\
	__ABKHcn->pos = mpq_EG_ABKHEAP_POISON;}while(0)
/* ========================================================================= */
 ...
 * */
#define mpq_EGaBKheapCnReset(__hcn) ((__hcn)->pos = mpq_EG_ABKHEAP_POISON)
/* ========================================================================= */
 ...
 * */
#define mpq_EGaBKheapCnClear(__hcn) do{\
	mpq_EGaBKheapCn_t*const __ABKHcn = (__hcn);\
	int __ABKHi = mpq_EG_ABKHEAP_ENTRY;\
	for( ; __ABKHi-- ; ){mpq_EGlpNumClearVar(__ABKHcn->val[__ABKHi]);}}while(0)
/* ========================================================================= */
...
#define mpq_EGaBKheapIsFull(__hp) ({mpq_EGaBKheap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define mpq_EGaBKheapEmpty(__hp) ((__hp)->sz = 0)

/* ========================================================================= */
/** @brief Initialize a heap as an empty heap (with no space for conectors).
 * @param __hp heap to initialize.
 * */
#define mpq_EGaBKheapInit(__hp) (*(__hp) = (mpq_EGaBKheap_t){0,0,0})
 ...
 * */
#define mpq_EGaBKheapReset(__hp) mpq_EGaBKheapResize(__hp,0)
 ...
 * */
#define mpq_EGaBKheapClear(__hp) mpq_EGaBKheapResize(__hp,0)

/* ========================================================================= */
/** @brief get the minimum conector in the heap, if the heap is empty, return
 * mpq_EG_ABKHEAP_POISON.
 * @param __hp eap where we are working.
 * @return position (in the base array) of the minimum element in the heap.
 * */
#define mpq_EGaBKheapGetMin(__hp) ({\
	mpq_EGaBKheap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : mpq_EG_ABKHEAP_POISON;})
 ...
 * */
#define mpq_EGaBKheapResize(__hp,__new_sz) ({\
	mpq_EGaBKheap_t*const __EGehp = (__hp);\
 ...
 * */
#define mpq_EGaBKheapFatherId(__id) ((uint32_t)((__id)?(((uint32_t)((__id)-1))>>1U):0))
 ...
 * */
#define mpq___EGaBKheapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const uint32_t __EGstsz3=((uint32_t)(__psz3));\
	mpq_EGaBKheap_t*const __EGehp3 = (__hp3);\
	const uint32_t __EGapos3 = __EGehp3->cn[__hcn3];\
	mpq_EGaBKheapCn_t*const __EGecn3 = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	uint32_t __EGcpos3 = __EGecn3->pos;\
	uint32_t __EGfpos3 = mpq_EGaBKheapFatherId(__EGcpos3);\
	mpq_EGaBKheapCn_t*__EGfcn3 = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	mpq_EGaBKheapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				mpq_EGaBKheapIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((uint32_t)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = mpq_EGaBKheapFatherId(__EGcpos3);\
		__EGfcn3 = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define mpq_EGaBKheapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBKheapCn_t*const __EGlcn = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpq_EGaBKheapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	mpq___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
/** @brief Give the first child for a given position.
 * @param __id position that we want to get the first child.
 * */
#define mpq_EGaBKheapFirstChildId(__id) ((uint32_t)(((uint32_t)(__id))<<1U)+1U)
/* ========================================================================= */
 ...
 * */
#define mpq___EGaBKheapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const uint32_t __EGstsz=((uint32_t)(__psz));\
	mpq_EGaBKheap_t*const __EGehp = (__hp);\
	const uint32_t __EGapos = __EGehp->cn[__hcn];\
	mpq_EGaBKheapCn_t*const __EGecn = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const uint32_t __EGhsz = __EGehp->sz;\
	uint32_t __EGcpos = __EGecn->pos;\
	uint32_t __EGfchd = mpq_EGaBKheapFirstChildId(__EGcpos);\
	uint32_t __EGlchd = ((uint32_t)(__EGfchd + 2U));\
	mpq_EGaBKheapCn_t*__EGcchd=0,*__EGnchd=0;\
	mpq_EGaBKheapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		if(__EGlchd<=__EGhsz){\
			__EGnchd = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd+1])));\
			if(mpq_EGaBKheapIsLess(__EGnchd->val,__EGcchd->val)) __EGcchd=__EGnchd;}\
		/* if the minimum child is less than the current position, move the minimum\
		 * child to the position of the current element */\
		if(mpq_EGaBKheapIsLess(__EGcchd->val,__EGecn->val))\
		{\
			...
			__EGfchd = mpq_EGaBKheapFirstChildId(__EGcpos);\
			__EGlchd = ((uint32_t)(__EGfchd + 2U));\
		}\
		/* else we exit the main loop */\
		else __EGfchd = mpq_EG_ABKHEAP_POISON;\
	}\
}while(0)
/* ========================================================================= */
 ...
 * */
#define mpq_EGaBKheapCopyVal(__dst,__src) do{\
	mpq_t*const __EGABKHdst=(__dst);\
	const mpq_t*const __EGABKHsrc=(__src);\
	int __EGABKHi=mpq_EG_ABKHEAP_ENTRY;\
	for(;__EGABKHi--;){mpq_EGlpNumCopy(__EGABKHdst[__EGABKHi],__EGABKHsrc[__EGABKHi]);}}while(0)
/* ========================================================================= */
 ...
 * */
#define mpq_EGaBKheapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBKheap_t*const __EGlhp=(__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBKheapCn_t*__EGlcn2=((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	if(mpq_EGaBKheapIsLess(__new_val,__EGlcn2->val)){\
		mpq_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		mpq___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		mpq_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		mpq___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define mpq_EGaBKheapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBKheapCn_t*const __EGlhpcn = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const uint32_t __EGlcn = __EGlhpcn->pos;\
	const uint32_t __EGlhsz = __EGlhp->sz - 1;\
	mpq_EGaBKheapCn_t*const __EGllcn = ((mpq_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = mpq_EG_ABKHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		mpq___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = mpq_EG_ABKHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of mpq_eg_abkheap.h */
#endif
#endif
#endif
