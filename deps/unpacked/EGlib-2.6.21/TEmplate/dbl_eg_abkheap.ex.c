...
void dbl_aheap_usage (char *program)
{
...
int dbl_aheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 double * v,
					...
					char **argv)
{
	...
	double v[dbl_EG_ABKHEAP_ENTRY];
	dbl_EGaBKheap_t my_heap;
	dbl_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<dbl_EG_ABKHEAP_ENTRY;i++)
	{
	...
	EGcallD(dbl_aheap_parseargs(argc, argv, &d, &c, v, &dbl_file_name));
	dbl_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (dbl_file_name, "r");
	TEST (!file, "unable to open file %s", dbl_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(dbl_EGaBKheapCn_t, nval);
	dbl_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		dbl_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		dbl_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, dbl_EGlpNumToLf (all_cn[i].val[0]));
		dbl_EGaBKheapAdd (all_cn,sizeof(dbl_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = dbl_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, dbl_EGlpNumToLf (all_cn[ccn].val[0]));
		dbl_EGaBKheapDel (all_cn,sizeof(dbl_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) dbl_EGaBKheapAdd (all_cn,sizeof(dbl_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, dbl_EGlpNumToLf (all_cn[c].val[0]), dbl_EGlpNumToLf (v[0]));
	dbl_EGaBKheapChangeVal (all_cn,sizeof(dbl_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, dbl_EGlpNumToLf (all_cn[ccn].val[0]));
		dbl_EGaBKheapDel (all_cn,sizeof(dbl_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	dbl_EGaBKheapClear(&my_heap);
	for(i=0;i<dbl_EG_ABKHEAP_ENTRY;i++)
	{
		dbl_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
