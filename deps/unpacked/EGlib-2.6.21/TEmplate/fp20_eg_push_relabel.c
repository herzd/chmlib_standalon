...
/** @{ */
static unsigned long long fp20___PR_profile_push = 0;
static unsigned long long fp20___PR_profile_numb = 0;
static unsigned long long fp20___PR_profile_lvel = 0;
static unsigned long long fp20___PR_profile_move = 0;
static unsigned long long fp20___PR_profile_rela = 0;
void fp20_EGalgPRprofile(void)
{
	if(fp20___PR_PROFILE__ <= DEBUG)
	{
		fprintf(stderr,"PROFILE FOR EGalgPushRelabel:\nnpush %llu\n"
				"nrela %llu\nnlevl %llu\nnmove %llu\nnnumb %llu\n",
				fp20___PR_profile_push, fp20___PR_profile_rela, fp20___PR_profile_lvel,
				fp20___PR_profile_move,fp20___PR_profile_numb); fp20___PR_profile_push = 
			fp20___PR_profile_numb = fp20___PR_profile_lvel = fp20___PR_profile_move = 
			fp20___PR_profile_rela = 0;
	}
}
#if fp20___PR_PROFILE__ <= DEBUG
...
static unsigned long long fp20___last_global = 0;

/* ========================================================================= */
/** @brief Compute the exact label distance in the graph to the given node.
 * @param t pointer to the fp20_EGalgPRnode_t node to wich we are computing the
 * exact label distances.
 * @param G pointer to the fp20_EGalgPRgraph_t graph where we are working.
 ...
 * #fp20_EGalgPRnode_t::LVL_list is set to {0,0}. We also assume that the field
 * #fp20_EGalgPRnode_t::d is set to a suficiently large value that denotes that they
 * are disconnected from @a t, this value should be at least the number of nodes
 * in the graph that we are working on. */
static inline int fp20_EGalgPRcomputeLabels (fp20_EGalgPRgraph_t * const G,
																				...
																				EGeList_t * DIST)
{
	...
	unsigned int d;
	fp20_EGalgPRnode_t *node_pt,
	 *parent_pt;
	fp20_EGalgPRse_t *edge;
	MESSAGE (fp20___PR_VERBOSE__, "entering ");
	/* initialize the BFS list and set the distance to t as zero */
	numb[0]++;
	EGeListAddAfter (&(t->T_cut), DIST);
	t->d = 0;
	MESSAGE (fp20___PR_VERBOSE__, "label %u", 0);
	...
	{
		edge = EGcontainerOf (node_it, fp20_EGalgPRse_t, e.head_cn);
		/* discard edges without residual capacities */
		if (fp20_EGlpNumIsLeq (edge->r, fp20_epsLpNum))
			continue;
		node_pt = EGcontainerOf (edge->e.tail, fp20_EGalgPRnode_t, v);
		...
		MESSAGE (fp20___PR_VERBOSE__, "label %u", 1);
	}

	/* now we traverse the graph in BFS mode */
	while (!EGeListIsEmpty (BFS_head))
	{
		parent_pt = EGcontainerOf (BFS_head->prev, fp20_EGalgPRnode_t, LVL_list);
		...
		for (; node_it != node_head; node_it = node_it->next)
		{
			...
			if (d < n_nodes)
			{
				...
				MESSAGE (fp20___PR_VERBOSE__, "label %u", d);
			}
		}
	}
	...
	for (; node_it != node_head; node_it = node_it->next)
	{
		node_pt = EGcontainerOf (node_it, fp20_EGalgPRnode_t, v.node_cn);
	...
	/* ending */
	MESSAGE (fp20___PR_VERBOSE__, "done ");
	return rval;
}

/* ========================================================================= */
/** @brief Perform a push operation in an edge.
 * @param edge_pt pointer to fp20_EGalgPRse_t edge where we will perform the push.
...
static inline int fp20_EGalgPRpush (fp20_EGalgPRse_t * const edge_pt,
															 EGfp20_t flow,
															 ...
															 const unsigned int n_nodes)
{
	/* local variables */
	int rval = 0;
	fp20_EGalgPRse_t *const inv_edge = edge_pt->type ?
		&(EGcontainerOf (edge_pt, fp20_EGalgPRedge_t, bw)->fw) :
		&(EGcontainerOf (edge_pt, fp20_EGalgPRedge_t, fw)->bw);
	fp20_EGalgPRnode_t *const head = EGcontainerOf (edge_pt->e.head, fp20_EGalgPRnode_t, v);
	fp20_EGalgPRnode_t *const tail = EGcontainerOf (edge_pt->e.tail, fp20_EGalgPRnode_t, v);
	MESSAGE (fp20___PR_VERBOSE__, "pushing %lg r %lg u %lg", fp20_EGlpNumToLf (flow),
					 fp20_EGlpNumToLf (edge_pt->r), fp20_EGlpNumToLf (edge_pt->u));
	fp20_UPDATE (fp20___PR_profile_push);

	/* test feasibility of the change */
	EXITL (fp20___PR_DEBUGL__, fp20_EGlpNumIsLess (edge_pt->r, flow),
				 "Pushing more flow (%lg) "
				 "than the residual capacity (%lg) on the edge", fp20_EGlpNumToLf (flow),
				 fp20_EGlpNumToLf (edge_pt->r));

	/* make the changes where necesary */
	fp20_EGlpNumSubTo (edge_pt->r, flow);
	fp20_EGlpNumAddTo (inv_edge->r, flow);
	fp20_EGlpNumSubTo (tail->e, flow);
	fp20_EGlpNumAddTo (head->e, flow);
	...
	if (!(head->LVL_list.next) && head->d < n_nodes)
	{
		...
		MESSAGE (fp20___PR_VERBOSE__, "activating label %u", head->d);
	}
...
/* ========================================================================= */
/** @brief Initialize flow to zero, and saturate forward edges in the fp20_source.
 * @param s pointer to the fp20_EGalgPRnode_t node fp20_source for the flow.
 * @param t pointer to the fp20_EGalgPRnode_t node sink for the flow
 * @param G pointer to the fp20_EGalgPRgraph_t graph where we are working.
 * @param numb array of integers where numb[k] contain the number of nodes
 * with distance labels equal to k. This array should be of size at least
 * n_nodes.
...
static inline int fp20_EGalgPRpreprocess (fp20_EGalgPRgraph_t * const G,
																		 ...
																		 EGeList_t * const DIST)
{
	...
	for (; node_it != node_head; node_it = node_it->next)
	{
		...
		{
		0, 0};
		fp20_EGlpNumZero (node_pt->e);
		edge_head = &(node_pt->v.in_edge);
		edge_it = edge_head->next;
		for (; edge_it != edge_head; edge_it = edge_it->next)
		{
			edge_pt = EGcontainerOf (edge_it, fp20_EGalgPRse_t, e.head_cn);
			fp20_EGlpNumCopy (edge_pt->r, edge_pt->u);
		}
	}

	/* now we compute the exact label distances */
	rval = fp20_EGalgPRcomputeLabels (G, t, numb, DIST);
	CHECKRVAL (rval);
	...
	for (; edge_it != edge_head; edge_it = edge_it->next)
	{
		edge_pt = EGcontainerOf (edge_it, fp20_EGalgPRse_t, e.tail_cn);
		rval = fp20_EGalgPRpush (edge_pt, edge_pt->u, LEVEL, n_nodes);
		CHECKRVAL (rval);
	}
	/* set the distance from s to t to the number of nodes in the graph */
	MESSAGE (fp20___PR_VERBOSE__, "reset label from %u to %u", s->d, n_nodes);
	if (s->d < n_nodes)
	{
		numb[s->d]--;
		EGeListDel (&(s->T_cut));
 ...
 * @param node_pt pointer to the fp20_EGalgPRnode_t node structure that we will
 * relabel.
 * @param numb array of integers where numb[k] contain the number of nodes
 * with distance labels equal to k. This array should be of size at least
 * n_nodes.
...
static int fp20_EGalgPRpushRelabel (fp20_EGalgPRnode_t * const node_pt,
															 unsigned int *const numb,
															 const unsigned int n_nodes,
															 EGeList_t * const LEVEL,
															 EGeList_t * const DIST)
{
	...
	fp20_EGalgPRnode_t *head_pt;
	fp20_EGalgPRse_t *edge_pt;
	fp20_EGlpNumInitVar (flow);
	MESSAGE (fp20___PR_VERBOSE__, "entering ");
	EXITL (fp20___PR_DEBUGL__, !node_pt->LVL_list.next, "This is inconcivable!");
	/* loop through all residual outgoing edges */
	for (; edge_it != edge_head; edge_it = edge_it->next)
	{
		edge_pt = EGcontainerOf (edge_it, fp20_EGalgPRse_t, e.tail_cn);
		head_pt = EGcontainerOf (edge_pt->e.head, fp20_EGalgPRnode_t, v);
		/* if the edge is not in the residual network, discard it */
		if (fp20_EGlpNumIsLeq (edge_pt->r, fp20_epsLpNum))
		{
			continue;
		}
		...
		if (min_d > head_pt->d)
		{
			MESSAGE (fp20___PR_VERBOSE__, "set min_d from %u to %u", min_d, head_pt->d);
			min_d = head_pt->d;
		}
		...
		inf_flow = 0;
		fp20_EGlpNumCopy (flow, *excess);
		if (fp20_EGlpNumIsLess (edge_pt->r, flow))
			fp20_EGlpNumCopy (flow, edge_pt->r);
		fp20_EGalgPRpush (edge_pt, flow, LEVEL, n_nodes);
		/* if after the push the excess is exausted, we exit the loop */
		if (fp20_EGlpNumIsLeq (*excess, fp20_epsLpNum))
			break;
	}
	...
	if (!inf_flow)
	{
		...
		if (fp20_EGlpNumIsLeq (*excess, fp20_epsLpNum))
		{
			MESSAGE (fp20___PR_VERBOSE__, "saturated push, de-activating level %u",
							 node_pt->d);
		...
		{
			MESSAGE (fp20___PR_VERBOSE__, "non-saturated push cur %u min_d %d",
							 node_pt->d, min_d);
		}
	}
	else
	{
		...
		EXITL (fp20___PR_DEBUGL__, min_d <= node_pt->d, "relabeling to lower or equal "
					 "value %u %u", min_d, node_pt->d);
		numb[node_pt->d]--;
		EGeListDel (&(node_pt->T_cut));
		fp20_UPDATE (fp20___PR_profile_rela);
		fp20___last_global++;
		if (min_d < n_nodes)
		{
			MESSAGE (fp20___PR_VERBOSE__, "relabeling to %u from %u", min_d, node_pt->d);
			numb[min_d]++;
		...
		{
			MESSAGE (fp20___PR_VERBOSE__,
							 "relabeling to %u from %u and moving it to the S" " cut",
							 n_nodes, node_pt->d);
			node_pt->d = min_d;
			fp20_UPDATE (fp20___PR_profile_move);
			EGeListDel (&(node_pt->LVL_list));
			node_pt->LVL_list = (EGeList_t)
			{
			0, 0};
 ...
 * @return zero on success, non-zero otherwise.
 * */
static inline int fp20_EGalgPRnumb (const unsigned int hole,
															 ...
															 EGeList_t * const DIST)
{
	...
	for (d = hole + 1; d < max_numb; d++)
	{
		for (i = numb[d]; i--;)
		{
			MESSAGE (fp20___PR_VERBOSE__, "eliminating node of level %u", d);
			fp20_UPDATE (fp20___PR_profile_numb);
			node_pt = EGcontainerOf (DIST[d].next, fp20_EGalgPRnode_t, T_cut);
			node_pt->d = n_nodes;
			EGeListDel (&(node_pt->T_cut));
 ...
 * network.
 * */
static inline int fp20_EGalgPRglobalRelabel (fp20_EGalgPRgraph_t * const G,
																				fp20_EGalgPRnode_t * const s,
																				fp20_EGalgPRnode_t * const t,
																				...
																				EGeList_t * const DIST)
{
	...
	{
		active = EGcontainerOf (n_it, fp20_EGalgPRnode_t, v.node_cn);
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		...
		if (active == s)
			continue;
		d = active->d;
		/* discard nodes cut away from the fp20_source */
		if (d >= n_nodes)
			continue;
		/* discard non-active nodes */
		if (fp20_EGlpNumIsLeq (active->e, fp20_epsLpNum))
			continue;
		/* add the active node to their corresponding active list */
		MESSAGE (fp20___PR_VERBOSE__, "Activating node %p with level %u excess %lg",
						 (void *) active, d, fp20_EGlpNumToLf (active->e));
										 ...
										 fp20_EGalgPRnode_t * const t)
{
	...
	fp20_EGalgPRnode_t *active = 0;
#if fp20_EG_PR_RELABEL
	unsigned const int limit = fp20_EG_PR_RELABEL_FREC * n_nodes;
#endif
	MESSAGE (fp20___PR_VERBOSE__, "entering ");
	...
	fp20_EGalgPRpreprocess (G, s, t, numb, LEVEL, DIST);
	...
	if (i < n_nodes)
	{
		rval = fp20_EGalgPRnumb (i, max_numb, n_nodes, numb, DIST);
	...
	{
		level--;
		fp20_UPDATE (fp20___PR_profile_lvel);
	}
	/* now we do the loop among active nodes */
	while (level > 0)
	{
		MESSAGE (fp20___PR_VERBOSE__, "level %u", level);
		active = EGcontainerOf (LEVEL[level].next, fp20_EGalgPRnode_t, LVL_list);
		old_d = active->d;
		fp20_EGalgPRpushRelabel (active, numb, n_nodes, LEVEL, DIST);
		...
		if (active->d < n_nodes && old_d < active->d)
		{
			...
			if (!numb[old_d])
			{
				rval = fp20_EGalgPRnumb (old_d, max_numb, n_nodes, numb, DIST);
				CHECKRVALG (rval, CLEANUP);
				max_numb = old_d;
			}
		}
		...
		{
#if fp20_EG_PR_RELABEL
			if (fp20___last_global > limit)
			{
				fp20___last_global = 0;
				rval = fp20_EGalgPRglobalRelabel (G, s, t, n_nodes, numb, LEVEL, DIST);
				CHECKRVALG (rval, CLEANUP);
				level = n_nodes - 1;
				max_numb = n_nodes;
			}
											...
											fp20_EGalgPRnode_t * const t)
{
	...
	rval = fp20_EGalgPRglobalRelabel (G, t, s, n_nodes, numb, LEVEL, DIST);
	CHECKRVALG (rval, CLEANUP);
													 ...
													 EGfp20_t * error)
{
	...
	for (i = n_nodes; i--;)
		EGeListInit (DIST + i);
	fp20_EGlpNumZero (*error);
	fp20_EGlpNumInitVar (flow);
	fp20_EGlpNumInitVar (cut);
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		node_pt = EGcontainerOf (n_it, fp20_EGalgPRnode_t, v.node_cn);
		node_pt->LVL_list = (EGeList_t)
		{
		0, 0};
		node_pt->d = n_nodes;
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		...
		{
			fp20_EGlpNumSetToMaxAbs (*error, node_pt->e);
			/* check that the excess condition holds within epsilon */
			if (fp20_EGlpNumIsNeqZero (node_pt->e, fp20_epsLpNum))
			{
				MESSAGE (fp20___PR_TEST_VERBOSE__, "excess %lg for node %p is above error",
								 fp20_EGlpNumToLf (node_pt->e), (void *) node_pt);
				rval++;
			}
		}
		/* check flow condition and compute cut value */
		fp20_EGlpNumZero (flow);
		e_head = &(node_pt->v.out_edge);
		e_it = e_head->next;
		for (; e_it != e_head; e_it = e_it->next)
		{
			edge_pt = EGcontainerOf (e_it, fp20_EGalgPRse_t, e.tail_cn);
			inv_node = EGcontainerOf (edge_pt->e.head, fp20_EGalgPRnode_t, v);
			if (fp20_EGlpNumIsLess (edge_pt->r, edge_pt->u))
			{
				fp20_EGlpNumAddTo (flow, edge_pt->u);
				fp20_EGlpNumSubTo (flow, edge_pt->r);
			}
			if ((node_pt->d >= n_nodes) && (inv_node->d < n_nodes))
			{
				fp20_EGlpNumAddTo (cut, edge_pt->u);
			}
		}
		...
		for (; e_it != e_head; e_it = e_it->next)
		{
			edge_pt = EGcontainerOf (e_it, fp20_EGalgPRse_t, e.head_cn);
			inv_node = EGcontainerOf (edge_pt->e.tail, fp20_EGalgPRnode_t, v);
			if (fp20_EGlpNumIsLess (edge_pt->r, edge_pt->u))
			{
				fp20_EGlpNumSubTo (flow, edge_pt->u);
				fp20_EGlpNumAddTo (flow, edge_pt->r);
			}
		}
		if (node_pt != s && node_pt != t)
		{
			fp20_EGlpNumSetToMaxAbs (*error, flow);
			if (fp20_EGlpNumIsNeqZero (flow, fp20_epsLpNum))
			{
				rval++;
				MESSAGE (fp20___PR_TEST_VERBOSE__,
	...
	fp20_EGlpNumCopy (flow, t->e);
	fp20_EGlpNumAddTo (flow, s->e);
	fp20_EGlpNumSetToMaxAbs (*error, flow);
	if (fp20_EGlpNumIsNeqZero (flow, fp20_epsLpNum))
	{
		rval++;
		MESSAGE (fp20___PR_TEST_VERBOSE__, "flow balance between s and t is violated"
						 " by %lg", fp20_EGlpNumToLf (flow));
	}
	fp20_EGlpNumCopy (flow, s->e);
	fp20_EGlpNumAddTo (flow, cut);
	fp20_EGlpNumSetToMaxAbs (*error, flow);
	if (fp20_EGlpNumIsNeqZero (flow, fp20_epsLpNum))
	{
		rval++;
		MESSAGE (fp20___PR_TEST_VERBOSE__, "s-flow and cut don't agree by %lg",
						 fp20_EGlpNumToLf (flow));
	}
