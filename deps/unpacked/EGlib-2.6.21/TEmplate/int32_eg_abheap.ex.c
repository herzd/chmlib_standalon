					...
					char **argv)
{
	...
	int32_t v;
	int32_EGaBheap_t my_heap;
	int32_EGaBheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = int32_EGaBheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, int32_EGlpNumToLf (all_cn[ccn].val));
		int32_EGaBheapDel (all_cn,sizeof(int32_EGaBheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) int32_EGaBheapAdd (all_cn,sizeof(int32_EGaBheapCn_t),&my_heap, i);
	...
	int32_EGaBheapChangeVal (all_cn,sizeof(int32_EGaBheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
