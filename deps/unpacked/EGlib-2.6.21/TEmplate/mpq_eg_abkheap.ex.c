					...
					char **argv)
{
	...
	mpq_t v[mpq_EG_ABKHEAP_ENTRY];
	mpq_EGaBKheap_t my_heap;
	mpq_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<mpq_EG_ABKHEAP_ENTRY;i++)
	{
		mpq_EGlpNumInitVar (v[i]);
		mpq_EGlpNumZero (v[i]);
	}

	/* Parse command line input to get 'file name' and 'd'. */
	EGcallD(mpq_aheap_parseargs(argc, argv, &d, &c, v, &mpq_file_name));
	mpq_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (mpq_file_name, "r");
	TEST (!file, "unable to open file %s", mpq_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(mpq_EGaBKheapCn_t, nval);
	mpq_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpq_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		mpq_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, mpq_EGlpNumToLf (all_cn[i].val[0]));
		mpq_EGaBKheapAdd (all_cn,sizeof(mpq_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpq_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, mpq_EGlpNumToLf (all_cn[ccn].val[0]));
		mpq_EGaBKheapDel (all_cn,sizeof(mpq_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) mpq_EGaBKheapAdd (all_cn,sizeof(mpq_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, mpq_EGlpNumToLf (all_cn[c].val[0]), mpq_EGlpNumToLf (v[0]));
	mpq_EGaBKheapChangeVal (all_cn,sizeof(mpq_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, mpq_EGlpNumToLf (all_cn[ccn].val[0]));
		mpq_EGaBKheapDel (all_cn,sizeof(mpq_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	mpq_EGaBKheapClear(&my_heap);
	for(i=0;i<mpq_EG_ABKHEAP_ENTRY;i++)
	{
		mpq_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
