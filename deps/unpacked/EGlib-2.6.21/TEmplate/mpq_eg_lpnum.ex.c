...
#include "EGlib.h"
/* ========================================================================= */
/** @brief Tester program for mpq_t structure and functions
 * @return zero on success, non-zero otherwise 
 * @par Description:
 * Perform various tests on mpq_t and their functions 
 * */
int main (int argc,
					char **argv)
{
	...
	mpq_init (qnum);
	#endif
	mpq_EGlpNumInitVar (ntmp[0]);
	mpq_EGlpNumInitVar (ntmp[1]);
	mpq_EGlpNumInitVar (ntmp[2]);
	mpq_EGlpNumInitVar (ntmp[3]);
	...
	#endif
	mpq_EGlpNumReadStr (ntmp[0], argv[1]);
	mpq_EGlpNumReadStr (ntmp[1], argv[2]);
	dtmp[0] = mpq_EGlpNumToLf (ntmp[0]);
	dtmp[1] = mpq_EGlpNumToLf (ntmp[1]);

	/* convert numbers */
	strnum1 = mpq_EGlpNumGetStr (ntmp[0]);
	strnum2 = mpq_EGlpNumGetStr (ntmp[1]);
	...
	/* internal constants */
	strnum3 = mpq_EGlpNumGetStr (mpq_oneLpNum);
	fprintf (stderr, "1.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = mpq_EGlpNumGetStr (mpq_zeroLpNum);
	fprintf (stderr, "0.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = mpq_EGlpNumGetStr (mpq_epsLpNum);
	fprintf (stderr, "eps = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = mpq_EGlpNumGetStr (mpq_MaxLpNum);
	fprintf (stderr, "Max = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = mpq_EGlpNumGetStr (mpq_MinLpNum);
	fprintf (stderr, "Min = %s\n", strnum3);
	EGfree (strnum3);

	/* copying functions */
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s (%lg)\n", strnum3, strnum1, dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumCopyDiff (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s - %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	mpq_EGlpNumCopyDiffRatio (ntmp[2], ntmp[0], ntmp[1], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = (%s  - %s)/%s (%lg)\n", strnum3, strnum1, strnum2,
					 strnum1, (dtmp[0] - dtmp[1]) / dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumCopySum (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s + %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	mpq_EGlpNumCopySqrOver (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s^2/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] * dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumCopyAbs (ntmp[2], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = |%s| (%lg)\n", strnum3, strnum1, fabs (dtmp[0]));
	EGfree (strnum3);
	mpq_EGlpNumCopyNeg (ntmp[2], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = -1*%s (%lg)\n", strnum3, strnum1, -dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumCopyFrac (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	...
	/* add */
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumAddTo (ntmp[2], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s + %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumAddUiTo (ntmp[2], 0xffU);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	...
	/* substract */
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumSubTo (ntmp[2], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumSubUiTo (ntmp[2], 0xffU);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] - 0xffU);
	EGfree (strnum3);
	...
	/* multiply */
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumMultTo (ntmp[2], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s * %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] * dtmp[1]);
	EGfree (strnum3);
	...
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumSign (ntmp[2]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "-(%s) = %s (%lg)\n", strnum1, strnum3, -dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumOne (ntmp[2]);
	mpq_EGlpNumAddInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 + %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 + dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	mpq_EGlpNumOne (ntmp[2]);
	mpq_EGlpNumSubInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 - %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 - dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	...
	/* divide */
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumDivTo (ntmp[2], ntmp[1]);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] / dtmp[1]);
	EGfree (strnum3);
	mpq_EGlpNumCopy (ntmp[2], ntmp[0]);
	mpq_EGlpNumDivUiTo (ntmp[2], 0xffU);
	strnum3 = mpq_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] / 0xffU);
	EGfree (strnum3);
	...
	/* test transformation to rationals */
	{
		...
		mpf_init (f1);
		mpf_init (f2);
		mpf_EGlpNumSet(f1,mpq_EGlpNumToLf(ntmp[0]));
		mpf_EGlpNumSet(f2,mpq_EGlpNumToLf(ntmp[1]));
		...
		fprintf (stderr, "Your input in rational was:\n\t(%10.7lf) %s\n\t(%10.7lf)"
				" %s\n", mpq_EGlpNumToLf (ntmp[0]), strnum1, mpq_EGlpNumToLf (ntmp[1]),
				strnum2);
		EGfree (strnum1);
		EGfree (strnum2);
	...
	/* ending */
	CLEANUP:
	mpq_EGlpNumClearVar (ntmp[0]);
	mpq_EGlpNumClearVar (ntmp[1]);
	mpq_EGlpNumClearVar (ntmp[2]);
	mpq_EGlpNumClearVar (ntmp[3]);
	#ifdef HAVE_LIBGMP
	mpq_clear (qnum);
	#endif
	EGlpNumClear();
