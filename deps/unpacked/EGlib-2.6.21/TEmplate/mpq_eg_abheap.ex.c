					...
					char **argv)
{
	...
	mpq_t v;
	mpq_EGaBheap_t my_heap;
	mpq_EGaBheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpq_EGaBheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, mpq_EGlpNumToLf (all_cn[ccn].val));
		mpq_EGaBheapDel (all_cn,sizeof(mpq_EGaBheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) mpq_EGaBheapAdd (all_cn,sizeof(mpq_EGaBheapCn_t),&my_heap, i);
	...
	mpq_EGaBheapChangeVal (all_cn,sizeof(mpq_EGaBheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
