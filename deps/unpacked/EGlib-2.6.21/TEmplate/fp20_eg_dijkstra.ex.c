...
/* ========================================================================= */
#include "EGlib.h"
static char fp20_fname[1024];
static int32_t fp20_source=0;
static int fp20_verbose=1;
/* ========================================================================= */
static inline void fp20_djk_usage(const char*const progname)
{
...
/* ========================================================================= */
/** @brief parse-args */
static inline void fp20_djk_parseargs(int argc,char**argv)
{
	int c, has_file=0;
	while((c=getopt(argc,argv,"f:v:hs:"))!=EOF)
	{
		...
		{
			case 'f':
				snprintf(fp20_fname,1023,"%s",optarg);
				has_file=1;
				break;
			case 'v':
				fp20_verbose=atoi(optarg);
				break;
			case 'h':
				fp20_djk_usage(argv[0]);
				exit(EXIT_SUCCESS);
			case 's':
				fp20_source=atoi(optarg);
				break;
			default:
...
int main (int argc,char**argv)
{
	...
	EGfp20_t*weight=0,*sweight=0,*dist=0;
	EGioFile_t* input=0;
	EGtimerReset(&dt);
	EGtimerReset(&rt);
	EGtimerReset(&pp);
	...
	fp20_djk_parseargs(argc,argv);
	input = EGioOpen(fp20_fname,"r");
	EGcallD(fp20_EGguReadXgraph(input,&n,&m,&edges,&weight));
	EGioClose(input);
	EGtimerStop(&rt);
	EGtimerStart(&pp);
	...
	dist=fp20_EGlpNumAllocArray(n);
	sweight=fp20_EGlpNumAllocArray(m);
	...
	for(i=m;i--;)
		fp20_EGlpNumCopy(sweight[in_perm[i]],weight[i]);
	EGtimerStop(&pp);
	EGtimerStart(&dt);
	EGcallD(fp20_EGalgDJK(n,m,in_d,in_beg,in_e,sweight,fp20_source,n,t,father,dist));
	EGtimerStop(&dt);
	...
	if(fp20_verbose>1)
	{
		for(i=0;i<n;i++)
		{
			fprintf(stderr,"\t\tNode %d father %"PRId32" dist %.3lf\n",i,father[i],fp20_EGlpNumToLf(dist[i]));
		}
	}
	CLEANUP:
	fp20_EGlpNumFreeArray(weight);
	fp20_EGlpNumFreeArray(sweight);
	fp20_EGlpNumFreeArray(dist);
	EGfree(edges);
	EGfree(in_d);
	EGfree(in_e);
	EGfree(in_beg);
