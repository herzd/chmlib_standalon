...
static inline void int_pr_usage (char **argv)
{
					...
					char **argv)
{
	...
	int cap;
	EGtimer_t cut_timer;
	int_EGalgPRgraph_t G;
	int_EGalgPRnode_t *nodes = 0/*,
	 *node_pt*/;
	int_EGalgPRedge_t *edges = 0;
	/*EGeList_t *n_it;*/
	char l_str[1024];
	char*l_argv[128];
	int l_argc;
	...
	fprintf (stderr, "Reading graph on %u nodes and %u edges...", n_nodes,
					 n_edges);
	nodes = EGsMalloc (int_EGalgPRnode_t, n_nodes);
	edges = EGsMalloc (int_EGalgPRedge_t, n_edges);
	for (i = n_nodes; i--;)
	{
		int_EGalgPRnodeInit (nodes + i);
		EGeDgraphAddNode (&(G.G), &(nodes[i].v));
	}
	for (i = n_edges; i--;)
	{
		int_EGalgPRedgeInit (edges + i);
		EGioGets(l_str,1024,in_file);
		EGioNParse(l_str,128," ","%#",&l_argc,l_argv);
		...
		int_EGlpNumCopy (edges[i].fw.u, cap);
		int_EGlpNumCopy (edges[i].bw.u, cap);
	}
	EGioClose(in_file);
	in_file = 0;
	...
	rval = int_EGalgPRminSTcut (&G, nodes, nodes + (n_nodes >> 1));
	...
	EGtimerReset (&cut_timer);
	EGtimerStart (&cut_timer);
	rval = int_EGalgPRmaxSTflow (&G, nodes, nodes + (n_nodes >> 1));
	EGtimerStop (&cut_timer);
	fprintf (stderr, "...done in %lf seconds\n", cut_timer.time);
	/* now we print the value of the cut */
	fprintf (stderr, "Minimum Cut Capacity %lf\n",
					 int_EGlpNumToLf (nodes[n_nodes >> 1].e));
	/* check the solution */
	rval = int_EGalgPRoptimalityTest (&G, nodes, nodes + (n_nodes >> 1), &cap);
	TESTG (rval, CLEANUP, "Worst error %lg, standard expected error %lg,"
				 " number of errors %d", int_EGlpNumToLf (cap), int_EGlpNumToLf (int_epsLpNum),
				 rval);
...
CLEANUP:
	...
	int_EGalgPRgraphClear (&G);
	for (i = n_nodes; i--;)
		int_EGalgPRnodeClear (nodes + i);
	for (i = n_edges; i--;)
		int_EGalgPRedgeClear (edges + i);
	EGfree (nodes);
	EGfree (edges);
	int_EGalgPRprofile();
	EGlpNumClear();
