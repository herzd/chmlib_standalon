...
void dbl_eheap_usage (char *program)
{
...
int dbl_eheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 double * v,
					...
					char **argv)
{
	...
	unsigned int nval;
	double v;
	dbl_EGeHeap_t my_heap;
	dbl_EGeHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	dbl_EGlpNumZero (v);
	dbl_EGeHeapInit (&my_heap);
	rval = dbl_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = dbl_eheap_parseargs (argc, argv, &d, &c, &v, &dbl_file_name);
	CHECKRVALG (rval, CLEANUP);
	dbl_EGeHeapChangeD (&my_heap, d);
	rval = dbl_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	...
	fscanf (file, "%u", &nval);
	all_cn = EGsMalloc (dbl_EGeHeapCn_t, nval);
	dbl_EGeHeapResize (&my_heap, nval);
	rval = dbl_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		dbl_EGeHeapCnInit (all_cn + i);
		fscanf (file, "%s", str1);
		dbl_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1,
						 dbl_EGlpNumToLf (all_cn[i].val));
		rval = dbl_EGeHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = dbl_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = dbl_EGeHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %zd : %lg", i, ccn - all_cn,
						 dbl_EGlpNumToLf (ccn->val));
		dbl_EGeHeapDel (&my_heap, ccn);
		rval = dbl_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 dbl_EGlpNumToLf (all_cn[c].val), dbl_EGlpNumToLf (v));
	rval = dbl_EGeHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = dbl_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
