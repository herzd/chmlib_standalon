 ...
 * */
#include "mpq_eg_dmatrix.h"
/** @file
 * @ingroup EGdMatrix */
/** @addtogroup EGdMatrix */
/** @{ */
/* ========================================================================= */
int mpq_EGdMatrixGaussianElimination (mpq_EGdMatrix_t * const dmatrix,
																	...
																	int *const ext_status)
{
	...
	mpq_t **const rowval = dmatrix->matrow;
	mpq_t pivot,
	  tmpval;
	int rval = 0,
	  tmpint = 0,
	...
	while (1)
	{
		...
		pcol = rank;
		prow = rank;
		mpq_EGlpNumCopyAbs (pivot, rowval[row_ord[prow]][col_ord[pcol]]);
		/* in this case we are forced to choose as pivot element a_kk */
		if (!do_col_perm && !do_row_perm)
		{
			if (!mpq_EGlpNumIsNeqZero (pivot, zero_tol))
			{
		...
		else if (do_col_perm && do_row_perm)
		{
			for (i = rank; i < row_sz; i++)
				...
				{
					mpq_EGlpNumCopyAbs (tmpval, rowval[row_ord[i]][col_ord[j]]);
					if (mpq_EGlpNumIsLess (pivot, tmpval))
					{
						pcol = j;
						prow = i;
						mpq_EGlpNumCopy (pivot, tmpval);
					}
				}
			if (!mpq_EGlpNumIsNeqZero (pivot, zero_tol))
			{
		...
		else if (do_col_perm && !do_row_perm)
		{
			...
			for (j = rank; j < col_sz; j++)
			{
				mpq_EGlpNumCopyAbs (tmpval, rowval[tmpint][col_ord[j]]);
				if (mpq_EGlpNumIsLess (pivot, tmpval))
				{
					pcol = j;
					mpq_EGlpNumCopy (pivot, tmpval);
		...
		else
		{
			...
			for (i = rank; i < row_sz; i++)
			{
				mpq_EGlpNumCopyAbs (tmpval, rowval[row_ord[i]][tmpint]);
				if (mpq_EGlpNumIsLess (pivot, tmpval))
				{
					prow = i;
					mpq_EGlpNumCopy (pivot, tmpval);
		 ...
		 * and or rows */
		mpq_EGlpNumCopy (pivot, rowval[row_ord[rank]][col_ord[rank]]);
		/* now we do de pivot */
		tmpint = row_ord[rank];
		for (i = rank + 1; i < row_sz; i++)
		{
			tmpint2 = row_ord[i];
			mpq_EGlpNumCopy (tmpval, rowval[tmpint2][col_ord[rank]]);
			/* check wether we don't need to update this row */
			if (mpq_EGlpNumIsNeqZero (tmpval, zero_tol))
			{
				/* compute the row multiplier */
				mpq_EGlpNumDivTo (tmpval, pivot);
				/* set row_i = row_i - tmpval row_rank */
				mpq_EGlpNumCopy (rowval[tmpint2][col_ord[rank]], mpq_zeroLpNum);
				for (j = rank + 1; j < col_sz; j++)
					mpq_EGlpNumSubInnProdTo (rowval[tmpint2][col_ord[j]], tmpval,
