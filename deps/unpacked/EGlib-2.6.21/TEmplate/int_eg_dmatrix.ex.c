...
/** @file 
 * @ingroup EGdMatrix
 * @brief This file shows some examples of the use of #int_EGdMatrix_t structure 
...
#define int_HILBERT_TABLE_SIZE 9U

/* ========================================================================= */
/** @brief table containing 1/eigenvalues of the hilbert matrix. */
static unsigned int_dmatrix_hilbert_eigenvalues[int_HILBERT_TABLE_SIZE] = {
	1U,
	12U,
	180U,
	2800U,
...
void int_dmatrix_usage (char *program)
{
...
int int_dmatrix_parseargs (int argc,
											 char **argv,
											 size_t * n,
											 size_t * m)
{
					...
					char **argv)
{
	...
	register unsigned i = 0,
	  j = 0;
	unsigned rank;
	int_EGdMatrix_t dmatrix;
	int_EGdBsRed_t bsred;
	int error;
	EGlib_info();
	EGlib_version();
	/* start */
	EGlpNumStart();
	int_EGlpNumInitVar (error);
	int_EGdMatrixInit (&dmatrix);
	int_EGdBsRedInit (&bsred);
	/* parse args */
	rval = int_dmatrix_parseargs (argc, argv, &n, &m);
	CHECKRVALG (rval, CLEANUP);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	/* set the matrix size */
	int_EGdMatrixSetDimension (&dmatrix, n, m);
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
		{
			int_EGlpNumOne (dmatrix.matrow[i][j]);
			int_EGlpNumDivUiTo (dmatrix.matrow[i][j], i + j + 1);
		}
	/* Display the hilbert matrix */
	fprintf (stdout, "Hilber ");
	int_EGdMatrixDisplay (&dmatrix, 1, stdout);
	fflush (stdout);
	/* now we call gaussian elimination */
	rval =
		int_EGdMatrixGaussianElimination (&dmatrix, 0, 0, &rank, int_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	fprintf (stdout, "Gaussian Elimination ended with status %d, rank %u and ",
					 status, rank);
	int_EGdMatrixDisplay (&dmatrix, 0, stdout);
	fflush (stdout);
	/* now we compute the worst error in the diagonal up to
	 * EGmin(int_HILBERT_TABLE_SIZE,rank) */
	fprintf (stdout, "Relative errors in eigenvalues:\n");
	for (i = EGmin (int_HILBERT_TABLE_SIZE, rank); i--;)
	{
		int_EGlpNumCopy (error, dmatrix.matrow[dmatrix.row_ord[i]][dmatrix.col_ord[i]]);
		fprintf (stdout, "Lambda_%u = %lg", i + 1, int_EGlpNumToLf (error));
		int_EGlpNumMultUiTo (error, int_dmatrix_hilbert_eigenvalues[i]);
		int_EGlpNumSubTo (error, int_oneLpNum);
		fprintf (stdout, " error = %lg\n", int_EGlpNumToLf (error));
	}
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
	...
	/* ending */
	int_EGdBsRedSetLength (&bsred, m);
	for (i = 0; i < rank; i++)
		int_EGdBsRedAddElement (&bsred, dmatrix.matrow[dmatrix.row_ord[i]]);
	/* now call the Basis Reduction Algorithm and print the reduced basis */
	rval = int_EGdBsRed (&bsred, &rank, int_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	...
	fflush (stdout);
CLEANUP:
	int_EGdBsRedProfile (stderr);
	int_EGlpNumClearVar (error);
	int_EGdMatrixClear (&dmatrix);
	int_EGdBsRedClear (&bsred);
	MESSAGE(0,"done");
	EGlpNumClear();
	return rval;
}
