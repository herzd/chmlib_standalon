...
static inline void fp20_srk_usage (char **argv)
{
	fprintf (stderr, "Usage: %s input_file\n", argv[0]);
}

/* ========================================================================= */
/** @brief given a fp20_EGsrkGraph_t structure, display the current shrunken graph
 * and it's node weights */
static inline void fp20_display_srkG (fp20_EGsrkGraph_t * G,
																 fp20_EGsrkNode_t * const all_nodes,
																 fp20_EGsrkEdge_t * const all_edges)
{
	EGeList_t *n_it,
	 *e_it,
	 *n_end,
	...
	if (!EGeListIsEmpty (n_end))
	{
		...
		{
			lcn = EGcontainerOf (n_it, fp20_EGsrkNode_t, node.node_cn);
			fprintf (stderr, "\t%zd (%lf): ", lcn - all_nodes,
							 fp20_EGlpNumToLf (lcn->weight));
			e_end = &(lcn->members);
			if (!EGeListIsEmpty (e_end))
			{
				...
				{
					onode = EGcontainerOf (e_it, fp20_EGsrkNode_t, members);
			...
			if (!EGeListIsEmpty (e_end))
			{
				...
				for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
				{
					...
					ce = EGcontainerOf (cep, fp20_EGsrkEdge_t, edge.ep[cep->type]);
					fprintf (stderr, "%zd[%lf] ", ce - all_edges,
									 fp20_EGlpNumToLf (ce->weight));
					ee_end = &(ce->members);
					if (!EGeListIsEmpty (ee_end))
					{
						...
						{
							cee = EGcontainerOf (ee_it, fp20_EGsrkEdge_t, members);
					...
					char **argv)
{
	...
	EGfp20_t cap;
	fp20_EGsrkGraph_t G;
	fp20_EGsrkNode_t *all_nodes = 0;
	fp20_EGsrkEdge_t *all_edges = 0;
	fp20_EGsrkNode_t *N1,
	 *N2;
	char l_str[1024];
	...
	all_nodes = EGsMalloc (fp20_EGsrkNode_t, G.n_onodes);
	all_edges = EGsMalloc (fp20_EGsrkEdge_t, G.n_oedges);
	for (i = G.n_onodes; i--;)
	{
		fp20_EGsrkNodeInit (all_nodes + i);
		fp20_EGsrkAddNode (&G, all_nodes + i);
	}
	for (i = G.n_oedges; i--;)
	{
		fp20_EGsrkEdgeInit (all_edges + i);
		n_read = fscanf (in_file, "%u %u %s", &head, &tail, l_str);
		TESTG ((rval = (n_read != 3)), CLEANUP, "couldn't read three fields");
		fp20_EGlpNumReadStr (cap, l_str);
		fp20_EGlpNumCopy (all_edges[i].weight, cap);
		fp20_EGsrkAddEdge (&G, all_nodes + head, all_nodes + tail, all_edges + i);
	}
	fclose (in_file);
	in_file = 0;
	fprintf (stderr, "done\n");
	...
	while (G.G.n_nodes > 2)
	{
		//fp20_display_srkG (&G, all_nodes, all_edges);
		N1 = EGcontainerOf (G.G.nodes.next, fp20_EGsrkNode_t, node.node_cn);
		N2 = EGcontainerOf (N1->node.node_cn.next, fp20_EGsrkNode_t, node.node_cn);
		fp20_EGsrkIdentifyNodes (&G, N1, N2);
	}
	fp20_display_srkG (&G, all_nodes, all_edges);
	/* ending */
CLEANUP:
	...
	for (i = G.n_onodes; i--;)
		fp20_EGsrkNodeClear (all_nodes + i);
	for (i = G.n_oedges; i--;)
		fp20_EGsrkEdgeClear (all_edges + i);
	if (all_nodes)
		EGfree (all_nodes);
	if (all_edges)
		EGfree (all_edges);
	fp20_EGsrkGraphClear (&G);
	EGlpNumClear();
	return rval;
}
