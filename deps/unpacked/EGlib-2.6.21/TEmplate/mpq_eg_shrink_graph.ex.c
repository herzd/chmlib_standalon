...
static inline void mpq_srk_usage (char **argv)
{
	fprintf (stderr, "Usage: %s input_file\n", argv[0]);
}

/* ========================================================================= */
/** @brief given a mpq_EGsrkGraph_t structure, display the current shrunken graph
 * and it's node weights */
static inline void mpq_display_srkG (mpq_EGsrkGraph_t * G,
																 mpq_EGsrkNode_t * const all_nodes,
																 mpq_EGsrkEdge_t * const all_edges)
{
	EGeList_t *n_it,
	 *e_it,
	 *n_end,
	...
	if (!EGeListIsEmpty (n_end))
	{
		...
		{
			lcn = EGcontainerOf (n_it, mpq_EGsrkNode_t, node.node_cn);
			fprintf (stderr, "\t%zd (%lf): ", lcn - all_nodes,
							 mpq_EGlpNumToLf (lcn->weight));
			e_end = &(lcn->members);
			if (!EGeListIsEmpty (e_end))
			{
				...
				{
					onode = EGcontainerOf (e_it, mpq_EGsrkNode_t, members);
			...
			if (!EGeListIsEmpty (e_end))
			{
				...
				for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
				{
					...
					ce = EGcontainerOf (cep, mpq_EGsrkEdge_t, edge.ep[cep->type]);
					fprintf (stderr, "%zd[%lf] ", ce - all_edges,
									 mpq_EGlpNumToLf (ce->weight));
					ee_end = &(ce->members);
					if (!EGeListIsEmpty (ee_end))
					{
						...
						{
							cee = EGcontainerOf (ee_it, mpq_EGsrkEdge_t, members);
					...
					char **argv)
{
	...
	mpq_t cap;
	mpq_EGsrkGraph_t G;
	mpq_EGsrkNode_t *all_nodes = 0;
	mpq_EGsrkEdge_t *all_edges = 0;
	mpq_EGsrkNode_t *N1,
	 *N2;
	char l_str[1024];
	...
	all_nodes = EGsMalloc (mpq_EGsrkNode_t, G.n_onodes);
	all_edges = EGsMalloc (mpq_EGsrkEdge_t, G.n_oedges);
	for (i = G.n_onodes; i--;)
	{
		mpq_EGsrkNodeInit (all_nodes + i);
		mpq_EGsrkAddNode (&G, all_nodes + i);
	}
	for (i = G.n_oedges; i--;)
	{
		mpq_EGsrkEdgeInit (all_edges + i);
		n_read = fscanf (in_file, "%u %u %s", &head, &tail, l_str);
		TESTG ((rval = (n_read != 3)), CLEANUP, "couldn't read three fields");
		mpq_EGlpNumReadStr (cap, l_str);
		mpq_EGlpNumCopy (all_edges[i].weight, cap);
		mpq_EGsrkAddEdge (&G, all_nodes + head, all_nodes + tail, all_edges + i);
	}
	fclose (in_file);
	in_file = 0;
	fprintf (stderr, "done\n");
	...
	while (G.G.n_nodes > 2)
	{
		//mpq_display_srkG (&G, all_nodes, all_edges);
		N1 = EGcontainerOf (G.G.nodes.next, mpq_EGsrkNode_t, node.node_cn);
		N2 = EGcontainerOf (N1->node.node_cn.next, mpq_EGsrkNode_t, node.node_cn);
		mpq_EGsrkIdentifyNodes (&G, N1, N2);
	}
	mpq_display_srkG (&G, all_nodes, all_edges);
	/* ending */
CLEANUP:
	...
	for (i = G.n_onodes; i--;)
		mpq_EGsrkNodeClear (all_nodes + i);
	for (i = G.n_oedges; i--;)
		mpq_EGsrkEdgeClear (all_edges + i);
	if (all_nodes)
		EGfree (all_nodes);
	if (all_edges)
		EGfree (all_edges);
	mpq_EGsrkGraphClear (&G);
	EGlpNumClear();
	return rval;
}
