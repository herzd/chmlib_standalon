...
/* ========================================================================= */
#include "EGlib.h"
static char mpq_fname[1024];
static int32_t mpq_source=0;
static int mpq_verbose=1;
/* ========================================================================= */
static inline void mpq_djk_usage(const char*const progname)
{
...
/* ========================================================================= */
/** @brief parse-args */
static inline void mpq_djk_parseargs(int argc,char**argv)
{
	int c, has_file=0;
	while((c=getopt(argc,argv,"f:v:hs:"))!=EOF)
	{
		...
		{
			case 'f':
				snprintf(mpq_fname,1023,"%s",optarg);
				has_file=1;
				break;
			case 'v':
				mpq_verbose=atoi(optarg);
				break;
			case 'h':
				mpq_djk_usage(argv[0]);
				exit(EXIT_SUCCESS);
			case 's':
				mpq_source=atoi(optarg);
				break;
			default:
...
int main (int argc,char**argv)
{
	...
	mpq_t*weight=0,*sweight=0,*dist=0;
	EGioFile_t* input=0;
	EGtimerReset(&dt);
	EGtimerReset(&rt);
	EGtimerReset(&pp);
	...
	mpq_djk_parseargs(argc,argv);
	input = EGioOpen(mpq_fname,"r");
	EGcallD(mpq_EGguReadXgraph(input,&n,&m,&edges,&weight));
	EGioClose(input);
	EGtimerStop(&rt);
	EGtimerStart(&pp);
	...
	dist=mpq_EGlpNumAllocArray(n);
	sweight=mpq_EGlpNumAllocArray(m);
	...
	for(i=m;i--;)
		mpq_EGlpNumCopy(sweight[in_perm[i]],weight[i]);
	EGtimerStop(&pp);
	EGtimerStart(&dt);
	EGcallD(mpq_EGalgDJK(n,m,in_d,in_beg,in_e,sweight,mpq_source,n,t,father,dist));
	EGtimerStop(&dt);
	...
	if(mpq_verbose>1)
	{
		for(i=0;i<n;i++)
		{
			fprintf(stderr,"\t\tNode %d father %"PRId32" dist %.3lf\n",i,father[i],mpq_EGlpNumToLf(dist[i]));
		}
	}
	CLEANUP:
	mpq_EGlpNumFreeArray(weight);
	mpq_EGlpNumFreeArray(sweight);
	mpq_EGlpNumFreeArray(dist);
	EGfree(edges);
	EGfree(in_d);
	EGfree(in_e);
	EGfree(in_beg);
