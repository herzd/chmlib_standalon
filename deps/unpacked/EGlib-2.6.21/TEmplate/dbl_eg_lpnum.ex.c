...
#include "EGlib.h"
/* ========================================================================= */
/** @brief Tester program for double structure and functions
 * @return zero on success, non-zero otherwise 
 * @par Description:
 * Perform various tests on double and their functions 
 * */
int main (int argc,
					char **argv)
{
	...
	mpq_init (qnum);
	#endif
	dbl_EGlpNumInitVar (ntmp[0]);
	dbl_EGlpNumInitVar (ntmp[1]);
	dbl_EGlpNumInitVar (ntmp[2]);
	dbl_EGlpNumInitVar (ntmp[3]);
	...
	#endif
	dbl_EGlpNumReadStr (ntmp[0], argv[1]);
	dbl_EGlpNumReadStr (ntmp[1], argv[2]);
	dtmp[0] = dbl_EGlpNumToLf (ntmp[0]);
	dtmp[1] = dbl_EGlpNumToLf (ntmp[1]);

	/* convert numbers */
	strnum1 = dbl_EGlpNumGetStr (ntmp[0]);
	strnum2 = dbl_EGlpNumGetStr (ntmp[1]);
	...
	/* internal constants */
	strnum3 = dbl_EGlpNumGetStr (dbl_oneLpNum);
	fprintf (stderr, "1.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = dbl_EGlpNumGetStr (dbl_zeroLpNum);
	fprintf (stderr, "0.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = dbl_EGlpNumGetStr (dbl_epsLpNum);
	fprintf (stderr, "eps = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = dbl_EGlpNumGetStr (dbl_MaxLpNum);
	fprintf (stderr, "Max = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = dbl_EGlpNumGetStr (dbl_MinLpNum);
	fprintf (stderr, "Min = %s\n", strnum3);
	EGfree (strnum3);

	/* copying functions */
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s (%lg)\n", strnum3, strnum1, dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumCopyDiff (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s - %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	dbl_EGlpNumCopyDiffRatio (ntmp[2], ntmp[0], ntmp[1], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = (%s  - %s)/%s (%lg)\n", strnum3, strnum1, strnum2,
					 strnum1, (dtmp[0] - dtmp[1]) / dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumCopySum (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s + %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	dbl_EGlpNumCopySqrOver (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s^2/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] * dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumCopyAbs (ntmp[2], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = |%s| (%lg)\n", strnum3, strnum1, fabs (dtmp[0]));
	EGfree (strnum3);
	dbl_EGlpNumCopyNeg (ntmp[2], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = -1*%s (%lg)\n", strnum3, strnum1, -dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumCopyFrac (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	...
	/* add */
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumAddTo (ntmp[2], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s + %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumAddUiTo (ntmp[2], 0xffU);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	...
	/* substract */
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumSubTo (ntmp[2], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumSubUiTo (ntmp[2], 0xffU);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] - 0xffU);
	EGfree (strnum3);
	...
	/* multiply */
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumMultTo (ntmp[2], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s * %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] * dtmp[1]);
	EGfree (strnum3);
	...
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumSign (ntmp[2]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "-(%s) = %s (%lg)\n", strnum1, strnum3, -dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumOne (ntmp[2]);
	dbl_EGlpNumAddInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 + %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 + dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	dbl_EGlpNumOne (ntmp[2]);
	dbl_EGlpNumSubInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 - %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 - dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	...
	/* divide */
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumDivTo (ntmp[2], ntmp[1]);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] / dtmp[1]);
	EGfree (strnum3);
	dbl_EGlpNumCopy (ntmp[2], ntmp[0]);
	dbl_EGlpNumDivUiTo (ntmp[2], 0xffU);
	strnum3 = dbl_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] / 0xffU);
	EGfree (strnum3);
	...
	/* test transformation to rationals */
	{
		...
		mpf_init (f1);
		mpf_init (f2);
		mpf_EGlpNumSet(f1,dbl_EGlpNumToLf(ntmp[0]));
		mpf_EGlpNumSet(f2,dbl_EGlpNumToLf(ntmp[1]));
		...
		fprintf (stderr, "Your input in rational was:\n\t(%10.7lf) %s\n\t(%10.7lf)"
				" %s\n", dbl_EGlpNumToLf (ntmp[0]), strnum1, dbl_EGlpNumToLf (ntmp[1]),
				strnum2);
		EGfree (strnum1);
		EGfree (strnum2);
	...
	/* ending */
	CLEANUP:
	dbl_EGlpNumClearVar (ntmp[0]);
	dbl_EGlpNumClearVar (ntmp[1]);
	dbl_EGlpNumClearVar (ntmp[2]);
	dbl_EGlpNumClearVar (ntmp[3]);
	#ifdef HAVE_LIBGMP
	mpq_clear (qnum);
	#endif
	EGlpNumClear();
