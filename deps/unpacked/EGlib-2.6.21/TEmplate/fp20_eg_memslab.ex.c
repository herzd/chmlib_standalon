...
static void fp20_my_constr (void *ptr)
{
	fp20_EGeHeapCnInit ((fp20_EGeHeapCn_t *) ptr);
}

/* ========================================================================= */
/** @brief simple destructor for heap connectors */
static void fp20_my_dest (void *ptr)
{
	fp20_EGeHeapCnClear ((fp20_EGeHeapCn_t *) ptr);
	ptr = 0;
}
					...
					char **argv)
{
	...
	EGmemSlabPoolInit (&pool, sizeof (fp20_EGeHeapCn_t), fp20_my_constr, fp20_my_dest);
	fp20_EGeHeapInit (&my_heap);
	fp20_EGeHeapChangeD (&my_heap, 4);
	...
	while (n_elem >= 1000)
	{
		for (cnt = 1000; cnt--;)
		{
			if (fp20_EGeHeapIsFull (&my_heap))
				fp20_EGeHeapResize (&my_heap, 1000 + my_heap.sz);
			hp_cn = (fp20_EGeHeapCn_t *) EGmemSlabPoolAlloc (&pool);
			dbl = EGrandU01(&seed);
			fp20_EGlpNumSet (hp_cn->val, dbl);
			fp20_EGeHeapAdd (&my_heap, hp_cn);
		}
		for (cnt = 875; cnt--;)
		{
			hp_cn = fp20_EGeHeapGetMin (&my_heap);
			fp20_EGeHeapDel (&my_heap, hp_cn);
