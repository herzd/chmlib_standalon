...
{
	fp20_EGsrkNode_t *o_end;
	fp20_EGsrkEdge_t *c_edge,
	 *o_edge;
	EGeUgraphEP_t *c_ep;
	EGeList_t *e_it,
	 *e_end,
	...
	fp20_EGlpNumAddTo (base->weight, srkN->weight);
	...
	for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
	{
		...
		c_edge = EGcontainerOf (c_ep, fp20_EGsrkEdge_t, edge.ep[c_ep->type]);
		o_type = c_ep->type ? 0U : 1U;
		o_end = EGcontainerOf (c_edge->edge.ep[o_type].node, fp20_EGsrkNode_t, node);
		o_end->hit = c_edge;
	}
	...
	for (; e_it != e_end; e_it = e_next)
	{
		...
		if (o_end == base)
		{
			fp20_EGlpNumSubTo (base->weight, c_edge->weight);
			fp20_EGlpNumSubTo (base->weight, c_edge->weight);
		...
		fp20_EGlpNumAddTo (o_edge->weight, c_edge->weight);
