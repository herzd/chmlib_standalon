					...
					char **argv)
{
	...
	int v;
	int_EGaHeap_t my_heap;
	int_EGaHeapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = int_EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, int_EGlpNumToLf (all_cn[ccn].val));
		int_EGaHeapDel (all_cn,sizeof(int_EGaHeapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) int_EGaHeapAdd (all_cn,sizeof(int_EGaHeapCn_t),&my_heap, i);
	...
	int_EGaHeapChangeVal (all_cn,sizeof(int_EGaHeapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
