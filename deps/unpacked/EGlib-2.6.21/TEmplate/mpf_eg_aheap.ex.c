					...
					char **argv)
{
	...
	mpf_t v;
	mpf_EGaHeap_t my_heap;
	mpf_EGaHeapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpf_EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, mpf_EGlpNumToLf (all_cn[ccn].val));
		mpf_EGaHeapDel (all_cn,sizeof(mpf_EGaHeapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) mpf_EGaHeapAdd (all_cn,sizeof(mpf_EGaHeapCn_t),&my_heap, i);
	...
	mpf_EGaHeapChangeVal (all_cn,sizeof(mpf_EGaHeapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
