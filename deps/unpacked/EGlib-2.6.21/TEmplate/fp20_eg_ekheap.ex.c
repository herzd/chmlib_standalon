...
void fp20_ekheap_usage (char *program)
{
...
int fp20_ekheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 EGfp20_t * v,
					...
					char **argv)
{
	...
	EGioFile_t *file = 0;
	unsigned int nval = 0;
	EGfp20_t v[fp20_EG_EKHEAP_ENTRY];
	fp20_EGeKHeap_t my_heap;
	fp20_EGeKHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	for( i = fp20_EG_EKHEAP_ENTRY ; i-- ; )
	{
		fp20_EGlpNumInitVar (v[i]);
		fp20_EGlpNumZero (v[i]);
	}
	fp20_EGeKHeapInit (&my_heap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	rval = fp20_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = fp20_ekheap_parseargs (argc, argv, &d, &c, v, &fp20_file_name);
	CHECKRVALG (rval, CLEANUP);
	fp20_EGeKHeapChangeD (&my_heap, d);
	rval = fp20_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Read the objects to sort from the file */
	file = EGioOpen (fp20_file_name, "r");
	...
	nval = atoi(argv1[0]);
	all_cn = EGsMalloc (fp20_EGeKHeapCn_t, nval);
	fp20_EGeKHeapResize (&my_heap, nval);
	rval = fp20_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE(1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		fp20_EGeKHeapCnInit (all_cn + i);
		EGioGets(str1,1024,file);
		EGioNParse(str1,128," ","%#",&argc1,argv1);
		TESTG((rval=(argc1!=1)),CLEANUP,"Expected one token, found %d",argc1);
		fp20_EGlpNumReadStr (all_cn[i].val[0], argv1[0]);
		IFMESSAGE(1, "Adding value (%s,%lf) to the heap", str1,
						 fp20_EGlpNumToLf (all_cn[i].val[0]));
		rval = fp20_EGeKHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = fp20_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = fp20_EGeKHeapGetMin (&my_heap);
		IFMESSAGE(1, "%u: item %zd : %lf", i, ccn - all_cn,
						 fp20_EGlpNumToLf (ccn->val[0]));
		fp20_EGeKHeapDel (&my_heap, ccn);
		rval = fp20_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 fp20_EGlpNumToLf (all_cn[c].val[0]), fp20_EGlpNumToLf (v[0]));
	rval = fp20_EGeKHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = fp20_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
...
CLEANUP:
	...
	if (all_cn)
	{
		for( i = nval ; i-- ; ) fp20_EGeKHeapCnClear(all_cn+i);
		EGfree (all_cn);
	}
	fp20_EGeKHeapClear (&my_heap);
	for( i = fp20_EG_EKHEAP_ENTRY ; i-- ; ) fp20_EGlpNumClearVar (v[i]);
	EGlpNumClear();
	return rval;
}
/* ========================================================================= */
