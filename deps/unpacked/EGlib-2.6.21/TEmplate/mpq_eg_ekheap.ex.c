...
void mpq_ekheap_usage (char *program)
{
...
int mpq_ekheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 mpq_t * v,
					...
					char **argv)
{
	...
	EGioFile_t *file = 0;
	unsigned int nval = 0;
	mpq_t v[mpq_EG_EKHEAP_ENTRY];
	mpq_EGeKHeap_t my_heap;
	mpq_EGeKHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	for( i = mpq_EG_EKHEAP_ENTRY ; i-- ; )
	{
		mpq_EGlpNumInitVar (v[i]);
		mpq_EGlpNumZero (v[i]);
	}
	mpq_EGeKHeapInit (&my_heap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	rval = mpq_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = mpq_ekheap_parseargs (argc, argv, &d, &c, v, &mpq_file_name);
	CHECKRVALG (rval, CLEANUP);
	mpq_EGeKHeapChangeD (&my_heap, d);
	rval = mpq_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Read the objects to sort from the file */
	file = EGioOpen (mpq_file_name, "r");
	...
	nval = atoi(argv1[0]);
	all_cn = EGsMalloc (mpq_EGeKHeapCn_t, nval);
	mpq_EGeKHeapResize (&my_heap, nval);
	rval = mpq_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE(1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpq_EGeKHeapCnInit (all_cn + i);
		EGioGets(str1,1024,file);
		EGioNParse(str1,128," ","%#",&argc1,argv1);
		TESTG((rval=(argc1!=1)),CLEANUP,"Expected one token, found %d",argc1);
		mpq_EGlpNumReadStr (all_cn[i].val[0], argv1[0]);
		IFMESSAGE(1, "Adding value (%s,%lf) to the heap", str1,
						 mpq_EGlpNumToLf (all_cn[i].val[0]));
		rval = mpq_EGeKHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = mpq_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpq_EGeKHeapGetMin (&my_heap);
		IFMESSAGE(1, "%u: item %zd : %lf", i, ccn - all_cn,
						 mpq_EGlpNumToLf (ccn->val[0]));
		mpq_EGeKHeapDel (&my_heap, ccn);
		rval = mpq_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 mpq_EGlpNumToLf (all_cn[c].val[0]), mpq_EGlpNumToLf (v[0]));
	rval = mpq_EGeKHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = mpq_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
...
CLEANUP:
	...
	if (all_cn)
	{
		for( i = nval ; i-- ; ) mpq_EGeKHeapCnClear(all_cn+i);
		EGfree (all_cn);
	}
	mpq_EGeKHeapClear (&my_heap);
	for( i = mpq_EG_EKHEAP_ENTRY ; i-- ; ) mpq_EGlpNumClearVar (v[i]);
	EGlpNumClear();
	return rval;
}
/* ========================================================================= */
