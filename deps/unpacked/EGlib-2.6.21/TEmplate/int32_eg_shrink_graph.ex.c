...
static inline void int32_srk_usage (char **argv)
{
	fprintf (stderr, "Usage: %s input_file\n", argv[0]);
}

/* ========================================================================= */
/** @brief given a int32_EGsrkGraph_t structure, display the current shrunken graph
 * and it's node weights */
static inline void int32_display_srkG (int32_EGsrkGraph_t * G,
																 int32_EGsrkNode_t * const all_nodes,
																 int32_EGsrkEdge_t * const all_edges)
{
	EGeList_t *n_it,
	 *e_it,
	 *n_end,
	 ...
	 *ee_it,
	 *ee_end;
	int32_EGsrkNode_t *lcn,
	 *onode;
	int32_EGsrkEdge_t *ce,
	 *cee;
	EGeUgraphEP_t *cep;
	...
	if (!EGeListIsEmpty (n_end))
	{
		...
		{
			lcn = EGcontainerOf (n_it, int32_EGsrkNode_t, node.node_cn);
			fprintf (stderr, "\t%zd (%lf): ", lcn - all_nodes,
							 int32_EGlpNumToLf (lcn->weight));
			e_end = &(lcn->members);
			if (!EGeListIsEmpty (e_end))
			{
				...
				{
					onode = EGcontainerOf (e_it, int32_EGsrkNode_t, members);
			...
			if (!EGeListIsEmpty (e_end))
			{
				...
				for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
				{
					...
					ce = EGcontainerOf (cep, int32_EGsrkEdge_t, edge.ep[cep->type]);
					fprintf (stderr, "%zd[%lf] ", ce - all_edges,
									 int32_EGlpNumToLf (ce->weight));
					ee_end = &(ce->members);
					if (!EGeListIsEmpty (ee_end))
					{
						...
						{
							cee = EGcontainerOf (ee_it, int32_EGsrkEdge_t, members);
					...
					char **argv)
{
	...
	unsigned int head,
	  tail;
	int32_t cap;
	int32_EGsrkGraph_t G;
	int32_EGsrkNode_t *all_nodes = 0;
	int32_EGsrkEdge_t *all_edges = 0;
	int32_EGsrkNode_t *N1,
	 *N2;
	char l_str[1024];
	...
	int32_EGsrkGraphInit (&G);
	/* test we have an input file */
	if (argc < 2)
	{
		int32_srk_usage (argv);
		rval = 1;
		goto CLEANUP;
	}
	...
	all_nodes = EGsMalloc (int32_EGsrkNode_t, G.n_onodes);
	all_edges = EGsMalloc (int32_EGsrkEdge_t, G.n_oedges);
	for (i = G.n_onodes; i--;)
	{
		int32_EGsrkNodeInit (all_nodes + i);
		int32_EGsrkAddNode (&G, all_nodes + i);
	}
	for (i = G.n_oedges; i--;)
	{
		int32_EGsrkEdgeInit (all_edges + i);
		n_read = fscanf (in_file, "%u %u %s", &head, &tail, l_str);
		TESTG ((rval = (n_read != 3)), CLEANUP, "couldn't read three fields");
		int32_EGlpNumReadStr (cap, l_str);
		int32_EGlpNumCopy (all_edges[i].weight, cap);
		int32_EGsrkAddEdge (&G, all_nodes + head, all_nodes + tail, all_edges + i);
	}
	fclose (in_file);
	in_file = 0;
	fprintf (stderr, "done\n");
	...
	while (G.G.n_nodes > 2)
	{
		//int32_display_srkG (&G, all_nodes, all_edges);
		N1 = EGcontainerOf (G.G.nodes.next, int32_EGsrkNode_t, node.node_cn);
		N2 = EGcontainerOf (N1->node.node_cn.next, int32_EGsrkNode_t, node.node_cn);
		int32_EGsrkIdentifyNodes (&G, N1, N2);
	}
	int32_display_srkG (&G, all_nodes, all_edges);
	/* ending */
CLEANUP:
	...
	for (i = G.n_onodes; i--;)
		int32_EGsrkNodeClear (all_nodes + i);
	for (i = G.n_oedges; i--;)
		int32_EGsrkEdgeClear (all_edges + i);
	if (all_nodes)
		EGfree (all_nodes);
	if (all_edges)
		EGfree (all_edges);
	int32_EGsrkGraphClear (&G);
	EGlpNumClear();
	return rval;
}
