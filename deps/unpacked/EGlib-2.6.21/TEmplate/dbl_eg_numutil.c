										 ...
										 const double * const elem)
{
	...
	for (;;)
	{
		...
		do
			j--;
		while (j && dbl_EGlpNumIsLess (t, elem[perm[j]]));
		if (j < i)
			break;
										 ...
										 const double * const elem)
{
	...
	for (;;)
	{
		do
			i++;
		while (i < sz && dbl_EGlpNumIsLess (t, elem[perm[i]]));
		do
			j--;
		while (dbl_EGlpNumIsLess (elem[perm[j]], t));
		if (j < i)
			break;
		EGswap (perm[i], perm[j], temp);
	}
	EGswap (perm[0], perm[j], temp);
	dbl_EGlpNumClearVar (t);
	dbl_EGutilPermSort2 (j, perm, elem);
	dbl_EGutilPermSort2 (sz - i, perm + i, elem);
}

/* ========================================================================= */
void dbl___EGlpNumInnProd(double*rop,
											double*const __EGa1,
											double*const __EGa2, 
											const size_t length )
{
	...
	dbl_EGlpNumZero((*rop));
	while(__EGdim--) dbl_EGlpNumAddInnProdTo((*rop),__EGa1[__EGdim],__EGa2[__EGdim]);
}
/* ========================================================================= */
/** @} */
