					...
					char **argv)
{
	...
	int32_t v[int32_EG_ABKHEAP_ENTRY];
	int32_EGaBKheap_t my_heap;
	int32_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<int32_EG_ABKHEAP_ENTRY;i++)
	{
	...
	int32_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (int32_file_name, "r");
	TEST (!file, "unable to open file %s", int32_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(int32_EGaBKheapCn_t, nval);
	int32_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		int32_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		int32_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, int32_EGlpNumToLf (all_cn[i].val[0]));
		int32_EGaBKheapAdd (all_cn,sizeof(int32_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = int32_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, int32_EGlpNumToLf (all_cn[ccn].val[0]));
		int32_EGaBKheapDel (all_cn,sizeof(int32_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) int32_EGaBKheapAdd (all_cn,sizeof(int32_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, int32_EGlpNumToLf (all_cn[c].val[0]), int32_EGlpNumToLf (v[0]));
	int32_EGaBKheapChangeVal (all_cn,sizeof(int32_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, int32_EGlpNumToLf (all_cn[ccn].val[0]));
		int32_EGaBKheapDel (all_cn,sizeof(int32_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	int32_EGaBKheapClear(&my_heap);
	for(i=0;i<int32_EG_ABKHEAP_ENTRY;i++)
	{
		int32_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
