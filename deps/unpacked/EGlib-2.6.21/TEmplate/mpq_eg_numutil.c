										 ...
										 const mpq_t * const elem)
{
	...
	for (;;)
	{
		...
		do
			j--;
		while (j && mpq_EGlpNumIsLess (t, elem[perm[j]]));
		if (j < i)
			break;
										 ...
										 const mpq_t * const elem)
{
	...
	for (;;)
	{
		do
			i++;
		while (i < sz && mpq_EGlpNumIsLess (t, elem[perm[i]]));
		do
			j--;
		while (mpq_EGlpNumIsLess (elem[perm[j]], t));
		if (j < i)
			break;
		EGswap (perm[i], perm[j], temp);
	}
	EGswap (perm[0], perm[j], temp);
	mpq_EGlpNumClearVar (t);
	mpq_EGutilPermSort2 (j, perm, elem);
	mpq_EGutilPermSort2 (sz - i, perm + i, elem);
}

/* ========================================================================= */
void mpq___EGlpNumInnProd(mpq_t*rop,
											mpq_t*const __EGa1,
											mpq_t*const __EGa2, 
											const size_t length )
{
	...
	mpq_EGlpNumZero((*rop));
	while(__EGdim--) mpq_EGlpNumAddInnProdTo((*rop),__EGa1[__EGdim],__EGa2[__EGdim]);
}
/* ========================================================================= */
/** @} */
