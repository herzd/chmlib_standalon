...
/** @{ */
#include "EGlib.h"
static char *int_file_name = 0;
static unsigned int int_lvl = 5;
/* ========================================================================= */
/** @brief display usage of this program */
static void int_mc_usage (char **argv)
{
...
typedef struct int_mc_all_cuts_t
{
	unsigned int sz;
	unsigned int max_sz;
	int *cut_val;
...
int int_mc_store_cuts (int value,
									 const unsigned int c_sz,
									 const unsigned int *const cut,
									 void *data)
{
	int_mc_all_cuts_t *cuts = (int_mc_all_cuts_t *) data;
	...
	int_EGlpNumInitVar (cuts->cut_val[cuts->sz]);
	int_EGlpNumCopy (cuts->cut_val[cuts->sz], value);
...
/* ========================================================================= */
/** @brief parse input */
static inline int int_mc_parseargs (int argc,
																char **argv)
{
	int c;
					...
					char **argv)
{
	...
	int cap;
	EGtimer_t cut_timer;
	int_EGalgMCgraph_t G;
	char l_str[4096];
	char*l_argv[128];
	int_EGalgMCcbk_t cb;
	int_mc_all_cuts_t all_cuts;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	all_cuts.max_sz = all_cuts.sz = 0;
	EGtimerReset (&cut_timer);
	int_EGalgMCcbkInit (&cb);
	cb.do_fn = int_mc_store_cuts;
	int_EGlpNumSet (cb.cutoff, 2.0);
	cb.param = &all_cuts;
	int_EGalgMCgraphInit (&G);
	int_EGlpNumInitVar (cap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	/* test we have an input file */
	rval = int_mc_parseargs (argc, argv);
	CHECKRVALG (rval, CLEANUP);
	in_file = EGioOpen (int_file_name, "r");
	if (!in_file)
	{
	...
	fprintf (stderr, "Reading graph on %u nodes and %u edges...", G.G.n_onodes,
					 G.G.n_oedges);
	G.all_nodes = EGsMalloc (int_EGalgMCnode_t, G.G.n_onodes);
	G.all_edges = EGsMalloc (int_EGalgMCedge_t, G.G.n_oedges);
	for (i = G.G.n_onodes; i--;)
	{
		int_EGalgMCnodeInit (G.all_nodes + i);
		EGeListAddAfter (&(G.all_nodes[i].lvl_cn), G.lvl_list);
		G.all_nodes[i].id = i;
		int_EGsrkAddNode (&(G.G), &(G.all_nodes[i].node));
	}
	for (i = G.G.n_oedges; i--;)
	{
		int_EGalgMCedgeInit (G.all_edges + i);
		G.all_edges[i].id = i;
		EGioGets(l_str,(size_t)4095,in_file);
		EGioNParse(l_str,128," ","%#",&n_read,l_argv);
		...
		head = atoi(l_argv[0]);
		tail = atoi(l_argv[1]);
		int_EGlpNumReadStr(cap,l_argv[2]);
		int_EGlpNumCopy (G.all_edges[i].edge.weight, cap);
		int_EGsrkAddEdge (&(G.G), &(G.all_nodes[head].node), &(G.all_nodes[tail].node),
									&(G.all_edges[i].edge));
	}
	int_EGlpNumCopySum (G.cut_val, G.all_nodes[0].node.weight, int_oneLpNum);
	int_EGlpNumCopy (G.epsilon, int_epsLpNum);
	...
	rval = int_EGalgMC (&G, &cb, int_lvl);
	EGtimerStop (&cut_timer);
	fprintf (stderr, "...done in %lf seconds\n\tValue: %lf\n", cut_timer.time,
					 int_EGlpNumToLf (G.cut_val));
	...
	for (i = all_cuts.sz; i--;)
	{
		fprintf (stderr, "cut %u: val %lf, sz %u, elems ", i,
						 int_EGlpNumToLf (all_cuts.cut_val[i]), all_cuts.cut_sz[i]);
...
CLEANUP:
	...
	if (all_cuts.sz)
	{
		for (i = all_cuts.sz; i--;)
		{
			int_EGlpNumClearVar (all_cuts.cut_val[i]);
	...
	{
		for (i = G.G.n_onodes; i--;)
			int_EGalgMCnodeClear (G.all_nodes + i);
		EGfree (G.all_nodes);
	}
	if (G.G.n_oedges)
	{
		for (i = G.G.n_oedges; i--;)
			int_EGalgMCedgeClear (G.all_edges + i);
		EGfree (G.all_edges);
	}
	int_EGalgMCprofile();
	int_EGalgPRprofile();
