...
static inline void ldbl_pr_usage (char **argv)
{
					...
					char **argv)
{
	...
	long double cap;
	EGtimer_t cut_timer;
	ldbl_EGalgPRgraph_t G;
	ldbl_EGalgPRnode_t *nodes = 0/*,
	 *node_pt*/;
	ldbl_EGalgPRedge_t *edges = 0;
	/*EGeList_t *n_it;*/
	char l_str[1024];
	char*l_argv[128];
	int l_argc;
	...
	fprintf (stderr, "Reading graph on %u nodes and %u edges...", n_nodes,
					 n_edges);
	nodes = EGsMalloc (ldbl_EGalgPRnode_t, n_nodes);
	edges = EGsMalloc (ldbl_EGalgPRedge_t, n_edges);
	for (i = n_nodes; i--;)
	{
		ldbl_EGalgPRnodeInit (nodes + i);
		EGeDgraphAddNode (&(G.G), &(nodes[i].v));
	}
	for (i = n_edges; i--;)
	{
		ldbl_EGalgPRedgeInit (edges + i);
		EGioGets(l_str,1024,in_file);
		EGioNParse(l_str,128," ","%#",&l_argc,l_argv);
		...
		ldbl_EGlpNumCopy (edges[i].fw.u, cap);
		ldbl_EGlpNumCopy (edges[i].bw.u, cap);
	}
	EGioClose(in_file);
	in_file = 0;
	...
	rval = ldbl_EGalgPRminSTcut (&G, nodes, nodes + (n_nodes >> 1));
	...
	EGtimerReset (&cut_timer);
	EGtimerStart (&cut_timer);
	rval = ldbl_EGalgPRmaxSTflow (&G, nodes, nodes + (n_nodes >> 1));
	EGtimerStop (&cut_timer);
	fprintf (stderr, "...done in %lf seconds\n", cut_timer.time);
	/* now we print the value of the cut */
	fprintf (stderr, "Minimum Cut Capacity %lf\n",
					 ldbl_EGlpNumToLf (nodes[n_nodes >> 1].e));
	/* check the solution */
	rval = ldbl_EGalgPRoptimalityTest (&G, nodes, nodes + (n_nodes >> 1), &cap);
	TESTG (rval, CLEANUP, "Worst error %lg, standard expected error %lg,"
				 " number of errors %d", ldbl_EGlpNumToLf (cap), ldbl_EGlpNumToLf (ldbl_epsLpNum),
				 rval);
...
CLEANUP:
	...
	ldbl_EGalgPRgraphClear (&G);
	for (i = n_nodes; i--;)
		ldbl_EGalgPRnodeClear (nodes + i);
	for (i = n_edges; i--;)
		ldbl_EGalgPRedgeClear (edges + i);
	EGfree (nodes);
	EGfree (edges);
	ldbl_EGalgPRprofile();
	EGlpNumClear();
