...
void fp20_aheap_usage (char *program)
{
...
int fp20_aheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 EGfp20_t * v,
					...
					char **argv)
{
	...
	EGfp20_t v[fp20_EG_ABKHEAP_ENTRY];
	fp20_EGaBKheap_t my_heap;
	fp20_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<fp20_EG_ABKHEAP_ENTRY;i++)
	{
		fp20_EGlpNumInitVar (v[i]);
		fp20_EGlpNumZero (v[i]);
	}

	/* Parse command line input to get 'file name' and 'd'. */
	EGcallD(fp20_aheap_parseargs(argc, argv, &d, &c, v, &fp20_file_name));
	fp20_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (fp20_file_name, "r");
	TEST (!file, "unable to open file %s", fp20_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(fp20_EGaBKheapCn_t, nval);
	fp20_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		fp20_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		fp20_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, fp20_EGlpNumToLf (all_cn[i].val[0]));
		fp20_EGaBKheapAdd (all_cn,sizeof(fp20_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = fp20_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, fp20_EGlpNumToLf (all_cn[ccn].val[0]));
		fp20_EGaBKheapDel (all_cn,sizeof(fp20_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) fp20_EGaBKheapAdd (all_cn,sizeof(fp20_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, fp20_EGlpNumToLf (all_cn[c].val[0]), fp20_EGlpNumToLf (v[0]));
	fp20_EGaBKheapChangeVal (all_cn,sizeof(fp20_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, fp20_EGlpNumToLf (all_cn[ccn].val[0]));
		fp20_EGaBKheapDel (all_cn,sizeof(fp20_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	fp20_EGaBKheapClear(&my_heap);
	for(i=0;i<fp20_EG_ABKHEAP_ENTRY;i++)
	{
		fp20_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
