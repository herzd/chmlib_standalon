...
void mpf_aheap_usage (char *program)
{
...
int mpf_aheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 mpf_t * v,
	...
	while ((c = getopt (argc, argv, "f:d:c:v:")) != EOF)
	{
		...
		case 'c':
			*ch = atoi (optarg);
			break;
		case 'v':
			mpf_EGlpNumReadStr (*v, optarg);
			break;
		default:
			mpf_aheap_usage (argv[0]);
			return 1;
		}
	}
					...
					char **argv)
{
	...
	unsigned int i,
	  c = UINT_MAX,
	  d = 2;
	char *mpf_file_name = 0,
	  str1[1024];
	FILE *file;
	unsigned int nval,ccn;
	mpf_t v[mpf_EG_ABKHEAP_ENTRY];
	mpf_EGaBKheap_t my_heap;
	mpf_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<mpf_EG_ABKHEAP_ENTRY;i++)
	{
		mpf_EGlpNumInitVar (v[i]);
		mpf_EGlpNumZero (v[i]);
	}

	/* Parse command line input to get 'file name' and 'd'. */
	EGcallD(mpf_aheap_parseargs(argc, argv, &d, &c, v, &mpf_file_name));
	mpf_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (mpf_file_name, "r");
	TEST (!file, "unable to open file %s", mpf_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(mpf_EGaBKheapCn_t, nval);
	mpf_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpf_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		mpf_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, mpf_EGlpNumToLf (all_cn[i].val[0]));
		mpf_EGaBKheapAdd (all_cn,sizeof(mpf_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpf_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, mpf_EGlpNumToLf (all_cn[ccn].val[0]));
		mpf_EGaBKheapDel (all_cn,sizeof(mpf_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) mpf_EGaBKheapAdd (all_cn,sizeof(mpf_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, mpf_EGlpNumToLf (all_cn[c].val[0]), mpf_EGlpNumToLf (v[0]));
	mpf_EGaBKheapChangeVal (all_cn,sizeof(mpf_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, mpf_EGlpNumToLf (all_cn[ccn].val[0]));
		mpf_EGaBKheapDel (all_cn,sizeof(mpf_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	mpf_EGaBKheapClear(&my_heap);
	for(i=0;i<mpf_EG_ABKHEAP_ENTRY;i++)
	{
		mpf_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
