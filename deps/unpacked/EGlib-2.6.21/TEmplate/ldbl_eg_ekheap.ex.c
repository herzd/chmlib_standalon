...
void ldbl_ekheap_usage (char *program)
{
...
int ldbl_ekheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 long double * v,
					...
					char **argv)
{
	...
	EGioFile_t *file = 0;
	unsigned int nval = 0;
	long double v[ldbl_EG_EKHEAP_ENTRY];
	ldbl_EGeKHeap_t my_heap;
	ldbl_EGeKHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	for( i = ldbl_EG_EKHEAP_ENTRY ; i-- ; )
	{
		ldbl_EGlpNumInitVar (v[i]);
		ldbl_EGlpNumZero (v[i]);
	}
	ldbl_EGeKHeapInit (&my_heap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	rval = ldbl_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = ldbl_ekheap_parseargs (argc, argv, &d, &c, v, &ldbl_file_name);
	CHECKRVALG (rval, CLEANUP);
	ldbl_EGeKHeapChangeD (&my_heap, d);
	rval = ldbl_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Read the objects to sort from the file */
	file = EGioOpen (ldbl_file_name, "r");
	...
	nval = atoi(argv1[0]);
	all_cn = EGsMalloc (ldbl_EGeKHeapCn_t, nval);
	ldbl_EGeKHeapResize (&my_heap, nval);
	rval = ldbl_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE(1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		ldbl_EGeKHeapCnInit (all_cn + i);
		EGioGets(str1,1024,file);
		EGioNParse(str1,128," ","%#",&argc1,argv1);
		TESTG((rval=(argc1!=1)),CLEANUP,"Expected one token, found %d",argc1);
		ldbl_EGlpNumReadStr (all_cn[i].val[0], argv1[0]);
		IFMESSAGE(1, "Adding value (%s,%lf) to the heap", str1,
						 ldbl_EGlpNumToLf (all_cn[i].val[0]));
		rval = ldbl_EGeKHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = ldbl_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = ldbl_EGeKHeapGetMin (&my_heap);
		IFMESSAGE(1, "%u: item %zd : %lf", i, ccn - all_cn,
						 ldbl_EGlpNumToLf (ccn->val[0]));
		ldbl_EGeKHeapDel (&my_heap, ccn);
		rval = ldbl_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 ldbl_EGlpNumToLf (all_cn[c].val[0]), ldbl_EGlpNumToLf (v[0]));
	rval = ldbl_EGeKHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = ldbl_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
...
CLEANUP:
	...
	if (all_cn)
	{
		for( i = nval ; i-- ; ) ldbl_EGeKHeapCnClear(all_cn+i);
		EGfree (all_cn);
	}
	ldbl_EGeKHeapClear (&my_heap);
	for( i = ldbl_EG_EKHEAP_ENTRY ; i-- ; ) ldbl_EGlpNumClearVar (v[i]);
	EGlpNumClear();
	return rval;
}
/* ========================================================================= */
