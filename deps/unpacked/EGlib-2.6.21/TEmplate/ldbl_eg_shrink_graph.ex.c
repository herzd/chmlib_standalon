...
static inline void ldbl_srk_usage (char **argv)
{
	fprintf (stderr, "Usage: %s input_file\n", argv[0]);
}

/* ========================================================================= */
/** @brief given a ldbl_EGsrkGraph_t structure, display the current shrunken graph
 * and it's node weights */
static inline void ldbl_display_srkG (ldbl_EGsrkGraph_t * G,
																 ldbl_EGsrkNode_t * const all_nodes,
																 ldbl_EGsrkEdge_t * const all_edges)
{
	EGeList_t *n_it,
	 *e_it,
	 *n_end,
	...
	if (!EGeListIsEmpty (n_end))
	{
		...
		{
			lcn = EGcontainerOf (n_it, ldbl_EGsrkNode_t, node.node_cn);
			fprintf (stderr, "\t%zd (%lf): ", lcn - all_nodes,
							 ldbl_EGlpNumToLf (lcn->weight));
			e_end = &(lcn->members);
			if (!EGeListIsEmpty (e_end))
			{
				...
				{
					onode = EGcontainerOf (e_it, ldbl_EGsrkNode_t, members);
			...
			if (!EGeListIsEmpty (e_end))
			{
				...
				for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
				{
					...
					ce = EGcontainerOf (cep, ldbl_EGsrkEdge_t, edge.ep[cep->type]);
					fprintf (stderr, "%zd[%lf] ", ce - all_edges,
									 ldbl_EGlpNumToLf (ce->weight));
					ee_end = &(ce->members);
					if (!EGeListIsEmpty (ee_end))
					{
						...
						{
							cee = EGcontainerOf (ee_it, ldbl_EGsrkEdge_t, members);
					...
					char **argv)
{
	...
	long double cap;
	ldbl_EGsrkGraph_t G;
	ldbl_EGsrkNode_t *all_nodes = 0;
	ldbl_EGsrkEdge_t *all_edges = 0;
	ldbl_EGsrkNode_t *N1,
	 *N2;
	char l_str[1024];
	...
	all_nodes = EGsMalloc (ldbl_EGsrkNode_t, G.n_onodes);
	all_edges = EGsMalloc (ldbl_EGsrkEdge_t, G.n_oedges);
	for (i = G.n_onodes; i--;)
	{
		ldbl_EGsrkNodeInit (all_nodes + i);
		ldbl_EGsrkAddNode (&G, all_nodes + i);
	}
	for (i = G.n_oedges; i--;)
	{
		ldbl_EGsrkEdgeInit (all_edges + i);
		n_read = fscanf (in_file, "%u %u %s", &head, &tail, l_str);
		TESTG ((rval = (n_read != 3)), CLEANUP, "couldn't read three fields");
		ldbl_EGlpNumReadStr (cap, l_str);
		ldbl_EGlpNumCopy (all_edges[i].weight, cap);
		ldbl_EGsrkAddEdge (&G, all_nodes + head, all_nodes + tail, all_edges + i);
	}
	fclose (in_file);
	in_file = 0;
	fprintf (stderr, "done\n");
	...
	while (G.G.n_nodes > 2)
	{
		//ldbl_display_srkG (&G, all_nodes, all_edges);
		N1 = EGcontainerOf (G.G.nodes.next, ldbl_EGsrkNode_t, node.node_cn);
		N2 = EGcontainerOf (N1->node.node_cn.next, ldbl_EGsrkNode_t, node.node_cn);
		ldbl_EGsrkIdentifyNodes (&G, N1, N2);
	}
	ldbl_display_srkG (&G, all_nodes, all_edges);
	/* ending */
CLEANUP:
	...
	for (i = G.n_onodes; i--;)
		ldbl_EGsrkNodeClear (all_nodes + i);
	for (i = G.n_oedges; i--;)
		ldbl_EGsrkEdgeClear (all_edges + i);
	if (all_nodes)
		EGfree (all_nodes);
	if (all_edges)
		EGfree (all_edges);
	ldbl_EGsrkGraphClear (&G);
	EGlpNumClear();
	return rval;
}
