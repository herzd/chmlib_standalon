...
void mpq_eheap_usage (char *program)
{
...
int mpq_eheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 mpq_t * v,
					...
					char **argv)
{
	...
	unsigned int nval;
	mpq_t v;
	mpq_EGeHeap_t my_heap;
	mpq_EGeHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	mpq_EGlpNumZero (v);
	mpq_EGeHeapInit (&my_heap);
	rval = mpq_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = mpq_eheap_parseargs (argc, argv, &d, &c, &v, &mpq_file_name);
	CHECKRVALG (rval, CLEANUP);
	mpq_EGeHeapChangeD (&my_heap, d);
	rval = mpq_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	...
	fscanf (file, "%u", &nval);
	all_cn = EGsMalloc (mpq_EGeHeapCn_t, nval);
	mpq_EGeHeapResize (&my_heap, nval);
	rval = mpq_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpq_EGeHeapCnInit (all_cn + i);
		fscanf (file, "%s", str1);
		mpq_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1,
						 mpq_EGlpNumToLf (all_cn[i].val));
		rval = mpq_EGeHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = mpq_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpq_EGeHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %zd : %lg", i, ccn - all_cn,
						 mpq_EGlpNumToLf (ccn->val));
		mpq_EGeHeapDel (&my_heap, ccn);
		rval = mpq_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 mpq_EGlpNumToLf (all_cn[c].val), mpq_EGlpNumToLf (v));
	rval = mpq_EGeHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = mpq_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
