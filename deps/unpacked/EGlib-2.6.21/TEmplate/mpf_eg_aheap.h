...
/** @addtogroup EGaHeap */
/** @{ */
/** @example mpf_eg_aheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaHeap */
/* ========================================================================= */
#ifndef mpf___EG_AHEAP__
#define mpf___EG_AHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef mpf_EG_AHEAP_DEBUG
#define mpf_EG_AHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if mpf_EG_AHEAP_DEBUG <= DEBUG
#define mpf_EGaHeapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define mpf_EGaHeapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define mpf_EGaHeapCHECK_CN(__hp,__hcn) 
#define mpf_EGaHeapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
...
typedef struct 
{
	...
	unsigned pos;					/**< Position in the heap-array for this node, if set to
												 #mpf_EG_AHEAP_POISON, then the connector is not in any 
												 heap.*/
}
mpf_EGaHeapCn_t;
...
#define mpf_EG_AHEAP_POISON UINT_MAX
 ...
 * */
#define mpf_EGaHeapCnInit(__hcn) do{mpf_EGlpNumInitVar((__hcn)->val);(__hcn)->pos = mpf_EG_AHEAP_POISON;}while(0)
 ...
 * */
#define mpf_EGaHeapCnReset(__hcn) ((__hcn)->pos = mpf_EG_AHEAP_POISON)
 ...
 * */
#define mpf_EGaHeapCnClear(__hcn) mpf_EGlpNumClearVar((__hcn)->val)
...
#define mpf_EGaHeapIsFull(__hp) ({mpf_EGaHeap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define mpf_EGaHeapEmpty(__hp) ((__hp)->sz = 0)
 ...
 * */
#define mpf_EGaHeapInit(__hp,__d) (*(__hp) = (mpf_EGaHeap_t){0,__d,0,0})
 ...
 * */
#define mpf_EGaHeapReset(__hp) mpf_EGaHeapResize(__hp,0)
 ...
 * */
#define mpf_EGaHeapClear(__hp) mpf_EGaHeapResize(__hp,0)
 ...
 * */
#define mpf_EGaHeapGetMinVal(__hp,__number) ({\
	mpf_EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? (mpf_EGlpNumCopy(__number,__EGehp->cn[0]->val),0):1;})
 ...
 * */
#define mpf_EGaHeapGetMin(__hp) ({\
	mpf_EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : mpf_EG_AHEAP_POISON;})
 ...
 * */
#define mpf_EGaHeapResize(__hp,__new_sz) ({\
	mpf_EGaHeap_t*const __EGehp = (__hp);\
 ...
 * */
#define mpf_EGaHeapFatherId(__d,__id) ((unsigned)((__id)?(((__id)-1)/(__d)):0))
 ...
 * */
#define mpf___EGaHeapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const size_t __EGstsz3=((size_t)(__psz3));\
	mpf_EGaHeap_t*const __EGehp3 = (__hp3);\
	const unsigned __EGapos3 = __EGehp3->cn[__hcn3];\
	mpf_EGaHeapCn_t*const __EGecn3 = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	unsigned __EGcpos3 = __EGecn3->pos;\
	unsigned __EGfpos3 = mpf_EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
	mpf_EGaHeapCn_t*__EGfcn3 = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	mpf_EGaHeapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				mpf_EGlpNumIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((unsigned)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = mpf_EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
		__EGfcn3 = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define mpf_EGaHeapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	mpf_EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	mpf_EGaHeapCn_t*const __EGlcn = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpf_EGaHeapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	mpf___EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
...
#define mpf_EGaHeapFirstChildId(__d,__id) ((unsigned)((__d)*(__id)+1U))
/* ========================================================================= */
 ...
 * */
#define mpf___EGaHeapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const size_t __EGstsz=((size_t)(__psz));\
	mpf_EGaHeap_t*const __EGehp = (__hp);\
	const unsigned __EGapos = __EGehp->cn[__hcn];\
	mpf_EGaHeapCn_t*const __EGecn = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const unsigned __EGhsz = __EGehp->sz;\
	unsigned __EGcpos = __EGecn->pos;\
	unsigned __EGfchd = mpf_EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
	unsigned __EGlchd = __EGfchd + __EGehp->d;\
	mpf_EGaHeapCn_t*__EGcchd=0,*__EGnchd=0;\
	register unsigned __EGehi = 0;\
	mpf_EGaHeapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		for(__EGehi = (unsigned)((__EGlchd > __EGhsz) ? __EGhsz-1U:__EGlchd-1U); __EGehi > __EGfchd ; __EGehi--){\
			if(mpf_EGlpNumIsLeq((__EGnchd=((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGehi]))))->val,__EGcchd->val))\
				__EGcchd = __EGnchd;}\
		...
		{\
			...
			__EGfchd = mpf_EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
			__EGlchd = __EGfchd + __EGehp->d;\
		}\
		/* else we exit the main loop */\
		else __EGfchd = mpf_EG_AHEAP_POISON;\
	}\
}while(0)
 ...
 * */
#define mpf_EGaHeapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	mpf_EGaHeap_t*const __EGlhp=(__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	mpf_EGaHeapCn_t*__EGlcn2=((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpf_t __EGnum;\
	mpf_EGlpNumCopy(__EGnum,(__new_val));\
	if(mpf_EGlpNumIsLess(__EGnum,__EGlcn2->val)){\
		mpf_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		mpf___EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		mpf_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		mpf___EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define mpf_EGaHeapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	mpf_EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	mpf_EGaHeapCn_t*const __EGlhpcn = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const unsigned __EGlcn = __EGlhpcn->pos;\
	const unsigned __EGlhsz = __EGlhp->sz - 1;\
	mpf_EGaHeapCn_t*const __EGllcn = ((mpf_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = mpf_EG_AHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		mpf___EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = mpf_EG_AHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of mpf_eg_aheap.h */
#endif
#endif
#endif
