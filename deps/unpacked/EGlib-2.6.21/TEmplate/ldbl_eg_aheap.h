...
/** @addtogroup EGaHeap */
/** @{ */
/** @example ldbl_eg_aheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaHeap */
/* ========================================================================= */
#ifndef ldbl___EG_AHEAP__
#define ldbl___EG_AHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef ldbl_EG_AHEAP_DEBUG
#define ldbl_EG_AHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if ldbl_EG_AHEAP_DEBUG <= DEBUG
#define ldbl_EGaHeapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define ldbl_EGaHeapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define ldbl_EGaHeapCHECK_CN(__hp,__hcn) 
#define ldbl_EGaHeapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
...
typedef struct 
{
	...
	unsigned pos;					/**< Position in the heap-array for this node, if set to
												 #ldbl_EG_AHEAP_POISON, then the connector is not in any 
												 heap.*/
}
ldbl_EGaHeapCn_t;
...
#define ldbl_EG_AHEAP_POISON UINT_MAX
 ...
 * */
#define ldbl_EGaHeapCnInit(__hcn) do{ldbl_EGlpNumInitVar((__hcn)->val);(__hcn)->pos = ldbl_EG_AHEAP_POISON;}while(0)
 ...
 * */
#define ldbl_EGaHeapCnReset(__hcn) ((__hcn)->pos = ldbl_EG_AHEAP_POISON)
 ...
 * */
#define ldbl_EGaHeapCnClear(__hcn) ldbl_EGlpNumClearVar((__hcn)->val)
...
#define ldbl_EGaHeapIsFull(__hp) ({ldbl_EGaHeap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define ldbl_EGaHeapEmpty(__hp) ((__hp)->sz = 0)
 ...
 * */
#define ldbl_EGaHeapInit(__hp,__d) (*(__hp) = (ldbl_EGaHeap_t){0,__d,0,0})
 ...
 * */
#define ldbl_EGaHeapReset(__hp) ldbl_EGaHeapResize(__hp,0)
 ...
 * */
#define ldbl_EGaHeapClear(__hp) ldbl_EGaHeapResize(__hp,0)
 ...
 * */
#define ldbl_EGaHeapGetMinVal(__hp,__number) ({\
	ldbl_EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? (ldbl_EGlpNumCopy(__number,__EGehp->cn[0]->val),0):1;})

/* ========================================================================= */
/** @brief get the minimum conector in the heap, if the heap is empty, return
 * ldbl_EG_AHEAP_POISON.
 * @param __hp eap where we are working.
 * @return position (in the base array) of the minimum element in the heap.
 * */
#define ldbl_EGaHeapGetMin(__hp) ({\
	ldbl_EGaHeap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : ldbl_EG_AHEAP_POISON;})
 ...
 * */
#define ldbl_EGaHeapResize(__hp,__new_sz) ({\
	ldbl_EGaHeap_t*const __EGehp = (__hp);\
 ...
 * */
#define ldbl_EGaHeapFatherId(__d,__id) ((unsigned)((__id)?(((__id)-1)/(__d)):0))
 ...
 * */
#define ldbl___EGaHeapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const size_t __EGstsz3=((size_t)(__psz3));\
	ldbl_EGaHeap_t*const __EGehp3 = (__hp3);\
	const unsigned __EGapos3 = __EGehp3->cn[__hcn3];\
	ldbl_EGaHeapCn_t*const __EGecn3 = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	unsigned __EGcpos3 = __EGecn3->pos;\
	unsigned __EGfpos3 = ldbl_EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
	ldbl_EGaHeapCn_t*__EGfcn3 = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	ldbl_EGaHeapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				ldbl_EGlpNumIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((unsigned)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = ldbl_EGaHeapFatherId(__EGehp3->d,__EGcpos3);\
		__EGfcn3 = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define ldbl_EGaHeapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	ldbl_EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	ldbl_EGaHeapCn_t*const __EGlcn = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	ldbl_EGaHeapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	ldbl___EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
...
#define ldbl_EGaHeapFirstChildId(__d,__id) ((unsigned)((__d)*(__id)+1U))
/* ========================================================================= */
 ...
 * */
#define ldbl___EGaHeapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const size_t __EGstsz=((size_t)(__psz));\
	ldbl_EGaHeap_t*const __EGehp = (__hp);\
	const unsigned __EGapos = __EGehp->cn[__hcn];\
	ldbl_EGaHeapCn_t*const __EGecn = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const unsigned __EGhsz = __EGehp->sz;\
	unsigned __EGcpos = __EGecn->pos;\
	unsigned __EGfchd = ldbl_EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
	unsigned __EGlchd = __EGfchd + __EGehp->d;\
	ldbl_EGaHeapCn_t*__EGcchd=0,*__EGnchd=0;\
	register unsigned __EGehi = 0;\
	ldbl_EGaHeapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		for(__EGehi = (unsigned)((__EGlchd > __EGhsz) ? __EGhsz-1U:__EGlchd-1U); __EGehi > __EGfchd ; __EGehi--){\
			if(ldbl_EGlpNumIsLeq((__EGnchd=((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGehi]))))->val,__EGcchd->val))\
				__EGcchd = __EGnchd;}\
		...
		{\
			...
			__EGfchd = ldbl_EGaHeapFirstChildId(__EGehp->d,__EGcpos);\
			__EGlchd = __EGfchd + __EGehp->d;\
		}\
		/* else we exit the main loop */\
		else __EGfchd = ldbl_EG_AHEAP_POISON;\
	}\
}while(0)
 ...
 * */
#define ldbl_EGaHeapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	ldbl_EGaHeap_t*const __EGlhp=(__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	ldbl_EGaHeapCn_t*__EGlcn2=((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	long double __EGnum;\
	ldbl_EGlpNumCopy(__EGnum,(__new_val));\
	if(ldbl_EGlpNumIsLess(__EGnum,__EGlcn2->val)){\
		ldbl_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		ldbl___EGaHeapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		ldbl_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		ldbl___EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define ldbl_EGaHeapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const size_t __EGstsz2=((size_t)(__psz));\
	ldbl_EGaHeap_t*const __EGlhp = (__hp);\
	const unsigned __EGapos2=(unsigned)(__hcn);\
	ldbl_EGaHeapCn_t*const __EGlhpcn = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const unsigned __EGlcn = __EGlhpcn->pos;\
	const unsigned __EGlhsz = __EGlhp->sz - 1;\
	ldbl_EGaHeapCn_t*const __EGllcn = ((ldbl_EGaHeapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = ldbl_EG_AHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		ldbl___EGaHeapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = ldbl_EG_AHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of ldbl_eg_aheap.h */
#endif
#endif
#endif
