...
/** @{ */
static unsigned long long ldbl___PR_profile_push = 0;
static unsigned long long ldbl___PR_profile_numb = 0;
static unsigned long long ldbl___PR_profile_lvel = 0;
static unsigned long long ldbl___PR_profile_move = 0;
static unsigned long long ldbl___PR_profile_rela = 0;
void ldbl_EGalgPRprofile(void)
{
	if(ldbl___PR_PROFILE__ <= DEBUG)
	{
		fprintf(stderr,"PROFILE FOR EGalgPushRelabel:\nnpush %llu\n"
				"nrela %llu\nnlevl %llu\nnmove %llu\nnnumb %llu\n",
				ldbl___PR_profile_push, ldbl___PR_profile_rela, ldbl___PR_profile_lvel,
				ldbl___PR_profile_move,ldbl___PR_profile_numb); ldbl___PR_profile_push = 
			ldbl___PR_profile_numb = ldbl___PR_profile_lvel = ldbl___PR_profile_move = 
			ldbl___PR_profile_rela = 0;
	}
}
#if ldbl___PR_PROFILE__ <= DEBUG
...
static unsigned long long ldbl___last_global = 0;

/* ========================================================================= */
/** @brief Compute the exact label distance in the graph to the given node.
 * @param t pointer to the ldbl_EGalgPRnode_t node to wich we are computing the
 * exact label distances.
 * @param G pointer to the ldbl_EGalgPRgraph_t graph where we are working.
 ...
 * #ldbl_EGalgPRnode_t::LVL_list is set to {0,0}. We also assume that the field
 * #ldbl_EGalgPRnode_t::d is set to a suficiently large value that denotes that they
 * are disconnected from @a t, this value should be at least the number of nodes
 * in the graph that we are working on. */
static inline int ldbl_EGalgPRcomputeLabels (ldbl_EGalgPRgraph_t * const G,
																				...
																				EGeList_t * DIST)
{
	...
	unsigned int d;
	ldbl_EGalgPRnode_t *node_pt,
	 *parent_pt;
	ldbl_EGalgPRse_t *edge;
	MESSAGE (ldbl___PR_VERBOSE__, "entering ");
	/* initialize the BFS list and set the distance to t as zero */
	numb[0]++;
	EGeListAddAfter (&(t->T_cut), DIST);
	t->d = 0;
	MESSAGE (ldbl___PR_VERBOSE__, "label %u", 0);
	...
	{
		edge = EGcontainerOf (node_it, ldbl_EGalgPRse_t, e.head_cn);
		/* discard edges without residual capacities */
		if (ldbl_EGlpNumIsLeq (edge->r, ldbl_epsLpNum))
			continue;
		node_pt = EGcontainerOf (edge->e.tail, ldbl_EGalgPRnode_t, v);
		...
		MESSAGE (ldbl___PR_VERBOSE__, "label %u", 1);
	}

	/* now we traverse the graph in BFS mode */
	while (!EGeListIsEmpty (BFS_head))
	{
		parent_pt = EGcontainerOf (BFS_head->prev, ldbl_EGalgPRnode_t, LVL_list);
		...
		for (; node_it != node_head; node_it = node_it->next)
		{
			...
			if (d < n_nodes)
			{
				...
				MESSAGE (ldbl___PR_VERBOSE__, "label %u", d);
			}
		}
	}
	...
	for (; node_it != node_head; node_it = node_it->next)
	{
		node_pt = EGcontainerOf (node_it, ldbl_EGalgPRnode_t, v.node_cn);
	...
	/* ending */
	MESSAGE (ldbl___PR_VERBOSE__, "done ");
	return rval;
}

/* ========================================================================= */
/** @brief Perform a push operation in an edge.
 * @param edge_pt pointer to ldbl_EGalgPRse_t edge where we will perform the push.
...
static inline int ldbl_EGalgPRpush (ldbl_EGalgPRse_t * const edge_pt,
															 long double flow,
															 ...
															 const unsigned int n_nodes)
{
	/* local variables */
	int rval = 0;
	ldbl_EGalgPRse_t *const inv_edge = edge_pt->type ?
		&(EGcontainerOf (edge_pt, ldbl_EGalgPRedge_t, bw)->fw) :
		&(EGcontainerOf (edge_pt, ldbl_EGalgPRedge_t, fw)->bw);
	ldbl_EGalgPRnode_t *const head = EGcontainerOf (edge_pt->e.head, ldbl_EGalgPRnode_t, v);
	ldbl_EGalgPRnode_t *const tail = EGcontainerOf (edge_pt->e.tail, ldbl_EGalgPRnode_t, v);
	MESSAGE (ldbl___PR_VERBOSE__, "pushing %lg r %lg u %lg", ldbl_EGlpNumToLf (flow),
					 ldbl_EGlpNumToLf (edge_pt->r), ldbl_EGlpNumToLf (edge_pt->u));
	ldbl_UPDATE (ldbl___PR_profile_push);

	/* test feasibility of the change */
	EXITL (ldbl___PR_DEBUGL__, ldbl_EGlpNumIsLess (edge_pt->r, flow),
				 "Pushing more flow (%lg) "
				 "than the residual capacity (%lg) on the edge", ldbl_EGlpNumToLf (flow),
				 ldbl_EGlpNumToLf (edge_pt->r));

	/* make the changes where necesary */
	ldbl_EGlpNumSubTo (edge_pt->r, flow);
	ldbl_EGlpNumAddTo (inv_edge->r, flow);
	ldbl_EGlpNumSubTo (tail->e, flow);
	ldbl_EGlpNumAddTo (head->e, flow);
	...
	if (!(head->LVL_list.next) && head->d < n_nodes)
	{
		...
		MESSAGE (ldbl___PR_VERBOSE__, "activating label %u", head->d);
	}
...
/* ========================================================================= */
/** @brief Initialize flow to zero, and saturate forward edges in the ldbl_source.
 * @param s pointer to the ldbl_EGalgPRnode_t node ldbl_source for the flow.
 * @param t pointer to the ldbl_EGalgPRnode_t node sink for the flow
 * @param G pointer to the ldbl_EGalgPRgraph_t graph where we are working.
 * @param numb array of integers where numb[k] contain the number of nodes
 * with distance labels equal to k. This array should be of size at least
 * n_nodes.
...
static inline int ldbl_EGalgPRpreprocess (ldbl_EGalgPRgraph_t * const G,
																		 ...
																		 EGeList_t * const DIST)
{
	...
	for (; node_it != node_head; node_it = node_it->next)
	{
		...
		{
		0, 0};
		ldbl_EGlpNumZero (node_pt->e);
		edge_head = &(node_pt->v.in_edge);
		edge_it = edge_head->next;
		for (; edge_it != edge_head; edge_it = edge_it->next)
		{
			edge_pt = EGcontainerOf (edge_it, ldbl_EGalgPRse_t, e.head_cn);
			ldbl_EGlpNumCopy (edge_pt->r, edge_pt->u);
		}
	}

	/* now we compute the exact label distances */
	rval = ldbl_EGalgPRcomputeLabels (G, t, numb, DIST);
	CHECKRVAL (rval);
	...
	for (; edge_it != edge_head; edge_it = edge_it->next)
	{
		edge_pt = EGcontainerOf (edge_it, ldbl_EGalgPRse_t, e.tail_cn);
		rval = ldbl_EGalgPRpush (edge_pt, edge_pt->u, LEVEL, n_nodes);
		CHECKRVAL (rval);
	}
	/* set the distance from s to t to the number of nodes in the graph */
	MESSAGE (ldbl___PR_VERBOSE__, "reset label from %u to %u", s->d, n_nodes);
	if (s->d < n_nodes)
	{
		numb[s->d]--;
		EGeListDel (&(s->T_cut));
 ...
 * @param node_pt pointer to the ldbl_EGalgPRnode_t node structure that we will
 * relabel.
 * @param numb array of integers where numb[k] contain the number of nodes
 * with distance labels equal to k. This array should be of size at least
 * n_nodes.
...
static int ldbl_EGalgPRpushRelabel (ldbl_EGalgPRnode_t * const node_pt,
															 unsigned int *const numb,
															 const unsigned int n_nodes,
															 EGeList_t * const LEVEL,
															 EGeList_t * const DIST)
{
	...
	ldbl_EGalgPRnode_t *head_pt;
	ldbl_EGalgPRse_t *edge_pt;
	ldbl_EGlpNumInitVar (flow);
	MESSAGE (ldbl___PR_VERBOSE__, "entering ");
	EXITL (ldbl___PR_DEBUGL__, !node_pt->LVL_list.next, "This is inconcivable!");
	/* loop through all residual outgoing edges */
	for (; edge_it != edge_head; edge_it = edge_it->next)
	{
		edge_pt = EGcontainerOf (edge_it, ldbl_EGalgPRse_t, e.tail_cn);
		head_pt = EGcontainerOf (edge_pt->e.head, ldbl_EGalgPRnode_t, v);
		/* if the edge is not in the residual network, discard it */
		if (ldbl_EGlpNumIsLeq (edge_pt->r, ldbl_epsLpNum))
		{
			continue;
		}
		...
		if (min_d > head_pt->d)
		{
			MESSAGE (ldbl___PR_VERBOSE__, "set min_d from %u to %u", min_d, head_pt->d);
			min_d = head_pt->d;
		}
		...
		inf_flow = 0;
		ldbl_EGlpNumCopy (flow, *excess);
		if (ldbl_EGlpNumIsLess (edge_pt->r, flow))
			ldbl_EGlpNumCopy (flow, edge_pt->r);
		ldbl_EGalgPRpush (edge_pt, flow, LEVEL, n_nodes);
		/* if after the push the excess is exausted, we exit the loop */
		if (ldbl_EGlpNumIsLeq (*excess, ldbl_epsLpNum))
			break;
	}
	...
	if (!inf_flow)
	{
		...
		if (ldbl_EGlpNumIsLeq (*excess, ldbl_epsLpNum))
		{
			MESSAGE (ldbl___PR_VERBOSE__, "saturated push, de-activating level %u",
							 node_pt->d);
		...
		{
			MESSAGE (ldbl___PR_VERBOSE__, "non-saturated push cur %u min_d %d",
							 node_pt->d, min_d);
		}
	}
	else
	{
		...
		EXITL (ldbl___PR_DEBUGL__, min_d <= node_pt->d, "relabeling to lower or equal "
					 "value %u %u", min_d, node_pt->d);
		numb[node_pt->d]--;
		EGeListDel (&(node_pt->T_cut));
		ldbl_UPDATE (ldbl___PR_profile_rela);
		ldbl___last_global++;
		if (min_d < n_nodes)
		{
			MESSAGE (ldbl___PR_VERBOSE__, "relabeling to %u from %u", min_d, node_pt->d);
			numb[min_d]++;
		...
		{
			MESSAGE (ldbl___PR_VERBOSE__,
							 "relabeling to %u from %u and moving it to the S" " cut",
							 n_nodes, node_pt->d);
			node_pt->d = min_d;
			ldbl_UPDATE (ldbl___PR_profile_move);
			EGeListDel (&(node_pt->LVL_list));
			node_pt->LVL_list = (EGeList_t)
			{
			0, 0};
 ...
 * @return zero on success, non-zero otherwise.
 * */
static inline int ldbl_EGalgPRnumb (const unsigned int hole,
															 ...
															 EGeList_t * const DIST)
{
	...
	for (d = hole + 1; d < max_numb; d++)
	{
		for (i = numb[d]; i--;)
		{
			MESSAGE (ldbl___PR_VERBOSE__, "eliminating node of level %u", d);
			ldbl_UPDATE (ldbl___PR_profile_numb);
			node_pt = EGcontainerOf (DIST[d].next, ldbl_EGalgPRnode_t, T_cut);
			node_pt->d = n_nodes;
			EGeListDel (&(node_pt->T_cut));
 ...
 * network.
 * */
static inline int ldbl_EGalgPRglobalRelabel (ldbl_EGalgPRgraph_t * const G,
																				ldbl_EGalgPRnode_t * const s,
																				ldbl_EGalgPRnode_t * const t,
																				...
																				EGeList_t * const DIST)
{
	...
	{
		active = EGcontainerOf (n_it, ldbl_EGalgPRnode_t, v.node_cn);
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		...
		if (active == s)
			continue;
		d = active->d;
		/* discard nodes cut away from the ldbl_source */
		if (d >= n_nodes)
			continue;
		/* discard non-active nodes */
		if (ldbl_EGlpNumIsLeq (active->e, ldbl_epsLpNum))
			continue;
		/* add the active node to their corresponding active list */
		MESSAGE (ldbl___PR_VERBOSE__, "Activating node %p with level %u excess %lg",
						 (void *) active, d, ldbl_EGlpNumToLf (active->e));
										 ...
										 ldbl_EGalgPRnode_t * const t)
{
	...
	ldbl_EGalgPRnode_t *active = 0;
#if ldbl_EG_PR_RELABEL
	unsigned const int limit = ldbl_EG_PR_RELABEL_FREC * n_nodes;
#endif
	MESSAGE (ldbl___PR_VERBOSE__, "entering ");
	...
	ldbl_EGalgPRpreprocess (G, s, t, numb, LEVEL, DIST);
	...
	if (i < n_nodes)
	{
		rval = ldbl_EGalgPRnumb (i, max_numb, n_nodes, numb, DIST);
	...
	{
		level--;
		ldbl_UPDATE (ldbl___PR_profile_lvel);
	}
	/* now we do the loop among active nodes */
	while (level > 0)
	{
		MESSAGE (ldbl___PR_VERBOSE__, "level %u", level);
		active = EGcontainerOf (LEVEL[level].next, ldbl_EGalgPRnode_t, LVL_list);
		old_d = active->d;
		ldbl_EGalgPRpushRelabel (active, numb, n_nodes, LEVEL, DIST);
		...
		if (active->d < n_nodes && old_d < active->d)
		{
			...
			if (!numb[old_d])
			{
				rval = ldbl_EGalgPRnumb (old_d, max_numb, n_nodes, numb, DIST);
				CHECKRVALG (rval, CLEANUP);
				max_numb = old_d;
			}
		}
		...
		{
#if ldbl_EG_PR_RELABEL
			if (ldbl___last_global > limit)
			{
				ldbl___last_global = 0;
				rval = ldbl_EGalgPRglobalRelabel (G, s, t, n_nodes, numb, LEVEL, DIST);
				CHECKRVALG (rval, CLEANUP);
				level = n_nodes - 1;
				max_numb = n_nodes;
			}
											...
											ldbl_EGalgPRnode_t * const t)
{
	...
	rval = ldbl_EGalgPRglobalRelabel (G, t, s, n_nodes, numb, LEVEL, DIST);
	CHECKRVALG (rval, CLEANUP);
													 ...
													 long double * error)
{
	...
	for (i = n_nodes; i--;)
		EGeListInit (DIST + i);
	ldbl_EGlpNumZero (*error);
	ldbl_EGlpNumInitVar (flow);
	ldbl_EGlpNumInitVar (cut);
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		node_pt = EGcontainerOf (n_it, ldbl_EGalgPRnode_t, v.node_cn);
		node_pt->LVL_list = (EGeList_t)
		{
		0, 0};
		node_pt->d = n_nodes;
	...
	for (; n_it != n_head; n_it = n_it->next)
	{
		...
		{
			ldbl_EGlpNumSetToMaxAbs (*error, node_pt->e);
			/* check that the excess condition holds within epsilon */
			if (ldbl_EGlpNumIsNeqZero (node_pt->e, ldbl_epsLpNum))
			{
				MESSAGE (ldbl___PR_TEST_VERBOSE__, "excess %lg for node %p is above error",
								 ldbl_EGlpNumToLf (node_pt->e), (void *) node_pt);
				rval++;
			}
		}
		/* check flow condition and compute cut value */
		ldbl_EGlpNumZero (flow);
		e_head = &(node_pt->v.out_edge);
		e_it = e_head->next;
		for (; e_it != e_head; e_it = e_it->next)
		{
			edge_pt = EGcontainerOf (e_it, ldbl_EGalgPRse_t, e.tail_cn);
			inv_node = EGcontainerOf (edge_pt->e.head, ldbl_EGalgPRnode_t, v);
			if (ldbl_EGlpNumIsLess (edge_pt->r, edge_pt->u))
			{
				ldbl_EGlpNumAddTo (flow, edge_pt->u);
				ldbl_EGlpNumSubTo (flow, edge_pt->r);
			}
			if ((node_pt->d >= n_nodes) && (inv_node->d < n_nodes))
			{
				ldbl_EGlpNumAddTo (cut, edge_pt->u);
			}
		}
		...
		for (; e_it != e_head; e_it = e_it->next)
		{
			edge_pt = EGcontainerOf (e_it, ldbl_EGalgPRse_t, e.head_cn);
			inv_node = EGcontainerOf (edge_pt->e.tail, ldbl_EGalgPRnode_t, v);
			if (ldbl_EGlpNumIsLess (edge_pt->r, edge_pt->u))
			{
				ldbl_EGlpNumSubTo (flow, edge_pt->u);
				ldbl_EGlpNumAddTo (flow, edge_pt->r);
			}
		}
		if (node_pt != s && node_pt != t)
		{
			ldbl_EGlpNumSetToMaxAbs (*error, flow);
			if (ldbl_EGlpNumIsNeqZero (flow, ldbl_epsLpNum))
			{
				rval++;
				MESSAGE (ldbl___PR_TEST_VERBOSE__,
	...
	ldbl_EGlpNumCopy (flow, t->e);
	ldbl_EGlpNumAddTo (flow, s->e);
	ldbl_EGlpNumSetToMaxAbs (*error, flow);
	if (ldbl_EGlpNumIsNeqZero (flow, ldbl_epsLpNum))
	{
		rval++;
		MESSAGE (ldbl___PR_TEST_VERBOSE__, "flow balance between s and t is violated"
						 " by %lg", ldbl_EGlpNumToLf (flow));
	}
	ldbl_EGlpNumCopy (flow, s->e);
	ldbl_EGlpNumAddTo (flow, cut);
	ldbl_EGlpNumSetToMaxAbs (*error, flow);
	if (ldbl_EGlpNumIsNeqZero (flow, ldbl_epsLpNum))
	{
		rval++;
		MESSAGE (ldbl___PR_TEST_VERBOSE__, "s-flow and cut don't agree by %lg",
						 ldbl_EGlpNumToLf (flow));
	}
