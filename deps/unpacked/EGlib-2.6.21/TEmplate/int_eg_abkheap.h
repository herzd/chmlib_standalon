 ...
 * @par History:
 * - 2013-04-12
 * 						- First implementation, starting from base int_EGeHeap_t 
 * @note 
...
/** @{ */
/** @example int_eg_abkheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaBKheap */
/* ========================================================================= */
#ifndef int___EG_ABKHEAP__
#define int___EG_ABKHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef int_EG_ABKHEAP_DEBUG
#define int_EG_ABKHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if int_EG_ABKHEAP_DEBUG <= DEBUG
#define int_EGaBKheapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define int_EGaBKheapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define int_EGaBKheapCHECK_CN(__hp,__hcn) 
#define int_EGaBKheapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
/** @brief number of maximum entries in the vector values */
#ifndef int_EG_ABKHEAP_ENTRY
#define int_EG_ABKHEAP_ENTRY 3
#endif
/* ========================================================================= */
...
typedef struct 
{
	int val[int_EG_ABKHEAP_ENTRY];		/**< Value of this node in the heap */
	uint32_t pos;					/**< Position in the heap-array for this node, if set to
												 #int_EG_ABKHEAP_POISON, then the connector is not in any 
												 heap.*/
}
int_EGaBKheapCn_t;
/* ========================================================================= */
/** @brief Poison position for heap connector not in a heap. */
#define int_EG_ABKHEAP_POISON UINT32_MAX
/* ========================================================================= */
...
#define int_EGaBKheapIsLess(__hcn1,__hcn2) ({\
	const int*const __EGABKH1 = (__hcn1);\
	const int*const __EGABKH2 = (__hcn2);\
	int __EGABKHj = 0, __EGABKHrval = 0;\
	for( ; __EGABKHj < int_EG_ABKHEAP_ENTRY ; __EGABKHj++)\
	{\
		if(int_EGlpNumIsLess(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj])){\
			__EGABKHrval = 1; break;}\
		else if (int_EGlpNumIsNeq(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj],int_epsLpNum)){\
 ...
 * */
#define int_EGaBKheapCnInit(__hcn) do{\
	int_EGaBKheapCn_t*const __ABKHcn=(__hcn);\
	int __ABKHi = int_EG_ABKHEAP_ENTRY;\
	memset(__ABKHcn,0,sizeof(int_EGaBKheapCn_t));\
	for( ; __ABKHi-- ; ){int_EGlpNumInitVar(__ABKHcn->val[__ABKHi]);}\
	__ABKHcn->pos = int_EG_ABKHEAP_POISON;}while(0)
/* ========================================================================= */
 ...
 * */
#define int_EGaBKheapCnReset(__hcn) ((__hcn)->pos = int_EG_ABKHEAP_POISON)
/* ========================================================================= */
 ...
 * */
#define int_EGaBKheapCnClear(__hcn) do{\
	int_EGaBKheapCn_t*const __ABKHcn = (__hcn);\
	int __ABKHi = int_EG_ABKHEAP_ENTRY;\
	for( ; __ABKHi-- ; ){int_EGlpNumClearVar(__ABKHcn->val[__ABKHi]);}}while(0)
/* ========================================================================= */
...
#define int_EGaBKheapIsFull(__hp) ({int_EGaBKheap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define int_EGaBKheapEmpty(__hp) ((__hp)->sz = 0)

/* ========================================================================= */
/** @brief Initialize a heap as an empty heap (with no space for conectors).
 * @param __hp heap to initialize.
 * */
#define int_EGaBKheapInit(__hp) (*(__hp) = (int_EGaBKheap_t){0,0,0})
 ...
 * */
#define int_EGaBKheapReset(__hp) int_EGaBKheapResize(__hp,0)
 ...
 * */
#define int_EGaBKheapClear(__hp) int_EGaBKheapResize(__hp,0)

/* ========================================================================= */
/** @brief get the minimum conector in the heap, if the heap is empty, return
 * int_EG_ABKHEAP_POISON.
 * @param __hp eap where we are working.
 * @return position (in the base array) of the minimum element in the heap.
 * */
#define int_EGaBKheapGetMin(__hp) ({\
	int_EGaBKheap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : int_EG_ABKHEAP_POISON;})
 ...
 * */
#define int_EGaBKheapResize(__hp,__new_sz) ({\
	int_EGaBKheap_t*const __EGehp = (__hp);\
 ...
 * */
#define int_EGaBKheapFatherId(__id) ((uint32_t)((__id)?(((uint32_t)((__id)-1))>>1U):0))
 ...
 * */
#define int___EGaBKheapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const uint32_t __EGstsz3=((uint32_t)(__psz3));\
	int_EGaBKheap_t*const __EGehp3 = (__hp3);\
	const uint32_t __EGapos3 = __EGehp3->cn[__hcn3];\
	int_EGaBKheapCn_t*const __EGecn3 = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	uint32_t __EGcpos3 = __EGecn3->pos;\
	uint32_t __EGfpos3 = int_EGaBKheapFatherId(__EGcpos3);\
	int_EGaBKheapCn_t*__EGfcn3 = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	int_EGaBKheapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				int_EGaBKheapIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((uint32_t)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = int_EGaBKheapFatherId(__EGcpos3);\
		__EGfcn3 = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define int_EGaBKheapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	int_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	int_EGaBKheapCn_t*const __EGlcn = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	int_EGaBKheapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	int___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
/** @brief Give the first child for a given position.
 * @param __id position that we want to get the first child.
 * */
#define int_EGaBKheapFirstChildId(__id) ((uint32_t)(((uint32_t)(__id))<<1U)+1U)
/* ========================================================================= */
 ...
 * */
#define int___EGaBKheapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const uint32_t __EGstsz=((uint32_t)(__psz));\
	int_EGaBKheap_t*const __EGehp = (__hp);\
	const uint32_t __EGapos = __EGehp->cn[__hcn];\
	int_EGaBKheapCn_t*const __EGecn = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const uint32_t __EGhsz = __EGehp->sz;\
	uint32_t __EGcpos = __EGecn->pos;\
	uint32_t __EGfchd = int_EGaBKheapFirstChildId(__EGcpos);\
	uint32_t __EGlchd = ((uint32_t)(__EGfchd + 2U));\
	int_EGaBKheapCn_t*__EGcchd=0,*__EGnchd=0;\
	int_EGaBKheapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		if(__EGlchd<=__EGhsz){\
			__EGnchd = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd+1])));\
			if(int_EGaBKheapIsLess(__EGnchd->val,__EGcchd->val)) __EGcchd=__EGnchd;}\
		/* if the minimum child is less than the current position, move the minimum\
		 * child to the position of the current element */\
		if(int_EGaBKheapIsLess(__EGcchd->val,__EGecn->val))\
		{\
			...
			__EGfchd = int_EGaBKheapFirstChildId(__EGcpos);\
			__EGlchd = ((uint32_t)(__EGfchd + 2U));\
		}\
		/* else we exit the main loop */\
		else __EGfchd = int_EG_ABKHEAP_POISON;\
	}\
}while(0)
/* ========================================================================= */
 ...
 * */
#define int_EGaBKheapCopyVal(__dst,__src) do{\
	int*const __EGABKHdst=(__dst);\
	const int*const __EGABKHsrc=(__src);\
	int __EGABKHi=int_EG_ABKHEAP_ENTRY;\
	for(;__EGABKHi--;){int_EGlpNumCopy(__EGABKHdst[__EGABKHi],__EGABKHsrc[__EGABKHi]);}}while(0)
/* ========================================================================= */
 ...
 * */
#define int_EGaBKheapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	int_EGaBKheap_t*const __EGlhp=(__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	int_EGaBKheapCn_t*__EGlcn2=((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	if(int_EGaBKheapIsLess(__new_val,__EGlcn2->val)){\
		int_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		int___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		int_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		int___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define int_EGaBKheapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	int_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	int_EGaBKheapCn_t*const __EGlhpcn = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const uint32_t __EGlcn = __EGlhpcn->pos;\
	const uint32_t __EGlhsz = __EGlhp->sz - 1;\
	int_EGaBKheapCn_t*const __EGllcn = ((int_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = int_EG_ABKHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		int___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = int_EG_ABKHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of int_eg_abkheap.h */
#endif
