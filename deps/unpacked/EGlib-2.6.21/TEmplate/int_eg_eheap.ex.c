...
void int_eheap_usage (char *program)
{
...
int int_eheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 int * v,
					...
					char **argv)
{
	...
	unsigned int nval;
	int v;
	int_EGeHeap_t my_heap;
	int_EGeHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	int_EGlpNumZero (v);
	int_EGeHeapInit (&my_heap);
	rval = int_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = int_eheap_parseargs (argc, argv, &d, &c, &v, &int_file_name);
	CHECKRVALG (rval, CLEANUP);
	int_EGeHeapChangeD (&my_heap, d);
	rval = int_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	...
	fscanf (file, "%u", &nval);
	all_cn = EGsMalloc (int_EGeHeapCn_t, nval);
	int_EGeHeapResize (&my_heap, nval);
	rval = int_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		int_EGeHeapCnInit (all_cn + i);
		fscanf (file, "%s", str1);
		int_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1,
						 int_EGlpNumToLf (all_cn[i].val));
		rval = int_EGeHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = int_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = int_EGeHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %zd : %lg", i, ccn - all_cn,
						 int_EGlpNumToLf (ccn->val));
		int_EGeHeapDel (&my_heap, ccn);
		rval = int_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 int_EGlpNumToLf (all_cn[c].val), int_EGlpNumToLf (v));
	rval = int_EGeHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = int_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
