...
/** @file 
 * @ingroup EGdMatrix
 * @brief This file shows some examples of the use of #mpq_EGdMatrix_t structure 
...
#define mpq_HILBERT_TABLE_SIZE 9U

/* ========================================================================= */
/** @brief table containing 1/eigenvalues of the hilbert matrix. */
static unsigned mpq_dmatrix_hilbert_eigenvalues[mpq_HILBERT_TABLE_SIZE] = {
	1U,
	12U,
	180U,
	2800U,
...
void mpq_dmatrix_usage (char *program)
{
...
int mpq_dmatrix_parseargs (int argc,
											 char **argv,
											 size_t * n,
											 size_t * m)
{
					...
					char **argv)
{
	...
	register unsigned i = 0,
	  j = 0;
	unsigned rank;
	mpq_EGdMatrix_t dmatrix;
	mpq_EGdBsRed_t bsred;
	mpq_t error;
	EGlib_info();
	EGlib_version();
	/* start */
	EGlpNumStart();
	mpq_EGlpNumInitVar (error);
	mpq_EGdMatrixInit (&dmatrix);
	mpq_EGdBsRedInit (&bsred);
	/* parse args */
	rval = mpq_dmatrix_parseargs (argc, argv, &n, &m);
	CHECKRVALG (rval, CLEANUP);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	/* set the matrix size */
	mpq_EGdMatrixSetDimension (&dmatrix, n, m);
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
		{
			mpq_EGlpNumOne (dmatrix.matrow[i][j]);
			mpq_EGlpNumDivUiTo (dmatrix.matrow[i][j], i + j + 1);
		}
	/* Display the hilbert matrix */
	fprintf (stdout, "Hilber ");
	mpq_EGdMatrixDisplay (&dmatrix, 1, stdout);
	fflush (stdout);
	/* now we call gaussian elimination */
	rval =
		mpq_EGdMatrixGaussianElimination (&dmatrix, 0, 0, &rank, mpq_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	fprintf (stdout, "Gaussian Elimination ended with status %d, rank %u and ",
					 status, rank);
	mpq_EGdMatrixDisplay (&dmatrix, 0, stdout);
	fflush (stdout);
	/* now we compute the worst error in the diagonal up to
	 * EGmin(mpq_HILBERT_TABLE_SIZE,rank) */
	fprintf (stdout, "Relative errors in eigenvalues:\n");
	for (i = EGmin (mpq_HILBERT_TABLE_SIZE, rank); i--;)
	{
		mpq_EGlpNumCopy (error, dmatrix.matrow[dmatrix.row_ord[i]][dmatrix.col_ord[i]]);
		fprintf (stdout, "Lambda_%u = %lg", i + 1, mpq_EGlpNumToLf (error));
		mpq_EGlpNumMultUiTo (error, mpq_dmatrix_hilbert_eigenvalues[i]);
		mpq_EGlpNumSubTo (error, mpq_oneLpNum);
		fprintf (stdout, " error = %lg\n", mpq_EGlpNumToLf (error));
	}
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
	...
	/* ending */
	mpq_EGdBsRedSetLength (&bsred, m);
	for (i = 0; i < rank; i++)
		mpq_EGdBsRedAddElement (&bsred, dmatrix.matrow[dmatrix.row_ord[i]]);
	/* now call the Basis Reduction Algorithm and print the reduced basis */
	rval = mpq_EGdBsRed (&bsred, &rank, mpq_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	...
	fflush (stdout);
CLEANUP:
	mpq_EGdBsRedProfile (stderr);
	mpq_EGlpNumClearVar (error);
	mpq_EGdMatrixClear (&dmatrix);
	mpq_EGdBsRedClear (&bsred);
	MESSAGE(0,"done");
	EGlpNumClear();
	return rval;
}
