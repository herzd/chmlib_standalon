					...
					char **argv)
{
	...
	EGfp20_t v;
	fp20_EGaHeap_t my_heap;
	fp20_EGaHeapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = fp20_EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, fp20_EGlpNumToLf (all_cn[ccn].val));
		fp20_EGaHeapDel (all_cn,sizeof(fp20_EGaHeapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) fp20_EGaHeapAdd (all_cn,sizeof(fp20_EGaHeapCn_t),&my_heap, i);
	...
	fp20_EGaHeapChangeVal (all_cn,sizeof(fp20_EGaHeapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
