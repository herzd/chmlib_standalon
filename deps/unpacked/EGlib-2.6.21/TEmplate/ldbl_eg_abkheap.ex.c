					...
					char **argv)
{
	...
	long double v[ldbl_EG_ABKHEAP_ENTRY];
	ldbl_EGaBKheap_t my_heap;
	ldbl_EGaBKheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for(i=0;i<ldbl_EG_ABKHEAP_ENTRY;i++)
	{
		ldbl_EGlpNumInitVar (v[i]);
	...
	ldbl_EGaBKheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (ldbl_file_name, "r");
	TEST (!file, "unable to open file %s", ldbl_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(ldbl_EGaBKheapCn_t, nval);
	ldbl_EGaBKheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		ldbl_EGaBKheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		ldbl_EGlpNumReadStr (all_cn[i].val[0], str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, ldbl_EGlpNumToLf (all_cn[i].val[0]));
		ldbl_EGaBKheapAdd (all_cn,sizeof(ldbl_EGaBKheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = ldbl_EGaBKheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, ldbl_EGlpNumToLf (all_cn[ccn].val[0]));
		ldbl_EGaBKheapDel (all_cn,sizeof(ldbl_EGaBKheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) ldbl_EGaBKheapAdd (all_cn,sizeof(ldbl_EGaBKheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, ldbl_EGlpNumToLf (all_cn[c].val[0]), ldbl_EGlpNumToLf (v[0]));
	ldbl_EGaBKheapChangeVal (all_cn,sizeof(ldbl_EGaBKheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, ldbl_EGlpNumToLf (all_cn[ccn].val[0]));
		ldbl_EGaBKheapDel (all_cn,sizeof(ldbl_EGaBKheapCn_t),&my_heap, ccn);
	}
...
CLEANUP:
	...
	ldbl_EGaBKheapClear(&my_heap);
	for(i=0;i<ldbl_EG_ABKHEAP_ENTRY;i++)
	{
		ldbl_EGlpNumClearVar(v[i]);
	}
	EGlpNumClear();
	return rval;
}
