...
void mpf_ekheap_usage (char *program)
{
...
int mpf_ekheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 mpf_t * v,
					...
					char **argv)
{
	...
	EGioFile_t *file = 0;
	unsigned int nval = 0;
	mpf_t v[mpf_EG_EKHEAP_ENTRY];
	mpf_EGeKHeap_t my_heap;
	mpf_EGeKHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	for( i = mpf_EG_EKHEAP_ENTRY ; i-- ; )
	{
		mpf_EGlpNumInitVar (v[i]);
		mpf_EGlpNumZero (v[i]);
	}
	mpf_EGeKHeapInit (&my_heap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	rval = mpf_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = mpf_ekheap_parseargs (argc, argv, &d, &c, v, &mpf_file_name);
	CHECKRVALG (rval, CLEANUP);
	mpf_EGeKHeapChangeD (&my_heap, d);
	rval = mpf_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Read the objects to sort from the file */
	file = EGioOpen (mpf_file_name, "r");
	...
	nval = atoi(argv1[0]);
	all_cn = EGsMalloc (mpf_EGeKHeapCn_t, nval);
	mpf_EGeKHeapResize (&my_heap, nval);
	rval = mpf_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE(1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpf_EGeKHeapCnInit (all_cn + i);
		EGioGets(str1,1024,file);
		EGioNParse(str1,128," ","%#",&argc1,argv1);
		TESTG((rval=(argc1!=1)),CLEANUP,"Expected one token, found %d",argc1);
		mpf_EGlpNumReadStr (all_cn[i].val[0], argv1[0]);
		IFMESSAGE(1, "Adding value (%s,%lf) to the heap", str1,
						 mpf_EGlpNumToLf (all_cn[i].val[0]));
		rval = mpf_EGeKHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = mpf_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpf_EGeKHeapGetMin (&my_heap);
		IFMESSAGE(1, "%u: item %zd : %lf", i, ccn - all_cn,
						 mpf_EGlpNumToLf (ccn->val[0]));
		mpf_EGeKHeapDel (&my_heap, ccn);
		rval = mpf_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 mpf_EGlpNumToLf (all_cn[c].val[0]), mpf_EGlpNumToLf (v[0]));
	rval = mpf_EGeKHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = mpf_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
...
CLEANUP:
	...
	if (all_cn)
	{
		for( i = nval ; i-- ; ) mpf_EGeKHeapCnClear(all_cn+i);
		EGfree (all_cn);
	}
	mpf_EGeKHeapClear (&my_heap);
	for( i = mpf_EG_EKHEAP_ENTRY ; i-- ; ) mpf_EGlpNumClearVar (v[i]);
	EGlpNumClear();
	return rval;
}
/* ========================================================================= */
