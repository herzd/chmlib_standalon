					...
					char **argv)
{
	...
	double v;
	dbl_EGaHeap_t my_heap;
	dbl_EGaHeapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = dbl_EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, dbl_EGlpNumToLf (all_cn[ccn].val));
		dbl_EGaHeapDel (all_cn,sizeof(dbl_EGaHeapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) dbl_EGaHeapAdd (all_cn,sizeof(dbl_EGaHeapCn_t),&my_heap, i);
	...
	dbl_EGaHeapChangeVal (all_cn,sizeof(dbl_EGaHeapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
