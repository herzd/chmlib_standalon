...
static inline void fp20_pr_usage (char **argv)
{
					...
					char **argv)
{
	...
	EGfp20_t cap;
	EGtimer_t cut_timer;
	fp20_EGalgPRgraph_t G;
	fp20_EGalgPRnode_t *nodes = 0/*,
	 *node_pt*/;
	fp20_EGalgPRedge_t *edges = 0;
	/*EGeList_t *n_it;*/
	char l_str[1024];
	char*l_argv[128];
	int l_argc;
	...
	fprintf (stderr, "Reading graph on %u nodes and %u edges...", n_nodes,
					 n_edges);
	nodes = EGsMalloc (fp20_EGalgPRnode_t, n_nodes);
	edges = EGsMalloc (fp20_EGalgPRedge_t, n_edges);
	for (i = n_nodes; i--;)
	{
		fp20_EGalgPRnodeInit (nodes + i);
		EGeDgraphAddNode (&(G.G), &(nodes[i].v));
	}
	for (i = n_edges; i--;)
	{
		fp20_EGalgPRedgeInit (edges + i);
		EGioGets(l_str,1024,in_file);
		EGioNParse(l_str,128," ","%#",&l_argc,l_argv);
		...
		fp20_EGlpNumCopy (edges[i].fw.u, cap);
		fp20_EGlpNumCopy (edges[i].bw.u, cap);
	}
	EGioClose(in_file);
	in_file = 0;
	...
	rval = fp20_EGalgPRminSTcut (&G, nodes, nodes + (n_nodes >> 1));
	...
	EGtimerReset (&cut_timer);
	EGtimerStart (&cut_timer);
	rval = fp20_EGalgPRmaxSTflow (&G, nodes, nodes + (n_nodes >> 1));
	EGtimerStop (&cut_timer);
	fprintf (stderr, "...done in %lf seconds\n", cut_timer.time);
	/* now we print the value of the cut */
	fprintf (stderr, "Minimum Cut Capacity %lf\n",
					 fp20_EGlpNumToLf (nodes[n_nodes >> 1].e));
	/* check the solution */
	rval = fp20_EGalgPRoptimalityTest (&G, nodes, nodes + (n_nodes >> 1), &cap);
	TESTG (rval, CLEANUP, "Worst error %lg, standard expected error %lg,"
				 " number of errors %d", fp20_EGlpNumToLf (cap), fp20_EGlpNumToLf (fp20_epsLpNum),
				 rval);
...
CLEANUP:
	...
	fp20_EGalgPRgraphClear (&G);
	for (i = n_nodes; i--;)
		fp20_EGalgPRnodeClear (nodes + i);
	for (i = n_edges; i--;)
		fp20_EGalgPRedgeClear (edges + i);
	EGfree (nodes);
	EGfree (edges);
	fp20_EGalgPRprofile();
	EGlpNumClear();
