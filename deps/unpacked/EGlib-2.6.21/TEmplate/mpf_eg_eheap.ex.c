...
void mpf_eheap_usage (char *program)
{
...
int mpf_eheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 mpf_t * v,
					...
					char **argv)
{
	...
	unsigned int nval;
	mpf_t v;
	mpf_EGeHeap_t my_heap;
	mpf_EGeHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	mpf_EGlpNumZero (v);
	mpf_EGeHeapInit (&my_heap);
	rval = mpf_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = mpf_eheap_parseargs (argc, argv, &d, &c, &v, &mpf_file_name);
	CHECKRVALG (rval, CLEANUP);
	mpf_EGeHeapChangeD (&my_heap, d);
	rval = mpf_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	...
	fscanf (file, "%u", &nval);
	all_cn = EGsMalloc (mpf_EGeHeapCn_t, nval);
	mpf_EGeHeapResize (&my_heap, nval);
	rval = mpf_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		mpf_EGeHeapCnInit (all_cn + i);
		fscanf (file, "%s", str1);
		mpf_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1,
						 mpf_EGlpNumToLf (all_cn[i].val));
		rval = mpf_EGeHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = mpf_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = mpf_EGeHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %zd : %lg", i, ccn - all_cn,
						 mpf_EGlpNumToLf (ccn->val));
		mpf_EGeHeapDel (&my_heap, ccn);
		rval = mpf_EGeHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 mpf_EGlpNumToLf (all_cn[c].val), mpf_EGlpNumToLf (v));
	rval = mpf_EGeHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = mpf_EGeHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
