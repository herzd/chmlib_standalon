...
/** @file 
 * @ingroup EGdMatrix
 * @brief This file shows some examples of the use of #dbl_EGdMatrix_t structure 
...
#define dbl_HILBERT_TABLE_SIZE 9U

/* ========================================================================= */
/** @brief table containing 1/eigenvalues of the hilbert matrix. */
static unsigned dbl_dmatrix_hilbert_eigenvalues[dbl_HILBERT_TABLE_SIZE] = {
	1U,
	12U,
	180U,
	2800U,
...
void dbl_dmatrix_usage (char *program)
{
...
int dbl_dmatrix_parseargs (int argc,
											 char **argv,
											 size_t * n,
											 size_t * m)
{
					...
					char **argv)
{
	...
	register unsigned i = 0,
	  j = 0;
	unsigned rank;
	dbl_EGdMatrix_t dmatrix;
	dbl_EGdBsRed_t bsred;
	double error;
	EGlib_info();
	EGlib_version();
	/* start */
	EGlpNumStart();
	dbl_EGlpNumInitVar (error);
	dbl_EGdMatrixInit (&dmatrix);
	dbl_EGdBsRedInit (&bsred);
	/* parse args */
	rval = dbl_dmatrix_parseargs (argc, argv, &n, &m);
	CHECKRVALG (rval, CLEANUP);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	/* set the matrix size */
	dbl_EGdMatrixSetDimension (&dmatrix, n, m);
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
		{
			dbl_EGlpNumOne (dmatrix.matrow[i][j]);
			dbl_EGlpNumDivUiTo (dmatrix.matrow[i][j], i + j + 1);
		}
	/* Display the hilbert matrix */
	fprintf (stdout, "Hilber ");
	dbl_EGdMatrixDisplay (&dmatrix, 1, stdout);
	fflush (stdout);
	/* now we call gaussian elimination */
	rval =
		dbl_EGdMatrixGaussianElimination (&dmatrix, 0, 0, &rank, dbl_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	fprintf (stdout, "Gaussian Elimination ended with status %d, rank %u and ",
					 status, rank);
	dbl_EGdMatrixDisplay (&dmatrix, 0, stdout);
	fflush (stdout);
	/* now we compute the worst error in the diagonal up to
	 * EGmin(dbl_HILBERT_TABLE_SIZE,rank) */
	fprintf (stdout, "Relative errors in eigenvalues:\n");
	for (i = EGmin (dbl_HILBERT_TABLE_SIZE, rank); i--;)
	{
		dbl_EGlpNumCopy (error, dmatrix.matrow[dmatrix.row_ord[i]][dmatrix.col_ord[i]]);
		fprintf (stdout, "Lambda_%u = %lg", i + 1, dbl_EGlpNumToLf (error));
		dbl_EGlpNumMultUiTo (error, dbl_dmatrix_hilbert_eigenvalues[i]);
		dbl_EGlpNumSubTo (error, dbl_oneLpNum);
		fprintf (stdout, " error = %lg\n", dbl_EGlpNumToLf (error));
	}
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
	...
	/* ending */
	dbl_EGdBsRedSetLength (&bsred, m);
	for (i = 0; i < rank; i++)
		dbl_EGdBsRedAddElement (&bsred, dmatrix.matrow[dmatrix.row_ord[i]]);
	/* now call the Basis Reduction Algorithm and print the reduced basis */
	rval = dbl_EGdBsRed (&bsred, &rank, dbl_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	...
	fflush (stdout);
CLEANUP:
	dbl_EGdBsRedProfile (stderr);
	dbl_EGlpNumClearVar (error);
	dbl_EGdMatrixClear (&dmatrix);
	dbl_EGdBsRedClear (&bsred);
	MESSAGE(0,"done");
	EGlpNumClear();
	return rval;
}
