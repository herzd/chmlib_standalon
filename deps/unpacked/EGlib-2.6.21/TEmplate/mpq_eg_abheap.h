...
/** @addtogroup EGaBheap */
/** @{ */
/** @example mpq_eg_abheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaBheap */
/* ========================================================================= */
#ifndef mpq___EG_ABHEAP__
#define mpq___EG_ABHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef mpq_EG_ABHEAP_DEBUG
#define mpq_EG_ABHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if mpq_EG_ABHEAP_DEBUG <= DEBUG
#define mpq_EGaBheapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define mpq_EGaBheapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define mpq_EGaBheapCHECK_CN(__hp,__hcn) 
#define mpq_EGaBheapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
...
typedef struct 
{
	...
	uint32_t pos;					/**< Position in the heap-array for this node, if set to
												 #mpq_EG_ABHEAP_POISON, then the connector is not in any 
												 heap.*/
}
mpq_EGaBheapCn_t;
...
#define mpq_EG_ABHEAP_POISON UINT32_MAX
 ...
 * */
#define mpq_EGaBheapCnInit(__hcn) do{mpq_EGlpNumInitVar((__hcn)->val);(__hcn)->pos = mpq_EG_ABHEAP_POISON;}while(0)
 ...
 * */
#define mpq_EGaBheapCnReset(__hcn) ((__hcn)->pos = mpq_EG_ABHEAP_POISON)
 ...
 * */
#define mpq_EGaBheapCnClear(__hcn) mpq_EGlpNumClearVar((__hcn)->val)
...
#define mpq_EGaBheapIsFull(__hp) ({mpq_EGaBheap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define mpq_EGaBheapEmpty(__hp) ((__hp)->sz = 0)

/* ========================================================================= */
/** @brief Initialize a heap as an empty heap (with no space for conectors).
 * @param __hp heap to initialize.
 * */
#define mpq_EGaBheapInit(__hp) (*(__hp) = (mpq_EGaBheap_t){0,0,0})
 ...
 * */
#define mpq_EGaBheapReset(__hp) mpq_EGaBheapResize(__hp,0)
 ...
 * */
#define mpq_EGaBheapClear(__hp) mpq_EGaBheapResize(__hp,0)

/* ========================================================================= */
/** @brief get the minimum conector in the heap, if the heap is empty, return
 * mpq_EG_ABHEAP_POISON.
 * @param __hp eap where we are working.
 * @return position (in the base array) of the minimum element in the heap.
 * */
#define mpq_EGaBheapGetMin(__hp) ({\
	mpq_EGaBheap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : mpq_EG_ABHEAP_POISON;})
 ...
 * */
#define mpq_EGaBheapResize(__hp,__new_sz) ({\
	mpq_EGaBheap_t*const __EGehp = (__hp);\
 ...
 * */
#define mpq_EGaBheapFatherId(__id) ((uint32_t)((__id)?(((uint32_t)((__id)-1))>>1U):0))
 ...
 * */
#define mpq___EGaBheapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const uint32_t __EGstsz3=((uint32_t)(__psz3));\
	mpq_EGaBheap_t*const __EGehp3 = (__hp3);\
	const uint32_t __EGapos3 = __EGehp3->cn[__hcn3];\
	mpq_EGaBheapCn_t*const __EGecn3 = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	uint32_t __EGcpos3 = __EGecn3->pos;\
	uint32_t __EGfpos3 = mpq_EGaBheapFatherId(__EGcpos3);\
	mpq_EGaBheapCn_t*__EGfcn3 = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	mpq_EGaBheapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
	...
	{\
		...
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = mpq_EGaBheapFatherId(__EGcpos3);\
		__EGfcn3 = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define mpq_EGaBheapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBheapCn_t*const __EGlcn = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpq_EGaBheapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	mpq___EGaBheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
/** @brief Give the first child for a given position.
 * @param __id position that we want to get the first child.
 * */
#define mpq_EGaBheapFirstChildId(__id) ((uint32_t)(((uint32_t)(__id))<<1U)+1U)
/* ========================================================================= */
 ...
 * */
#define mpq___EGaBheapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const uint32_t __EGstsz=((uint32_t)(__psz));\
	mpq_EGaBheap_t*const __EGehp = (__hp);\
	const uint32_t __EGapos = __EGehp->cn[__hcn];\
	mpq_EGaBheapCn_t*const __EGecn = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const uint32_t __EGhsz = __EGehp->sz;\
	uint32_t __EGcpos = __EGecn->pos;\
	uint32_t __EGfchd = mpq_EGaBheapFirstChildId(__EGcpos);\
	uint32_t __EGlchd = ((uint32_t)(__EGfchd + 2U));\
	mpq_EGaBheapCn_t*__EGcchd=0,*__EGnchd=0;\
	mpq_EGaBheapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		if(__EGlchd<=__EGhsz){\
			__EGnchd = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd+1])));\
			if(mpq_EGlpNumIsLeq(__EGnchd->val,__EGcchd->val)) __EGcchd=__EGnchd;}\
		...
		{\
			...
			__EGfchd = mpq_EGaBheapFirstChildId(__EGcpos);\
			__EGlchd = ((uint32_t)(__EGfchd + 2U));\
		}\
		/* else we exit the main loop */\
		else __EGfchd = mpq_EG_ABHEAP_POISON;\
	}\
}while(0)
 ...
 * */
#define mpq_EGaBheapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBheap_t*const __EGlhp=(__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBheapCn_t*__EGlcn2=((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpq_t __EGnum;\
	mpq_EGlpNumCopy(__EGnum,(__new_val));\
	if(mpq_EGlpNumIsLess(__EGnum,__EGlcn2->val)){\
		mpq_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		mpq___EGaBheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		mpq_EGlpNumCopy(__EGlcn2->val,__EGnum);\
		mpq___EGaBheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define mpq_EGaBheapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpq_EGaBheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpq_EGaBheapCn_t*const __EGlhpcn = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const uint32_t __EGlcn = __EGlhpcn->pos;\
	const uint32_t __EGlhsz = __EGlhp->sz - 1;\
	mpq_EGaBheapCn_t*const __EGllcn = ((mpq_EGaBheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = mpq_EG_ABHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		mpq___EGaBheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = mpq_EG_ABHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of mpq_eg_abheap.h */
#endif
#endif
#endif
