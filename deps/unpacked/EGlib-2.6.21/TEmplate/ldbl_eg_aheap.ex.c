					...
					char **argv)
{
	...
	long double v;
	ldbl_EGaHeap_t my_heap;
	ldbl_EGaHeapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	for (i = 0; i < nval; i++)
	{
		ccn = ldbl_EGaHeapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, ldbl_EGlpNumToLf (all_cn[ccn].val));
		ldbl_EGaHeapDel (all_cn,sizeof(ldbl_EGaHeapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) ldbl_EGaHeapAdd (all_cn,sizeof(ldbl_EGaHeapCn_t),&my_heap, i);
	...
	ldbl_EGaHeapChangeVal (all_cn,sizeof(ldbl_EGaHeapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
