...
#include "EGlib.h"
/* ========================================================================= */
/** @brief Tester program for int structure and functions
 * @return zero on success, non-zero otherwise 
 * @par Description:
 * Perform various tests on int and their functions 
 * */
int main (int argc,
					char **argv)
{
	...
	mpq_init (qnum);
	#endif
	int_EGlpNumInitVar (ntmp[0]);
	int_EGlpNumInitVar (ntmp[1]);
	int_EGlpNumInitVar (ntmp[2]);
	int_EGlpNumInitVar (ntmp[3]);
	...
	#endif
	int_EGlpNumReadStr (ntmp[0], argv[1]);
	int_EGlpNumReadStr (ntmp[1], argv[2]);
	dtmp[0] = int_EGlpNumToLf (ntmp[0]);
	dtmp[1] = int_EGlpNumToLf (ntmp[1]);

	/* convert numbers */
	strnum1 = int_EGlpNumGetStr (ntmp[0]);
	strnum2 = int_EGlpNumGetStr (ntmp[1]);
	...
	/* internal constants */
	strnum3 = int_EGlpNumGetStr (int_oneLpNum);
	fprintf (stderr, "1.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = int_EGlpNumGetStr (int_zeroLpNum);
	fprintf (stderr, "0.0 = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = int_EGlpNumGetStr (int_epsLpNum);
	fprintf (stderr, "eps = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = int_EGlpNumGetStr (int_MaxLpNum);
	fprintf (stderr, "Max = %s\n", strnum3);
	EGfree (strnum3);
	strnum3 = int_EGlpNumGetStr (int_MinLpNum);
	fprintf (stderr, "Min = %s\n", strnum3);
	EGfree (strnum3);

	/* copying functions */
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s (%lg)\n", strnum3, strnum1, dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumCopyDiff (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s - %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	int_EGlpNumCopyDiffRatio (ntmp[2], ntmp[0], ntmp[1], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = (%s  - %s)/%s (%lg)\n", strnum3, strnum1, strnum2,
					 strnum1, (dtmp[0] - dtmp[1]) / dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumCopySum (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s + %s (%lg)\n", strnum3, strnum1, strnum2,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	int_EGlpNumCopySqrOver (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s^2/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] * dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumCopyAbs (ntmp[2], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = |%s| (%lg)\n", strnum3, strnum1, fabs (dtmp[0]));
	EGfree (strnum3);
	int_EGlpNumCopyNeg (ntmp[2], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = -1*%s (%lg)\n", strnum3, strnum1, -dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumCopyFrac (ntmp[2], ntmp[1], ntmp[0]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s = %s/%s (%lg)\n", strnum3, strnum2, strnum1,
					 dtmp[1] / dtmp[0]);
	EGfree (strnum3);
	...
	/* add */
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumAddTo (ntmp[2], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s + %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] + dtmp[1]);
	EGfree (strnum3);
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumAddUiTo (ntmp[2], 0xffU);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	...
	/* substract */
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumSubTo (ntmp[2], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] - dtmp[1]);
	EGfree (strnum3);
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumSubUiTo (ntmp[2], 0xffU);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s - %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] - 0xffU);
	EGfree (strnum3);
	...
	/* multiply */
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumMultTo (ntmp[2], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s * %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] * dtmp[1]);
	EGfree (strnum3);
	...
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumSign (ntmp[2]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "-(%s) = %s (%lg)\n", strnum1, strnum3, -dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumOne (ntmp[2]);
	int_EGlpNumAddInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 + %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 + dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	int_EGlpNumOne (ntmp[2]);
	int_EGlpNumSubInnProdTo (ntmp[2], ntmp[0], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "1.0 - %s*%s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 1.0 - dtmp[1] * dtmp[0]);
	EGfree (strnum3);
	...
	/* divide */
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumDivTo (ntmp[2], ntmp[1]);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %s = %s (%lg)\n", strnum1, strnum2, strnum3,
					 dtmp[0] / dtmp[1]);
	EGfree (strnum3);
	int_EGlpNumCopy (ntmp[2], ntmp[0]);
	int_EGlpNumDivUiTo (ntmp[2], 0xffU);
	strnum3 = int_EGlpNumGetStr (ntmp[2]);
	fprintf (stderr, "%s / %u = %s (%lg)\n", strnum1, 0xffU, strnum3,
					 dtmp[0] / 0xffU);
	EGfree (strnum3);
	...
	/* test transformation to rationals */
	{
		...
		mpf_init (f1);
		mpf_init (f2);
		mpf_EGlpNumSet(f1,int_EGlpNumToLf(ntmp[0]));
		mpf_EGlpNumSet(f2,int_EGlpNumToLf(ntmp[1]));
		...
		fprintf (stderr, "Your input in rational was:\n\t(%10.7lf) %s\n\t(%10.7lf)"
				" %s\n", int_EGlpNumToLf (ntmp[0]), strnum1, int_EGlpNumToLf (ntmp[1]),
				strnum2);
		EGfree (strnum1);
		EGfree (strnum2);
	...
	/* ending */
	CLEANUP:
	int_EGlpNumClearVar (ntmp[0]);
	int_EGlpNumClearVar (ntmp[1]);
	int_EGlpNumClearVar (ntmp[2]);
	int_EGlpNumClearVar (ntmp[3]);
	#ifdef HAVE_LIBGMP
	mpq_clear (qnum);
	#endif
	EGlpNumClear();
