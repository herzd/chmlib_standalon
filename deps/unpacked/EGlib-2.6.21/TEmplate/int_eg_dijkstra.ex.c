...
/* ========================================================================= */
#include "EGlib.h"
static char int_fname[1024];
static int32_t int_source=0;
static int int_verbose=1;
/* ========================================================================= */
static inline void int_djk_usage(const char*const progname)
{
...
/* ========================================================================= */
/** @brief parse-args */
static inline void int_djk_parseargs(int argc,char**argv)
{
	int c, has_file=0;
	while((c=getopt(argc,argv,"f:v:hs:"))!=EOF)
	{
		...
		{
			case 'f':
				snprintf(int_fname,1023,"%s",optarg);
				has_file=1;
				break;
			case 'v':
				int_verbose=atoi(optarg);
				break;
			case 'h':
				int_djk_usage(argv[0]);
				exit(EXIT_SUCCESS);
			case 's':
				int_source=atoi(optarg);
				break;
			default:
...
int main (int argc,char**argv)
{
	...
	int_djk_parseargs(argc,argv);
	input = EGioOpen(int_fname,"r");
	EGcallD(int_EGguReadXgraph(input,&n,&m,&edges,&weight));
	EGioClose(input);
	EGtimerStop(&rt);
	EGtimerStart(&pp);
	...
	dist=int_EGlpNumAllocArray(n);
	sweight=int_EGlpNumAllocArray(m);
	...
	for(i=m;i--;)
		int_EGlpNumCopy(sweight[in_perm[i]],weight[i]);
	EGtimerStop(&pp);
	EGtimerStart(&dt);
	EGcallD(int_EGalgDJK(n,m,in_d,in_beg,in_e,sweight,int_source,n,t,father,dist));
	EGtimerStop(&dt);
	...
	if(int_verbose>1)
	{
		for(i=0;i<n;i++)
		{
			fprintf(stderr,"\t\tNode %d father %"PRId32" dist %.3lf\n",i,father[i],int_EGlpNumToLf(dist[i]));
		}
	}
	CLEANUP:
	int_EGlpNumFreeArray(weight);
	int_EGlpNumFreeArray(sweight);
	int_EGlpNumFreeArray(dist);
	EGfree(edges);
	EGfree(in_d);
	EGfree(in_e);
	EGfree(in_beg);
