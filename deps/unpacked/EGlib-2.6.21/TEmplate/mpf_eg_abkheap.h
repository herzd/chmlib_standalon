 ...
 * @par History:
 * - 2013-04-12
 * 						- First implementation, starting from base mpf_EGeHeap_t 
 * @note 
...
/** @{ */
/** @example mpf_eg_abkheap.ex.c
 * This is a simple example of the usage of heaps using @ref EGaBKheap */
/* ========================================================================= */
#ifndef mpf___EG_ABKHEAP__
#define mpf___EG_ABKHEAP__
#include "eg_config.h"
#include "eg_macros.h"
#include "eg_lpnum.h"
/* ========================================================================= */
/** @brief Debug level for the heap */
#ifndef mpf_EG_ABKHEAP_DEBUG
#define mpf_EG_ABKHEAP_DEBUG 1000
#endif

/* ========================================================================= */
/** @name Test macros, enabled only if debug level is high enough. */
/** @{ */
#if mpf_EG_ABKHEAP_DEBUG <= DEBUG
#define mpf_EGaBKheapCHECK_CN(__hp2,__hcn2) EXIT(__hcn2->pos>=__hp2->sz,"Heap Connector possition %d out of range [0,%d]",__hcn2->pos,__hp2->sz)
#define mpf_EGaBKheapCHECK_NF(__hp2) EXIT(__hp2->sz >= __hp2->max_sz,"Heap "#__hp2" Is full, can't add an element")
#else
#define mpf_EGaBKheapCHECK_CN(__hp,__hcn) 
#define mpf_EGaBKheapCHECK_NF(__hp) 
#endif
/** @} */
/* ========================================================================= */
/** @brief number of maximum entries in the vector values */
#ifndef mpf_EG_ABKHEAP_ENTRY
#define mpf_EG_ABKHEAP_ENTRY 3
#endif
/* ========================================================================= */
...
typedef struct 
{
	mpf_t val[mpf_EG_ABKHEAP_ENTRY];		/**< Value of this node in the heap */
	uint32_t pos;					/**< Position in the heap-array for this node, if set to
												 #mpf_EG_ABKHEAP_POISON, then the connector is not in any 
												 heap.*/
}
mpf_EGaBKheapCn_t;
/* ========================================================================= */
/** @brief Poison position for heap connector not in a heap. */
#define mpf_EG_ABKHEAP_POISON UINT32_MAX
/* ========================================================================= */
...
#define mpf_EGaBKheapIsLess(__hcn1,__hcn2) ({\
	const mpf_t*const __EGABKH1 = (__hcn1);\
	const mpf_t*const __EGABKH2 = (__hcn2);\
	int __EGABKHj = 0, __EGABKHrval = 0;\
	for( ; __EGABKHj < mpf_EG_ABKHEAP_ENTRY ; __EGABKHj++)\
	{\
		if(mpf_EGlpNumIsLess(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj])){\
			__EGABKHrval = 1; break;}\
		else if (mpf_EGlpNumIsNeq(__EGABKH1[__EGABKHj], __EGABKH2[__EGABKHj],mpf_epsLpNum)){\
 ...
 * */
#define mpf_EGaBKheapCnInit(__hcn) do{\
	mpf_EGaBKheapCn_t*const __ABKHcn=(__hcn);\
	int __ABKHi = mpf_EG_ABKHEAP_ENTRY;\
	memset(__ABKHcn,0,sizeof(mpf_EGaBKheapCn_t));\
	for( ; __ABKHi-- ; ){mpf_EGlpNumInitVar(__ABKHcn->val[__ABKHi]);}\
	__ABKHcn->pos = mpf_EG_ABKHEAP_POISON;}while(0)
/* ========================================================================= */
 ...
 * */
#define mpf_EGaBKheapCnReset(__hcn) ((__hcn)->pos = mpf_EG_ABKHEAP_POISON)
/* ========================================================================= */
 ...
 * */
#define mpf_EGaBKheapCnClear(__hcn) do{\
	mpf_EGaBKheapCn_t*const __ABKHcn = (__hcn);\
	int __ABKHi = mpf_EG_ABKHEAP_ENTRY;\
	for( ; __ABKHi-- ; ){mpf_EGlpNumClearVar(__ABKHcn->val[__ABKHi]);}}while(0)
/* ========================================================================= */
...
#define mpf_EGaBKheapIsFull(__hp) ({mpf_EGaBKheap_t*const __EGehp = (__hp); __EGehp->sz == __EGehp->max_sz;})

/* ========================================================================= */
/** @brief set the number of elements in hte heap to zero.
 * @param __hp heap to empty.
 * */
#define mpf_EGaBKheapEmpty(__hp) ((__hp)->sz = 0)

/* ========================================================================= */
/** @brief Initialize a heap as an empty heap (with no space for conectors).
 * @param __hp heap to initialize.
 * */
#define mpf_EGaBKheapInit(__hp) (*(__hp) = (mpf_EGaBKheap_t){0,0,0})
 ...
 * */
#define mpf_EGaBKheapReset(__hp) mpf_EGaBKheapResize(__hp,0)
 ...
 * */
#define mpf_EGaBKheapClear(__hp) mpf_EGaBKheapResize(__hp,0)

/* ========================================================================= */
/** @brief get the minimum conector in the heap, if the heap is empty, return
 * mpf_EG_ABKHEAP_POISON.
 * @param __hp eap where we are working.
 * @return position (in the base array) of the minimum element in the heap.
 * */
#define mpf_EGaBKheapGetMin(__hp) ({\
	mpf_EGaBKheap_t*const __EGehp = (__hp);\
	__EGehp->sz ? __EGehp->cn[0] : mpf_EG_ABKHEAP_POISON;})
 ...
 * */
#define mpf_EGaBKheapResize(__hp,__new_sz) ({\
	mpf_EGaBKheap_t*const __EGehp = (__hp);\
 ...
 * */
#define mpf_EGaBKheapFatherId(__id) ((uint32_t)((__id)?(((uint32_t)((__id)-1))>>1U):0))
 ...
 * */
#define mpf___EGaBKheapSiftUp(__pbase3,__psz3,__hp3,__hcn3) do{\
	char*const __EGbase3=((char*)(__pbase3));\
	const uint32_t __EGstsz3=((uint32_t)(__psz3));\
	mpf_EGaBKheap_t*const __EGehp3 = (__hp3);\
	const uint32_t __EGapos3 = __EGehp3->cn[__hcn3];\
	mpf_EGaBKheapCn_t*const __EGecn3 = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGapos3)));\
	uint32_t __EGcpos3 = __EGecn3->pos;\
	uint32_t __EGfpos3 = mpf_EGaBKheapFatherId(__EGcpos3);\
	mpf_EGaBKheapCn_t*__EGfcn3 = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	mpf_EGaBKheapCHECK_CN(__EGehp3,__EGecn3);\
	while(__EGcpos3 && \
				mpf_EGaBKheapIsLess(__EGecn3->val,__EGfcn3->val))\
	{\
		__EGfcn3->pos = __EGcpos3;\
		__EGehp3->cn[__EGcpos3] = ((uint32_t)(EGaGetPos(__EGbase3,__EGstsz3,__EGfcn3)));\
		__EGcpos3 = __EGfpos3;\
		__EGfpos3 = mpf_EGaBKheapFatherId(__EGcpos3);\
		__EGfcn3 = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase3,__EGstsz3,__EGehp3->cn[__EGfpos3])));\
	}\
 ...
 * */
#define mpf_EGaBKheapAdd(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpf_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpf_EGaBKheapCn_t*const __EGlcn = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	mpf_EGaBKheapCHECK_NF(__EGlhp);\
	__EGlcn->pos=__EGlhp->sz,__EGlhp->cn[__EGlhp->sz]=__EGapos2;\
	__EGlhp->sz +=1;\
	mpf___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlhp->sz-1);}while(0)
/* ========================================================================= */
/** @brief Give the first child for a given position.
 * @param __id position that we want to get the first child.
 * */
#define mpf_EGaBKheapFirstChildId(__id) ((uint32_t)(((uint32_t)(__id))<<1U)+1U)
/* ========================================================================= */
 ...
 * */
#define mpf___EGaBKheapSiftDown(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase=((char*)(__pbase));\
	const uint32_t __EGstsz=((uint32_t)(__psz));\
	mpf_EGaBKheap_t*const __EGehp = (__hp);\
	const uint32_t __EGapos = __EGehp->cn[__hcn];\
	mpf_EGaBKheapCn_t*const __EGecn = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGapos)));\
	const uint32_t __EGhsz = __EGehp->sz;\
	uint32_t __EGcpos = __EGecn->pos;\
	uint32_t __EGfchd = mpf_EGaBKheapFirstChildId(__EGcpos);\
	uint32_t __EGlchd = ((uint32_t)(__EGfchd + 2U));\
	mpf_EGaBKheapCn_t*__EGcchd=0,*__EGnchd=0;\
	mpf_EGaBKheapCHECK_CN(__EGehp,__EGecn);\
	while(__EGfchd < __EGhsz)\
	{\
		/* detect the minimum child */\
		__EGcchd = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd])));\
		if(__EGlchd<=__EGhsz){\
			__EGnchd = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase,__EGstsz,__EGehp->cn[__EGfchd+1])));\
			if(mpf_EGaBKheapIsLess(__EGnchd->val,__EGcchd->val)) __EGcchd=__EGnchd;}\
		/* if the minimum child is less than the current position, move the minimum\
		 * child to the position of the current element */\
		if(mpf_EGaBKheapIsLess(__EGcchd->val,__EGecn->val))\
		{\
			...
			__EGfchd = mpf_EGaBKheapFirstChildId(__EGcpos);\
			__EGlchd = ((uint32_t)(__EGfchd + 2U));\
		}\
		/* else we exit the main loop */\
		else __EGfchd = mpf_EG_ABKHEAP_POISON;\
	}\
}while(0)
/* ========================================================================= */
 ...
 * */
#define mpf_EGaBKheapCopyVal(__dst,__src) do{\
	mpf_t*const __EGABKHdst=(__dst);\
	const mpf_t*const __EGABKHsrc=(__src);\
	int __EGABKHi=mpf_EG_ABKHEAP_ENTRY;\
	for(;__EGABKHi--;){mpf_EGlpNumCopy(__EGABKHdst[__EGABKHi],__EGABKHsrc[__EGABKHi]);}}while(0)
/* ========================================================================= */
 ...
 * */
#define mpf_EGaBKheapChangeVal(__pbase,__psz,__hp,__hcn,__new_val) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpf_EGaBKheap_t*const __EGlhp=(__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpf_EGaBKheapCn_t*__EGlcn2=((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	if(mpf_EGaBKheapIsLess(__new_val,__EGlcn2->val)){\
		mpf_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		mpf___EGaBKheapSiftUp(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}\
	else{\
		mpf_EGaBKheapCopyVal(__EGlcn2->val,__new_val);\
		mpf___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn2->pos);}}while(0)
 ...
 * */
#define mpf_EGaBKheapDel(__pbase,__psz,__hp,__hcn) do{\
	char*const __EGbase2=((char*)(__pbase));\
	const uint32_t __EGstsz2=((uint32_t)(__psz));\
	mpf_EGaBKheap_t*const __EGlhp = (__hp);\
	const uint32_t __EGapos2=(uint32_t)(__hcn);\
	mpf_EGaBKheapCn_t*const __EGlhpcn = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGapos2)));\
	const uint32_t __EGlcn = __EGlhpcn->pos;\
	const uint32_t __EGlhsz = __EGlhp->sz - 1;\
	mpf_EGaBKheapCn_t*const __EGllcn = ((mpf_EGaBKheapCn_t*)(EGaGetElem(__EGbase2,__EGstsz2,__EGlhp->cn[__EGlhsz])));\
	__EGlhpcn->pos = mpf_EG_ABKHEAP_POISON, __EGlhp->sz = __EGlhsz;\
	if(__EGlhsz && __EGlhsz != __EGlcn){\
		__EGlhp->cn[__EGlcn] = __EGlhp->cn[__EGlhsz],__EGllcn->pos = __EGlcn;\
		mpf___EGaBKheapSiftDown(__EGbase2,__EGstsz2,__EGlhp,__EGlcn);}\
	__EGlhp->cn[__EGlhp->sz] = mpf_EG_ABKHEAP_POISON;}while(0)

/* ========================================================================= */
/** @} */
/* end of mpf_eg_abkheap.h */
#endif
#endif
#endif
