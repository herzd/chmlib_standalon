...
static inline void dbl_srk_usage (char **argv)
{
	fprintf (stderr, "Usage: %s input_file\n", argv[0]);
}

/* ========================================================================= */
/** @brief given a dbl_EGsrkGraph_t structure, display the current shrunken graph
 * and it's node weights */
static inline void dbl_display_srkG (dbl_EGsrkGraph_t * G,
																 dbl_EGsrkNode_t * const all_nodes,
																 dbl_EGsrkEdge_t * const all_edges)
{
	EGeList_t *n_it,
	 *e_it,
	 *n_end,
	...
	if (!EGeListIsEmpty (n_end))
	{
		...
		{
			lcn = EGcontainerOf (n_it, dbl_EGsrkNode_t, node.node_cn);
			fprintf (stderr, "\t%zd (%lf): ", lcn - all_nodes,
							 dbl_EGlpNumToLf (lcn->weight));
			e_end = &(lcn->members);
			if (!EGeListIsEmpty (e_end))
			{
				...
				{
					onode = EGcontainerOf (e_it, dbl_EGsrkNode_t, members);
			...
			if (!EGeListIsEmpty (e_end))
			{
				...
				for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
				{
					...
					ce = EGcontainerOf (cep, dbl_EGsrkEdge_t, edge.ep[cep->type]);
					fprintf (stderr, "%zd[%lf] ", ce - all_edges,
									 dbl_EGlpNumToLf (ce->weight));
					ee_end = &(ce->members);
					if (!EGeListIsEmpty (ee_end))
					{
						...
						{
							cee = EGcontainerOf (ee_it, dbl_EGsrkEdge_t, members);
					...
					char **argv)
{
	...
	double cap;
	dbl_EGsrkGraph_t G;
	dbl_EGsrkNode_t *all_nodes = 0;
	dbl_EGsrkEdge_t *all_edges = 0;
	dbl_EGsrkNode_t *N1,
	 *N2;
	char l_str[1024];
	...
	all_nodes = EGsMalloc (dbl_EGsrkNode_t, G.n_onodes);
	all_edges = EGsMalloc (dbl_EGsrkEdge_t, G.n_oedges);
	for (i = G.n_onodes; i--;)
	{
		dbl_EGsrkNodeInit (all_nodes + i);
		dbl_EGsrkAddNode (&G, all_nodes + i);
	}
	for (i = G.n_oedges; i--;)
	{
		dbl_EGsrkEdgeInit (all_edges + i);
		n_read = fscanf (in_file, "%u %u %s", &head, &tail, l_str);
		TESTG ((rval = (n_read != 3)), CLEANUP, "couldn't read three fields");
		dbl_EGlpNumReadStr (cap, l_str);
		dbl_EGlpNumCopy (all_edges[i].weight, cap);
		dbl_EGsrkAddEdge (&G, all_nodes + head, all_nodes + tail, all_edges + i);
	}
	fclose (in_file);
	in_file = 0;
	fprintf (stderr, "done\n");
	...
	while (G.G.n_nodes > 2)
	{
		//dbl_display_srkG (&G, all_nodes, all_edges);
		N1 = EGcontainerOf (G.G.nodes.next, dbl_EGsrkNode_t, node.node_cn);
		N2 = EGcontainerOf (N1->node.node_cn.next, dbl_EGsrkNode_t, node.node_cn);
		dbl_EGsrkIdentifyNodes (&G, N1, N2);
	}
	dbl_display_srkG (&G, all_nodes, all_edges);
	/* ending */
CLEANUP:
	...
	for (i = G.n_onodes; i--;)
		dbl_EGsrkNodeClear (all_nodes + i);
	for (i = G.n_oedges; i--;)
		dbl_EGsrkEdgeClear (all_edges + i);
	if (all_nodes)
		EGfree (all_nodes);
	if (all_edges)
		EGfree (all_edges);
	dbl_EGsrkGraphClear (&G);
	EGlpNumClear();
	return rval;
}
