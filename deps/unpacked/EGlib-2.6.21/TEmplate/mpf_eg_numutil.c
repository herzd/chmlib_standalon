										 ...
										 const mpf_t * const elem)
{
	...
	for (;;)
	{
		...
		do
			j--;
		while (j && mpf_EGlpNumIsLess (t, elem[perm[j]]));
		if (j < i)
			break;
										 ...
										 const mpf_t * const elem)
{
	...
	for (;;)
	{
		do
			i++;
		while (i < sz && mpf_EGlpNumIsLess (t, elem[perm[i]]));
		do
			j--;
		while (mpf_EGlpNumIsLess (elem[perm[j]], t));
		if (j < i)
			break;
		EGswap (perm[i], perm[j], temp);
	}
	EGswap (perm[0], perm[j], temp);
	mpf_EGlpNumClearVar (t);
	mpf_EGutilPermSort2 (j, perm, elem);
	mpf_EGutilPermSort2 (sz - i, perm + i, elem);
}

/* ========================================================================= */
void mpf___EGlpNumInnProd(mpf_t*rop,
											mpf_t*const __EGa1,
											mpf_t*const __EGa2, 
											const size_t length )
{
	...
	mpf_EGlpNumZero((*rop));
	while(__EGdim--) mpf_EGlpNumAddInnProdTo((*rop),__EGa1[__EGdim],__EGa2[__EGdim]);
}
/* ========================================================================= */
/** @} */
