					...
					char **argv)
{
	...
	EGfp20_t v;
	fp20_EGaBheap_t my_heap;
	fp20_EGaBheapCn_t *all_cn = 0;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	...
	fp20_EGaBheapInit(&my_heap);

	/* Read the objects to sort from the file */
	file = fopen (fp20_file_name, "r");
	TEST (!file, "unable to open file %s", fp20_file_name);
	fscanf (file, "%u", &nval);
	all_cn=EGsMalloc(fp20_EGaBheapCn_t, nval);
	fp20_EGaBheapResize(&my_heap, nval);
	IFMESSAGE (1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		fp20_EGaBheapCnInit(all_cn+i);
		fscanf (file, "%s", str1);
		fp20_EGlpNumReadStr (all_cn[i].val, str1);
		IFMESSAGE (1, "Adding value (%s,%lg) to the heap", str1, fp20_EGlpNumToLf (all_cn[i].val));
		fp20_EGaBheapAdd (all_cn,sizeof(fp20_EGaBheapCn_t),&my_heap,i);
	}
	fclose (file);
	...
	for (i = 0; i < nval; i++)
	{
		ccn = fp20_EGaBheapGetMin (&my_heap);
		IFMESSAGE (1, "%u: item %u : %lg", i, ccn, fp20_EGlpNumToLf (all_cn[ccn].val));
		fp20_EGaBheapDel (all_cn,sizeof(fp20_EGaBheapCn_t),&my_heap, ccn);
	}
	...
	fprintf (stderr, "\nRe-Inserting.\n\n");
	for (i = 0; i < nval; i++) fp20_EGaBheapAdd (all_cn,sizeof(fp20_EGaBheapCn_t),&my_heap, i);

	/* Changing value of an item */
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c, fp20_EGlpNumToLf (all_cn[c].val), fp20_EGlpNumToLf (v));
	fp20_EGaBheapChangeVal (all_cn,sizeof(fp20_EGaBheapCn_t),&my_heap,c,v);
	/* Popping the values from the heap */
	fprintf (stderr, "\nRemoving:\n\n");
	for (i = 0; i < nval; i++)
	{
		ccn = my_heap.cn[my_heap.sz-1];
		IFMESSAGE (1, "%u: item %u : %lf", i, ccn, fp20_EGlpNumToLf (all_cn[ccn].val));
		fp20_EGaBheapDel (all_cn,sizeof(fp20_EGaBheapCn_t),&my_heap, ccn);
	}
