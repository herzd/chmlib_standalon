...
{
	int_EGsrkNode_t *o_end;
	int_EGsrkEdge_t *c_edge,
	 *o_edge;
	EGeUgraphEP_t *c_ep;
	EGeList_t *e_it,
	 *e_end,
	...
	int_EGlpNumAddTo (base->weight, srkN->weight);
	...
	for (e_it = e_end->next; e_it != e_end; e_it = e_it->next)
	{
		...
		c_edge = EGcontainerOf (c_ep, int_EGsrkEdge_t, edge.ep[c_ep->type]);
		o_type = c_ep->type ? 0U : 1U;
		o_end = EGcontainerOf (c_edge->edge.ep[o_type].node, int_EGsrkNode_t, node);
		o_end->hit = c_edge;
	}
	...
	for (; e_it != e_end; e_it = e_next)
	{
		...
		if (o_end == base)
		{
			int_EGlpNumSubTo (base->weight, c_edge->weight);
			int_EGlpNumSubTo (base->weight, c_edge->weight);
		...
		int_EGlpNumAddTo (o_edge->weight, c_edge->weight);
