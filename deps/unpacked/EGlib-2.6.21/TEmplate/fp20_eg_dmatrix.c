 ...
 * */
#include "fp20_eg_dmatrix.h"
/** @file
 * @ingroup EGdMatrix */
/** @addtogroup EGdMatrix */
/** @{ */
/* ========================================================================= */
int fp20_EGdMatrixGaussianElimination (fp20_EGdMatrix_t * const dmatrix,
																	...
																	int *const ext_status)
{
	...
	EGfp20_t **const rowval = dmatrix->matrow;
	EGfp20_t pivot,
	  tmpval;
	int rval = 0,
	  tmpint = 0,
	...
	while (1)
	{
		...
		pcol = rank;
		prow = rank;
		fp20_EGlpNumCopyAbs (pivot, rowval[row_ord[prow]][col_ord[pcol]]);
		/* in this case we are forced to choose as pivot element a_kk */
		if (!do_col_perm && !do_row_perm)
		{
			if (!fp20_EGlpNumIsNeqZero (pivot, zero_tol))
			{
		...
		else if (do_col_perm && do_row_perm)
		{
			for (i = rank; i < row_sz; i++)
				...
				{
					fp20_EGlpNumCopyAbs (tmpval, rowval[row_ord[i]][col_ord[j]]);
					if (fp20_EGlpNumIsLess (pivot, tmpval))
					{
						pcol = j;
						prow = i;
						fp20_EGlpNumCopy (pivot, tmpval);
					}
				}
			if (!fp20_EGlpNumIsNeqZero (pivot, zero_tol))
			{
		...
		else if (do_col_perm && !do_row_perm)
		{
			...
			for (j = rank; j < col_sz; j++)
			{
				fp20_EGlpNumCopyAbs (tmpval, rowval[tmpint][col_ord[j]]);
				if (fp20_EGlpNumIsLess (pivot, tmpval))
				{
					pcol = j;
					fp20_EGlpNumCopy (pivot, tmpval);
		...
		else
		{
			...
			for (i = rank; i < row_sz; i++)
			{
				fp20_EGlpNumCopyAbs (tmpval, rowval[row_ord[i]][tmpint]);
				if (fp20_EGlpNumIsLess (pivot, tmpval))
				{
					prow = i;
					fp20_EGlpNumCopy (pivot, tmpval);
		 ...
		 * and or rows */
		fp20_EGlpNumCopy (pivot, rowval[row_ord[rank]][col_ord[rank]]);
		/* now we do de pivot */
		tmpint = row_ord[rank];
		for (i = rank + 1; i < row_sz; i++)
		{
			tmpint2 = row_ord[i];
			fp20_EGlpNumCopy (tmpval, rowval[tmpint2][col_ord[rank]]);
			/* check wether we don't need to update this row */
			if (fp20_EGlpNumIsNeqZero (tmpval, zero_tol))
			{
				/* compute the row multiplier */
				fp20_EGlpNumDivTo (tmpval, pivot);
				/* set row_i = row_i - tmpval row_rank */
				fp20_EGlpNumCopy (rowval[tmpint2][col_ord[rank]], fp20_zeroLpNum);
				for (j = rank + 1; j < col_sz; j++)
					fp20_EGlpNumSubInnProdTo (rowval[tmpint2][col_ord[j]], tmpval,
