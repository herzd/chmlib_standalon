...
void int_ekheap_usage (char *program)
{
...
int int_ekheap_parseargs (int argc,
										 char **argv,
										 unsigned int *d,
										 unsigned int *ch,
										 int * v,
					...
					char **argv)
{
	...
	EGioFile_t *file = 0;
	unsigned int nval = 0;
	int v[int_EG_EKHEAP_ENTRY];
	int_EGeKHeap_t my_heap;
	int_EGeKHeapCn_t *all_cn = 0,
	 *ccn;
	EGlib_info();
	EGlib_version();
	EGlpNumStart();
	for( i = int_EG_EKHEAP_ENTRY ; i-- ; )
	{
		int_EGlpNumInitVar (v[i]);
		int_EGlpNumZero (v[i]);
	}
	int_EGeKHeapInit (&my_heap);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	rval = int_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Parse command line input to get 'file name' and 'd'. */
	rval = int_ekheap_parseargs (argc, argv, &d, &c, v, &int_file_name);
	CHECKRVALG (rval, CLEANUP);
	int_EGeKHeapChangeD (&my_heap, d);
	rval = int_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);

	/* Read the objects to sort from the file */
	file = EGioOpen (int_file_name, "r");
	...
	nval = atoi(argv1[0]);
	all_cn = EGsMalloc (int_EGeKHeapCn_t, nval);
	int_EGeKHeapResize (&my_heap, nval);
	rval = int_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
	IFMESSAGE(1, "Inserting %u elements into the heap", nval);
	for (i = 0; i < nval; i++)
	{
		int_EGeKHeapCnInit (all_cn + i);
		EGioGets(str1,1024,file);
		EGioNParse(str1,128," ","%#",&argc1,argv1);
		TESTG((rval=(argc1!=1)),CLEANUP,"Expected one token, found %d",argc1);
		int_EGlpNumReadStr (all_cn[i].val[0], argv1[0]);
		IFMESSAGE(1, "Adding value (%s,%lf) to the heap", str1,
						 int_EGlpNumToLf (all_cn[i].val[0]));
		rval = int_EGeKHeapAdd (&my_heap, all_cn + i);
		CHECKRVALG (rval, CLEANUP);
		rval = int_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	for (i = 0; i < nval; i++)
	{
		ccn = int_EGeKHeapGetMin (&my_heap);
		IFMESSAGE(1, "%u: item %zd : %lf", i, ccn - all_cn,
						 int_EGlpNumToLf (ccn->val[0]));
		int_EGeKHeapDel (&my_heap, ccn);
		rval = int_EGeKHeapCheck (&my_heap);
		CHECKRVALG (rval, CLEANUP);
	}
	...
	fprintf (stderr, "Changing value of item %u from %lf to %lf.\n", c,
					 int_EGlpNumToLf (all_cn[c].val[0]), int_EGlpNumToLf (v[0]));
	rval = int_EGeKHeapChangeVal (&my_heap, all_cn + c, v);
	CHECKRVALG (rval, CLEANUP);
	rval = int_EGeKHeapCheck (&my_heap);
	CHECKRVALG (rval, CLEANUP);
...
CLEANUP:
	...
	if (all_cn)
	{
		for( i = nval ; i-- ; ) int_EGeKHeapCnClear(all_cn+i);
		EGfree (all_cn);
	}
	int_EGeKHeapClear (&my_heap);
	for( i = int_EG_EKHEAP_ENTRY ; i-- ; ) int_EGlpNumClearVar (v[i]);
	EGlpNumClear();
	return rval;
}
/* ========================================================================= */
