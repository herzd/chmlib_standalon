...
/** @file 
 * @ingroup EGdMatrix
 * @brief This file shows some examples of the use of #mpf_EGdMatrix_t structure 
...
#define mpf_HILBERT_TABLE_SIZE 9U

/* ========================================================================= */
/** @brief table containing 1/eigenvalues of the hilbert matrix. */
static unsigned mpf_dmatrix_hilbert_eigenvalues[mpf_HILBERT_TABLE_SIZE] = {
	1U,
	12U,
	180U,
	2800U,
...
void mpf_dmatrix_usage (char *program)
{
...
int mpf_dmatrix_parseargs (int argc,
											 char **argv,
											 size_t * n,
											 size_t * m)
{
					...
					char **argv)
{
	...
	register unsigned i = 0,
	  j = 0;
	unsigned rank;
	mpf_EGdMatrix_t dmatrix;
	mpf_EGdBsRed_t bsred;
	mpf_t error;
	EGlib_info();
	EGlib_version();
	/* start */
	EGlpNumStart();
	mpf_EGlpNumInitVar (error);
	mpf_EGdMatrixInit (&dmatrix);
	mpf_EGdBsRedInit (&bsred);
	/* parse args */
	rval = mpf_dmatrix_parseargs (argc, argv, &n, &m);
	CHECKRVALG (rval, CLEANUP);
	/* set signal and limits */
	EGsigSet(rval,CLEANUP);
	EGsetLimits(3600.0,4294967295UL);
	/* set the matrix size */
	mpf_EGdMatrixSetDimension (&dmatrix, n, m);
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
		{
			mpf_EGlpNumOne (dmatrix.matrow[i][j]);
			mpf_EGlpNumDivUiTo (dmatrix.matrow[i][j], i + j + 1);
		}
	/* Display the hilbert matrix */
	fprintf (stdout, "Hilber ");
	mpf_EGdMatrixDisplay (&dmatrix, 1, stdout);
	fflush (stdout);
	/* now we call gaussian elimination */
	rval =
		mpf_EGdMatrixGaussianElimination (&dmatrix, 0, 0, &rank, mpf_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	fprintf (stdout, "Gaussian Elimination ended with status %d, rank %u and ",
					 status, rank);
	mpf_EGdMatrixDisplay (&dmatrix, 0, stdout);
	fflush (stdout);
	/* now we compute the worst error in the diagonal up to
	 * EGmin(mpf_HILBERT_TABLE_SIZE,rank) */
	fprintf (stdout, "Relative errors in eigenvalues:\n");
	for (i = EGmin (mpf_HILBERT_TABLE_SIZE, rank); i--;)
	{
		mpf_EGlpNumCopy (error, dmatrix.matrow[dmatrix.row_ord[i]][dmatrix.col_ord[i]]);
		fprintf (stdout, "Lambda_%u = %lg", i + 1, mpf_EGlpNumToLf (error));
		mpf_EGlpNumMultUiTo (error, mpf_dmatrix_hilbert_eigenvalues[i]);
		mpf_EGlpNumSubTo (error, mpf_oneLpNum);
		fprintf (stdout, " error = %lg\n", mpf_EGlpNumToLf (error));
	}
	/* set the coefficients of the hilbert matrix */
	for (i = n; i--;)
		for (j = m; j--;)
	...
	/* ending */
	mpf_EGdBsRedSetLength (&bsred, m);
	for (i = 0; i < rank; i++)
		mpf_EGdBsRedAddElement (&bsred, dmatrix.matrow[dmatrix.row_ord[i]]);
	/* now call the Basis Reduction Algorithm and print the reduced basis */
	rval = mpf_EGdBsRed (&bsred, &rank, mpf_epsLpNum, &status);
	CHECKRVALG (rval, CLEANUP);
	...
	fflush (stdout);
CLEANUP:
	mpf_EGdBsRedProfile (stderr);
	mpf_EGlpNumClearVar (error);
	mpf_EGdMatrixClear (&dmatrix);
	mpf_EGdBsRedClear (&bsred);
	MESSAGE(0,"done");
	EGlpNumClear();
	return rval;
}
